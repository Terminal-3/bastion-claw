...
pub struct TestRig {
    ...
    #[cfg(feature = "libsql")]
    skill_registry: Option<Arc<std::sync::RwLock<t3claw_skills::SkillRegistry>>>,
...
impl TestRig {
    ...
    pub async fn get_secret(&self, name: &str) -> Option<String> {
        ...
        match store.get_decrypted(&self.owner_id, name).await {
            ...
            Err(e) => {
                ...
                if !matches!(e, t3claw::secrets::SecretError::NotFound(_)) {
                    eprintln!(
    ...
    #[cfg(feature = "libsql")]
    pub fn secrets_store(&self) -> Option<&Arc<dyn t3claw::secrets::SecretsStore + Send + Sync>> {
    ...
    pub async fn build(self) -> TestRig {
        ...
        // 6. Register job tools, routine tools, and extra tools.
        {
            ...
            if enable_skills {
                ...
                let mut registry = t3claw_skills::SkillRegistry::new(skills_dir.clone())
        ...
        if !pre_seed_secrets.is_empty() {
            if let Some(ref secrets_store) = components.secrets_store {
                use t3claw::secrets::CreateSecretParams;
                ...
                for (name, value) in &pre_seed_secrets {
                    ...
                    match secrets_store.get_decrypted(&owner_id, name).await {
                        ...
                        Err(t3claw::secrets::SecretError::NotFound(_)) => {
