...
- `ProductActor` -> `t3claw_turns::TurnActor`
- explicit projection scope -> `t3claw_event_projections::ProjectionScope`
- outbound/thread fanout scope -> `t3claw_turns::TurnScope`, paired with the actor, projection scope, view, and target used for access authorization before fanout candidates are planned
- product target -> `ProjectionTarget` in `t3claw_event_streams`, carrying existing `ThreadId`, `MissionId`, `InvocationId`, and `ProcessId` types
