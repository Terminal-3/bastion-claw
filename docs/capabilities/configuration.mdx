...
description: Common operator configuration reference for T3Claw
---

T3Claw is configured through bootstrap environment variables, database-backed settings, and a small set of advanced or provider-specific environment variables. This page documents the primary options used during setup and normal operation.
...
<AccordionGroup>
  ...
  <Accordion title="Layer 2: Database Settings" icon="database">
    ...
    Some security-sensitive and advanced options remain environment-only. Manage database-backed values through the wizard or `t3claw config` command.
  </Accordion>
</AccordionGroup>
...
| `T3CLAW_INSTANCE_ID` | Relay instance ID. |
...
| Variable | Description |
|----------|-------------|
| `T3CLAW_BASE_DIR` | Overrides the T3Claw data directory. |
| `T3CLAW_OWNER_ID` | Stable owner scope for the instance. |
| `T3CLAW_PROFILE` | Built-in profile selected during onboarding. |
| `T3CLAW_OAUTH_CALLBACK_URL` | OAuth callback URL used by auth flows. |
| `NEARAI_SESSION_TOKEN` | NEAR AI session token; takes precedence over session file. |
| `T3CLAW_PID_LOCK_PATH` | Process PID lock path override. |
| `T3CLAW_PID_LOCK_HOLD_MS` | PID lock hold duration. |
| `T3CLAW_PID_LOCK_CHILD` | Internal child-process PID lock marker. |
