...
| Durable stores list | Incomplete + wrong path. `gate_resolution.rs` lives in `t3claw_reborn/subagent/`, not `t3claw_loop_support/`. Gate store has 3 denormalized maps under one mutex (atomic migration). A 4th in-memory store (`BoundedSubagentResultTombstoneStore`) is entirely missing from the doc. |
...
5. **Path correction:** `BoundedSubagentGateResolutionStore` is in `crates/t3claw_reborn/src/subagent/gate_resolution.rs`, not `t3claw_loop_support/`.
...
| Reborn-side DB layer convention | Where Reborn-crate persistence code lives (no precedent today). Recommend: new `crates/t3claw_reborn_persistence/` with `libsql/` and `postgres/` subdirs matching v1 `src/db/` shape. |
...
Do NOT create a new `t3claw_reborn_persistence` crate. Two compliance issues:

1. **Boundary-test rule (Reviewer 1 V1):** every active Reborn crate needs a `BoundaryRule` entry in `crates/t3claw_architecture/tests/reborn_dependency_boundaries.rs`. New crate without rule = silent dep escape.
...
**Owner:** `crates/t3claw_reborn_event_store/` — canonical owner of Reborn durable backend selection per `events.md` §2 and `t3claw_events/CLAUDE.md`. Existing boundary rule covers it.
...
- (a) → `crates/t3claw_reborn_event_store/src/{gate_resolution_fs.rs, goal_store_fs.rs, ...}` using `RootFilesystem` adapter
- (b) → `crates/t3claw_reborn_event_store/src/{libsql/, postgres/}` for typed repos that need indexes/atomicity

**Files modified (regardless of choice):**

- `crates/t3claw_loop_support/src/capability_port.rs` — **do NOT introduce `CapabilityResultStore` trait here** (Reviewer 1 R2: loop_support is adapter glue, not persistence). Introduce in `t3claw_reborn_event_store` (or its own `t3claw_loop_capability_store` if needs to be reusable above adapter layer).
- `crates/t3claw_reborn_composition/src/product_live_adapters.rs` — switch backend choice via config; keep in-memory as `local_dev` fallback only.
- `crates/t3claw_reborn_composition/src/runtime/local_dev.rs` — keep in-memory for dev; wire production-readiness check to require non-in-memory in prod.
- `crates/t3claw_reborn/src/composition/production_readiness.rs` — flip `SubagentRestartReconciler` from stub to required impl present.
...
| File | Change |
|---|---|
| `crates/t3claw_loop_support/src/subagent_spawn_port.rs` | Lines ~183-188: remove the two `background_subagents_disabled()` returns. Add field `mode: SpawnSubagentMode` to `SpawnSubagentArgs` (line 152). Carry `mode` from `SpawnSubagentWireArgs` through `TryFrom`. `SubagentDefinition` (line 207): add `background_policy: BackgroundPolicy` (enum: `Denied`, `Allowed { max_depth: u32, max_concurrent: u32 }`). |
| `crates/t3claw_host_runtime/src/first_party_tools/spawn_subagent.rs` (`spawn_subagent_parameters_schema()` around line 48) | Add `mode: { type: "string", enum: ["blocking", "background"], default: "blocking" }` property. |
| `crates/t3claw_loop_support/src/subagent_spawn_port.rs` (spawn body, around line 590 depth check) | Add concurrent-background-children counter check against `BackgroundPolicy::Allowed.max_concurrent`. Counter lives on a new per-parent state slot (or computed from goal store on demand). Mailbox admission cap: refuse new background spawns when pending mailbox is full. |
| `crates/t3claw_loop_support/src/subagent_spawn_port.rs` (`finish_spawn` ~line 863) | For background mode, return `CapabilityOutcome::SpawnedChildRun { child_run_id, result_ref, safe_summary }` (existing variant, not `Completed`) instead of `AwaitDependentRun`. `AwaitDependentRunGateStage` is bypassed automatically. |
| `crates/t3claw_reborn/src/subagent/completion_observer.rs` | Existing Background branch (line 180-183) is already wired through `mark_child_deliveries`. Confirm it still works with durable result store; add idempotency-ledger write. |
| New config knob | `t3claw_reborn` config: `subagent.background_enabled: bool` (default false). Gate the wire-args acceptance. |
...
- `crates/t3claw_turns/src/run_profile/host.rs` — add new trait `LoopBackgroundChildPort { fn drain_settled(&self, parent: TurnRunId) -> Vec<SettledChild>; }`. Add as supertrait of `AgentLoopDriverHost`. Total supertrait count goes from 10+Send+Sync to 11+Send+Sync.
- `crates/t3claw_loop_support/src/background_child_port.rs` (new) — `SettledChild` type, default impl backed by the durable capability result store + settlement log.
- `crates/t3claw_agent_loop/src/executor/post_capability.rs` (or wherever WU-A landed `PostCapabilityStage`) — fill the no-op stub:
...
- **Boundary precedent explicit:** `crates/t3claw_architecture/tests/reborn_dependency_boundaries.rs:1996` documents `t3claw_event_projections` IS permitted to depend on `t3claw_turns` for `TurnLifecycleEvent`-derived read models (existing example: `PendingGateProjection`). Subagent lineage is the same pattern.
...
| `crates/t3claw_webui_v2_static/static/js/pages/chat/lib/useChatEvents.js:252` | Destructure new fields. Render runs grouped by `tree_root_run_id`, indented by `subagent_depth`. Background runs get distinct status badge (running, settled, failed). |
...
Verified against `docs/reborn/contracts/_contract-freeze-index.md`, `events.md`, `events-projections.md`, crate-local `CLAUDE.md` for `t3claw_agent_loop`, `t3claw_events`, `t3claw_event_projections`, `t3claw_turns`, `t3claw_loop_support`, `t3claw_product_adapters`, `t3claw_reborn_composition`, and `crates/t3claw_architecture/tests/reborn_dependency_boundaries.rs`.
...
| 2 | WebUI nesting via projection-time JOIN against `TurnRunRecord` (NOT new `RuntimeEvent` fields) | LOCKED IN | Schema-fields = contract change per freeze index §1. JOIN explicitly permitted: `reborn_dependency_boundaries.rs:1996` allows `t3claw_event_projections → t3claw_turns`. Precedent: `PendingGateProjection`. |
...
| 4 | `pending_capability_bytes: BTreeMap<CapabilityId, u64>` | LOCKED IN | agent_loop already depends on `t3claw_host_api`; `CapabilityId` already used in `state.rs:169`. Counter map = allowed per CLAUDE.md. Future per-cap policies (`BudgetFractionPolicy`) consume it. |
...
- **`t3claw_agent_loop` deps unchanged.** No imports from `t3claw_reborn`, host_runtime, dispatcher, network, secrets, DB backends. `CompactionForceStrategy` trait + `ByteCapStrategy` impl stay crate-private. New file `executor/post_capability.rs` follows "one decision axis per file" rule.
- **`t3claw_turns` gains `CompactionInitiator::CapabilityResultOverflow` variant only.** No new ports. Stays neutral host/runner protocol per turns CLAUDE.md.
- **`t3claw_loop_support` writer return widened to surface existing `byte_len`.** Adapter glue per crate CLAUDE.md; no new ports, no dispatcher bypass.
- **`t3claw_events`, `t3claw_event_projections`, `t3claw_product_adapters`:** zero schema changes from WU-A. WU-F adds 3 optional fields to `RunStatusProjection` + `ProductProjectionItem::RunStatus`, all `#[serde(skip_serializing_if = "Option::is_none")]`. JOIN lookup via `TurnSpawnTreeStateStore::get_run_record(&scope, run_id)` per `events-projections.md` §5 (reducer rules: deterministic, side-effect free, rebuildable).
- **`t3claw_reborn_composition`:** `run_status_projection_state` (`projection.rs:967`) + `product_items_for_live_update` (`live_progress.rs:147`) gain the JOIN. Stays facade-shaped per composition CLAUDE.md (no new substrate exposure).
...
**WU-A — minor contract tweak:** Adding `CompactionInitiator::CapabilityResultOverflow` variant in `t3claw_turns` is a contract addition. Treat as same-PR contract note in `docs/reborn/contracts/turns-agent-loop.md` (or `loop-exit.md`). Reuses existing `LoopProgressEvent::CompactionStarted` event — no new event variant.

**WU-D — CONTRACT CHANGE REQUEST, not implementation:** `SubagentDefinition` lives in `t3claw_loop_support` (`subagent_spawn_port.rs:207`) and is pub-re-exported from `t3claw_loop_support/src/lib.rs:91` — published surface. Adding `BackgroundPolicy` enum field is a contract change per `_contract-freeze-index.md` §1. **Must land a contract-change request PR BEFORE WU-D opens.** Likely targets: `docs/reborn/contracts/turns-agent-loop.md` (subagent-spawn section) or a new `subagent-lifecycle.md` packet entry.
...
| WU-A.6 | `t3claw_agent_loop` boundary rule absent from `boundary_rules()` | Reviewer 1 R3 | Add `BoundaryRule` entry for `t3claw_agent_loop` to `reborn_dependency_boundaries.rs` as part of WU-A |
| WU-A.7 | `CompactionInitiator` addition IS a contract change in `t3claw_turns` | Reviewer 2 B2 | Called out in "Outstanding contract-change requests" above |
| WU-C.1 | New `t3claw_reborn_persistence` crate has no boundary-test rule + contradicts `database.md` direction | Reviewer 1 V1 + Reviewer 4 G2 | Pivoted to `t3claw_reborn_event_store` (canonical owner per `events.md` §2); WU-B sub-spec decides per-store ScopedFilesystem vs typed-repo per `_contract-freeze-index.md` §2 storage model |
| WU-C.2 | `CapabilityResultStore` trait wrongly placed in `t3claw_loop_support` (adapter glue, not persistence) | Reviewer 1 R2 | Move trait to `t3claw_reborn_event_store` |
...
- **E2E test placement spec** (Reviewer 4 G5). WU-G test goes at `tests/e2e/scenarios/test_subagent_background.py` per `tests/e2e/CLAUDE.md`; `asyncio_mode = "auto"`; t3claw_server fixture must inject `T3CLAW_SUBAGENT_BACKGROUND_ENABLED=true` env.
...
4. Add `BoundaryRule` for `t3claw_agent_loop` in same PR (BLOCKER 6).
5. Same-PR contract note for `CompactionInitiator::CapabilityResultOverflow`.
6. Caller-level tests pass `cargo test -p t3claw_agent_loop`; boundary test passes `cargo test -p t3claw_architecture`.
