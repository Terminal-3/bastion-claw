...
This slice adds a Docker-backed compatibility lane for dynamic process execution through Reborn process lifecycle. It is intentionally separate from the manifest-derived `t3claw_scripts` lane.

## Shape

- `t3claw_process_sandbox` defines `ProcessSandboxBackend` as the backend-neutral execution contract.
...
- Physical host paths for `/workspace`, `/t3claw/state/tools`, and `/t3claw/state/cache` live in trusted `DockerProcessSandboxConfig`, not in plan JSON.
...
- applies `iptables` broker-only egress lockdown when `T3CLAW_EGRESS_LOCKDOWN=broker-only`.
