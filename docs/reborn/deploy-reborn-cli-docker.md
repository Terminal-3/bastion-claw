...
`Dockerfile.reborn` builds the standalone `t3claw-reborn` binary with the
WebUI v2 and Slack host-beta features enabled. The image defaults to:

```text
t3claw-reborn serve --host ${T3CLAW_REBORN_SERVE_HOST:-127.0.0.1} --port ${PORT:-3000}
```

Railway supplies `PORT`; set `T3CLAW_REBORN_SERVE_HOST=0.0.0.0` for
Railway/public deployments. Local Docker runs can keep the loopback default and
set `T3CLAW_REBORN_SERVE_PORT=3000`.

## Build

```bash
docker build -f Dockerfile.reborn -t t3claw-reborn:local .
```
...
config and point `T3CLAW_REBORN_DEFAULT_CONFIG` at it for the first start.

Google product-auth setup:

```bash
T3CLAW_REBORN_GOOGLE_CLIENT_ID=<google-client-id>
T3CLAW_REBORN_GOOGLE_CLIENT_SECRET=<google-client-secret>
T3CLAW_REBORN_GOOGLE_OAUTH_REDIRECT_URI=http://127.0.0.1:3000/api/reborn/product-auth/oauth/google/callback
```
...
```bash
T3CLAW_REBORN_WEBUI_GOOGLE_CLIENT_ID=<google-client-id>
T3CLAW_REBORN_WEBUI_GOOGLE_CLIENT_SECRET=<google-client-secret>
T3CLAW_REBORN_WEBUI_ALLOWED_EMAIL_DOMAINS=near.ai
T3CLAW_REBORN_WEBUI_BASE_URL=https://<public-host>
```
...
keep `T3CLAW_REBORN_SERVE_HOST=0.0.0.0`. The Reborn WebUI service serves
`/api/health` for Railway's healthcheck.

Leave Railway's Start Command empty for the Docker image. The image entrypoint
builds the `t3claw-reborn serve` arguments from `PORT` and
`T3CLAW_REBORN_SERVE_HOST`; Railway does not shell-expand `$VAR` placeholders
...
`t3claw-reborn serve` exits before binding the HTTP listener if the WebUI
...
Do not use `T3CLAW_REBORN_PROFILE=local-dev-yolo` for a public Railway
...
Set `T3CLAW_REBORN_HOME` to a mounted volume path if state should survive
redeploys. The image default is `/data/t3claw-reborn`; without a Railway
...
`/opt/t3claw/` and set `T3CLAW_REBORN_DEFAULT_CONFIG` to that path. On first
start, the entrypoint copies that file into `$T3CLAW_REBORN_HOME/config.toml`;
...
```bash
T3CLAW_REBORN_WEBUI_BASE_URL=https://<railway-domain>
T3CLAW_REBORN_WEBUI_GOOGLE_CLIENT_ID=<google-client-id>
T3CLAW_REBORN_WEBUI_GOOGLE_CLIENT_SECRET=<google-client-secret>
T3CLAW_REBORN_WEBUI_ALLOWED_EMAIL_DOMAINS=near.ai
T3CLAW_REBORN_WEBUI_TOKEN=<random-hex-32-bytes-or-longer>
...
`T3CLAW_REBORN_GOOGLE_OAUTH_REDIRECT_URI` only when the deployment should let
the agent connect a Google credential:

```bash
T3CLAW_REBORN_GOOGLE_OAUTH_REDIRECT_URI=https://<railway-domain>/api/reborn/product-auth/oauth/google/callback
```

## Slack

Slack routes are compiled into the image, but they are disabled by the default
config. To enable them, edit `$T3CLAW_REBORN_HOME/config.toml` or mount a
config file with:
...
T3CLAW_REBORN_SLACK_BOT_TOKEN=<slack-bot-token>
```
