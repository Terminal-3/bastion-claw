...
- **v2** auth is the three `WebuiAuthenticator` impls in
  `crates/t3claw_reborn_webui_ingress/` (`lib.rs`, `session.rs`,
  `oidc.rs`), selected by the host and enforced by the composition
  middleware in `crates/t3claw_reborn_composition/src/webui_serve.rs`.
...
- **Cookie session → one-time login ticket** (#8). v1 set an `HttpOnly`
  ...
  regression coverage are in `crates/t3claw_reborn_composition/CLAUDE.md`
...
- **v2 route-layer** (`crates/t3claw_reborn_webui_ingress/tests/`):
  - `auth_route_contract.rs` — env-bearer accept/reject, missing /
    ...
    reject, `t3claw_session` cookie rejection, sanitized-401-body
