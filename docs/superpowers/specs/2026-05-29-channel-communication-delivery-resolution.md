...
modality, which outbound target should T3Claw try? The answer is a
**candidate only**. `t3claw_outbound` still owns reply-target validation and
delivery-attempt records before any transport sends.

This design keeps three concepts separate:

- **Ingress identity:** how a message or trigger entered T3Claw.
...
| Area | Owner | Notes |
| --- | --- | --- |
| Inbound binding and replay | `t3claw_conversations` | Owns `InboundTurnRequest`, `ExternalConversationRef`, `external_event_id`, reply-target binding semantics. |
...
| Approval resolution | `t3claw_approvals` | Resolves durable pending approvals into scoped leases. It does not prompt users or route UI. |
| Outbound policy and validation | `t3claw_outbound` | Owns notification policy state, `ReplyTargetBindingRef` validation, delivery attempts, sanitized failure records. |
...
Current implementation caveat: `t3claw_outbound` is presently a policy and
...
Add `t3claw_outbound::OutboundResolutionEngine` inside the outbound policy
...
Run event
  FinalReplyReady | ProgressUpdate | ApprovalNeeded | AuthRequired | RunBlocked
        |
        v
t3claw_outbound::OutboundResolutionEngine
  entrypoints:
...
CommunicationDeliveryCandidate
  ...
  kind: final_reply | progress | approval_needed | auth_required | delivery_status
        |
        v
t3claw_outbound::OutboundPolicyService
  revalidates target
...
`t3claw_outbound` owns this engine because the existing Reborn event /
...
that crate. The resolution engine must not live in `t3claw_triggers`,
`t3claw_conversations`, or `t3claw_approvals`. Reborn composition wires the
...
`t3claw_outbound` validation before transport send. This context must not
...
requires `t3claw_outbound` revalidation before send.
...
`t3claw_outbound` owns this configuration because it owns notification policy,
...
CapabilityHost returns RequireApproval
  ...
  -> t3claw_outbound::OutboundResolutionEngine may notify approval_prompt_target
...
- Inbound user-message binding stays in product workflow and
  `t3claw_conversations`. The engine consumes canonical scope, actor, and
