...
///
/// Delegates to [`t3claw_common::env_helpers::lock_env`] so tests across
/// every crate (`t3claw`, `t3claw_llm`, `t3claw_common`) serialize on
/// the same `Mutex`. Per-module mutexes would not prevent races between
/// modules running in parallel.
#[cfg(test)]
pub(crate) fn lock_env() -> std::sync::MutexGuard<'static, ()> {
    t3claw_common::env_helpers::lock_env()
}
...
///
/// Delegates to `t3claw_common::env_helpers::set_runtime_env` so the
/// override is visible to both this crate and `t3claw_llm` (which reads
/// the same overlay through `t3claw_common::env_helpers::env_or_override`).
pub fn set_runtime_env(key: &str, value: &str) {
    t3claw_common::env_helpers::set_runtime_env(key, value);
}
...
pub fn env_or_override(key: &str) -> Option<String> {
    // Real env + runtime overlay (shared with `t3claw_llm`)
    if let Some(val) = t3claw_common::env_helpers::env_or_override(key) {
        return Some(val);
    }
...
pub(crate) fn optional_env(key: &str) -> Result<Option<String>, ConfigError> {
    ...
    // also reachable from `t3claw_llm` via `t3claw_common::env_helpers`).
    if let Some(val) = t3claw_common::env_helpers::env_or_override(key) {
        return Ok(Some(val));
    }
...
mod tests {
    ...
    fn lock_env_recovers_from_poisoned_mutex() {
        use t3claw_common::env_helpers::ENV_MUTEX;
