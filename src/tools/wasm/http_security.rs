...
pub(crate) async fn validate_and_resolve_http_target(
    ...
    if let Ok(ip) = host.parse::<IpAddr>() {
        return if is_private_ip(ip) {
            // Test escape hatch: when `T3CLAW_TEST_HTTP_REWRITE_MAP`
            ...
            if cfg!(any(test, debug_assertions))
                && std::env::var("T3CLAW_TEST_HTTP_REWRITE_MAP")
                ...
                && ip.is_loopback()
            {
                tracing::debug!(
                    %ip,
                    "Allowing loopback target — T3CLAW_TEST_HTTP_REWRITE_MAP is set"
                );
