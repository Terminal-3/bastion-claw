...
// Core OAuth callback infrastructure lives in the standalone `t3claw_oauth`
// crate so non-LLM OAuth flows (WASM tools, MCP, NEAR AI session login) don't
// have to depend on `t3claw_llm` for transport. Re-exported here so existing
// `crate::auth::oauth::...` call sites continue to compile.
pub use t3claw_oauth::{
...
mod tests {
    ...
    /// `t3claw tool auth google-calendar` code path end-to-end. Loads the
