...
**Reference:** `src/auth/oauth.rs` / `crates/t3claw_oauth/src/lib.rs` — `OAUTH_CALLBACK_PORT`, `bind_callback_listener()`
...
**Reference:** `src/auth/oauth.rs` / `crates/t3claw_oauth/src/lib.rs` — `html_escape()`
...
**Reference:** `src/auth/oauth.rs` / `crates/t3claw_oauth/src/lib.rs` — `wait_for_callback()`
