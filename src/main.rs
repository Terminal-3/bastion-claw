...
async fn async_main() -> anyhow::Result<()> {
    ...
    let runtime_overrides = t3claw::config::RuntimeConfigOverrides {
        ...
        && config.channels.wasm_channels_dir.exists()
    {
        ...
        let startup_active_channels: Vec<String> =
            if let Some(ref ext_mgr) = components.extension_manager {
                ...
                t3claw::extensions::naming::normalize_extension_names(
                    config.channels.configured_wasm_channels.clone(),
                )
            };
            ...
            if let Some(ref tx) = job_event_tx {
                ...
                tokio::spawn(async move {
                    while let Ok((_job_id, user_id, event)) = rx.recv().await {
                        ...
                        t3claw::channels::web::dispatch_status_event(
                            &gw_state.sse,
    ...
    // The recorder lives in `t3claw_llm` and must not depend on the
            ...
            Ok(paths) => {
                for path in paths {
                    match ws.read(&path).await {
                        Ok(doc) => entries.push(t3claw_llm::MemorySnapshotEntry {
                            path: doc.path,
                            content: doc.content,
                        }),
                        Err(e) => {
    ...
    let auth_manager = components.tools.secrets_store().cloned().map(|secrets| {
        Arc::new(t3claw::auth::extension::AuthManager::new(
            secrets,
    ...
    let deps = AgentDeps {
        ...
        transcription: config
            .transcription
            .create_provider()
            .map(|p| Arc::new(t3claw_llm::transcription::TranscriptionMiddleware::new(p))),
