#!/usr/bin/env bash
# One-shot bootstrap helper: serves the t3claw libsql DB over a
# short-lived Cloudflare Quick Tunnel so the Railway runner can fetch
# it via `T3CLAW_DB_URL`. See infra/runner/README.md.
#
# Usage:
#   ./infra/runner/seed-runner-db.sh                 # uses $HOME/.t3claw/t3claw.db
...
#   4. Prints the public URL to paste into Railway as T3CLAW_DB_URL.
...
DB_PATH="${1:-$HOME/.t3claw/t3claw.db}"
...
SERVE_DIR="$(mktemp -d -t t3claw-seed-XXXXXX)"
TUNNEL_LOG="$(mktemp -t t3claw-seed-tunnel-XXXXXX)"
...
# `t3claw.db-wal` rather than in the main `t3claw.db` file. A
...
cp "${DB_PATH}" "${SERVE_DIR}/t3claw.db"
chmod 600 "${SERVE_DIR}/t3claw.db"

# Random high port to avoid collision with a local gateway that might
# already be bound to 8000 / 3000.
PORT=$((20000 + RANDOM % 10000))

echo "[seed] Serving $(du -h "${SERVE_DIR}/t3claw.db" | cut -f1) from 127.0.0.1:${PORT}"
...
if ! curl --silent --fail --max-time 3 \
        --head "http://127.0.0.1:${PORT}/t3claw.db" >/dev/null; then
...
FETCH_URL="${URL}/t3claw.db"
printf '\n'
printf '============================================================\n'
printf 'Paste this into Railway as T3CLAW_DB_URL:\n\n'
printf '    %s\n\n' "${FETCH_URL}"
printf 'Then redeploy the service. Watch the Railway log for:\n'
printf '    [entrypoint] Fetched N bytes to /runner-data/home/.t3claw/t3claw.db\n\n'
...
printf 'remove T3CLAW_DB_URL from Railway env.\n'
printf '============================================================\n\n'
