...
use t3claw_filesystem::{CasExpectation, Entry, InMemoryBackend, RecordKind, RootFilesystem};
use t3claw_host_api::{AgentId, ProjectId, TenantId, UserId, VirtualPath};
...
use t3claw_turns::{AcceptedMessageRef, TurnRunId};
...
async fn durable_store_rejects_corrupt_persisted_state() {
    ...
    assert!(matches!(
        error,
        t3claw_reborn_openai_compat::OpenAiCompatRefError::CorruptMapping
    ));
}
...
fn expect_created(
    result: Result<
        ...
        t3claw_reborn_openai_compat::OpenAiCompatRefError,
    >,
) -> t3claw_reborn_openai_compat::OpenAiCompatResourceMapping {
