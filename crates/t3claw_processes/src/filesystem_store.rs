...
use t3claw_events::sanitize_error_kind;
...
use t3claw_host_api::{ProcessId, ResourceScope, ScopedPath, VirtualPath};
...
/// [`MountView`](t3claw_host_api::MountView) per-operation ACL and
/// resolves the `/processes` alias to a tenant-scoped
/// [`VirtualPath`](t3claw_host_api::VirtualPath) before any backend
            ...
            let record: ProcessRecord =
                serde_json::from_slice(&entry.entry.body).map_err(|error| {
                    FilesystemError::Backend {
                        ...
                        operation: t3claw_filesystem::FilesystemOperation::Query,
