...
use t3claw_host_api::{AgentId, CapabilityId, ProjectId, RuntimeKind, TenantId, ThreadId, UserId};
use t3claw_turns::{
...
async fn host_managed_model_port_routes_gateway_and_emits_model_milestones() {
    ...
    gateway.push_response(Ok(LoopModelResponse {
        chunks: vec![t3claw_turns::run_profile::ModelStreamChunk {
...
impl AgentLoopDriver for ReplyDriver {
    ...
    async fn run(
        &self,
        request: t3claw_turns::AgentLoopDriverRunRequest,
    ...
    async fn resume(
        &self,
        request: t3claw_turns::AgentLoopDriverResumeRequest,
        host: &(dyn AgentLoopDriverHost + Send + Sync),
    ) -> Result<LoopExit, AgentLoopDriverError> {
        self.run(
            t3claw_turns::AgentLoopDriverRunRequest {
...
impl AgentLoopDriver for CapabilityDriver {
    ...
    async fn run(
        &self,
        _request: t3claw_turns::AgentLoopDriverRunRequest,
        host: &(dyn AgentLoopDriverHost + Send + Sync),
    ) -> Result<LoopExit, AgentLoopDriverError> {
        let surface = host
            .visible_capabilities(VisibleCapabilityRequest)
...
async fn claimed_run_context() -> LoopRunContext {
    let scope = t3claw_turns::TurnScope::new(
...
fn driver_run_request(host: &RecordingAgentLoopHost) -> t3claw_turns::AgentLoopDriverRunRequest {
    t3claw_turns::AgentLoopDriverRunRequest {
        turn_id: host.context.turn_id,
        run_id: host.context.run_id,
