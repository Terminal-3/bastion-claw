...
async fn trusted_skill_allows_operational_paths_in_prompt_content() {
    let prompt = concat!(
        "Create a review worktree under /tmp/t3claw-review-123 and ",
    ...
    assert_eq!(snippets.len(), 1);
    assert_eq!(snippets[0].safe_summary, "the description");
    assert!(snippets[0].model_content.contains("/tmp/t3claw-review-123"));
    assert!(
        snippets[0]
            .model_content
...
async fn into_loop_snippet_conversion() {
    use t3claw_turns::run_profile::LoopContextSnippet;
