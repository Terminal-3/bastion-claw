...
- Product adapters use `TurnCoordinator` methods only. Trusted workers may import `t3claw_turns::runner` explicitly; do not add runner transition APIs to the public prelude.
...
- Implementations of those contracts live elsewhere: host adapters in
  `t3claw_loop_support`, driver-side integration in `t3claw_reborn`, and
  reusable loop mechanics in `t3claw_agent_loop`.
