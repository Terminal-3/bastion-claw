...
impl TurnScope {
    ...
    /// Convert into a [`t3claw_host_api::ResourceScope`] for filesystem
    ...
    pub fn to_resource_scope(&self) -> t3claw_host_api::ResourceScope {
        let mut scope = t3claw_host_api::ResourceScope::system();
        scope.tenant_id = self.tenant_id.clone();
        scope.user_id = self.explicit_owner_user_id().cloned().unwrap_or_else(|| {
            UserId::from_trusted(t3claw_host_api::SYSTEM_RESERVED_ID.to_string())
        });
