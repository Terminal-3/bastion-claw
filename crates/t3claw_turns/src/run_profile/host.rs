...
pub struct ProviderToolCallReplay {
    ...
    /// Opaque provider thought-signature metadata, not an T3Claw auth signature.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Provider-facing tool definition derived from a visible T3Claw capability.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ProviderToolDefinition {
    /// Canonical T3Claw capability id backing this provider tool.
    pub capability_id: CapabilityId,
...
pub trait LoopCheckpointPort: Send + Sync {
    ...
    /// `t3claw_loop_support` and wrap the host's `CheckpointStateStore`.
    ///
