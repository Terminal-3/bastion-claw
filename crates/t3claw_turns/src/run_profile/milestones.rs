...
use t3claw_host_api::{CapabilityId, ExtensionId, RuntimeKind};
...
pub enum LoopHostMilestoneKind {
    ...
    /// `t3claw_hooks::HookId::to_hex`). The hook crate cannot be imported
    /// here without breaking the architecture-enforced dependency direction
    /// (`t3claw_turns -> t3claw_hooks` is forbidden), so the hook id is
    ...
    HookDispatched {
        ...
        /// Carried into [`t3claw_events::RuntimeEvent::provider`] so
...
/// `t3claw_hooks` is a process-wide shared object (`Arc<HookDispatcher>`)
...
/// `t3claw_reborn` wraps the real [`LoopHostMilestoneSink`] in an adapter
