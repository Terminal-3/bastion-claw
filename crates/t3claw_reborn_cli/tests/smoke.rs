...
const INVALID_PROFILE_MESSAGE: &str = "T3CLAW_REBORN_PROFILE must be one of";

fn reborn_bin() -> &'static str {
    env!("CARGO_BIN_EXE_t3claw-reborn")
}
...
fn isolated_no_llm_command(workspace: &Path, reborn_home: &Path) -> Command {
    ...
    command
        ...
        .env("OLLAMA_BASE_URL", "")
        .env("T3CLAW_REBORN_HOME", reborn_home);
    command
}
...
fn fake_reborn_bin(bin_dir: &Path) {
    ...
    let bin = bin_dir.join("t3claw-reborn");
    std::fs::write(
        &bin,
        "#!/bin/sh\nprintf 'home=%s\\n' \"$T3CLAW_REBORN_HOME\"\nprintf 'args=%s\\n' \"$*\"\n",
    )
...
fn dockerfile_reborn_builds_with_postgres_feature() {
    ...
    assert!(
        !dockerfile.contains("T3CLAW_REBORN_HOME=/data/t3claw-reborn"),
...
fn docker_reborn_config_defaults_to_local_dev() {
    ...
    let parsed = t3claw_reborn_config::RebornConfigFile::parse_text(
        &config,
...
fn docker_reborn_production_config_uses_postgres_storage() {
    ...
    assert_eq!(
        storage.backend,
        Some(t3claw_reborn_config::StorageBackend::Postgres)
    );
    assert_eq!(
        storage.url_env.as_deref(),
        Some("T3CLAW_REBORN_POSTGRES_URL")
    );
    assert_eq!(
        storage.secret_master_key_env.as_deref(),
        Some("T3CLAW_REBORN_SECRET_MASTER_KEY")
    );
...
fn docker_reborn_entrypoint_uses_railway_volume_mount_for_home() {
    ...
    let reborn_home = volume.join("t3claw-reborn");
...
fn docker_reborn_entrypoint_rejects_ephemeral_railway_without_volume() {
    ...
    assert!(
        stderr.contains("T3CLAW_REBORN_ALLOW_EPHEMERAL_RAILWAY=true"),
        "stderr: {stderr}"
    );
}
...
fn docker_reborn_entrypoint_allows_railway_production_without_volume() {
    ...
    let output = Command::new("/bin/sh")
        ...
        .env("HOME", temp.path().join("home"))
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .env("T3CLAW_REBORN_PROFILE", "production")
        .env("RAILWAY_ENVIRONMENT", "production")
        .output()
        .expect("entrypoint should run");
...
fn docker_reborn_entrypoint_rejects_stale_local_dev_config_for_production() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("T3CLAW_REBORN_PROFILE=production requires"),
        "stderr: {stderr}"
    );
...
fn help_mentions_reborn_commands() {
    let output = Command::new(reborn_bin())
        .arg("--help")
        .output()
        .expect("t3claw-reborn --help should run");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Standalone T3Claw Reborn runtime"),
        "stdout: {stdout}"
    );
...
fn extension_search_does_not_seed_reborn_config() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("HOME", temp.path().join("home"))
        .output()
        .expect("t3claw-reborn extension search should run");
...
fn profile_list_shows_supported_profiles_without_reborn_home() {
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .output()
        .expect("t3claw-reborn profile list should run");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("T3Claw Reborn profiles"),
        "stdout: {stdout}"
    );
    ...
    assert!(stdout.contains("T3CLAW_REBORN_PROFILE"), "stdout: {stdout}");
}
...
fn profile_list_json_is_stable_and_does_not_resolve_reborn_home() {
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .output()
        .expect("t3claw-reborn profile list --json should run");
    ...
    assert_eq!(json["selector"], "T3CLAW_REBORN_PROFILE");
...
fn skills_list_reports_reborn_skill_data() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .env("T3CLAW_BASE_DIR", &v1_home)
        .output()
        .expect("t3claw-reborn skills list should run");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("T3Claw Reborn skills"), "stdout: {stdout}");
...
fn skills_list_verbose_reports_reborn_skill_details() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .output()
        .expect("t3claw-reborn skills list --verbose should run");
...
fn skills_list_json_reports_reborn_skill_data() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .output()
        .expect("t3claw-reborn skills list --json should run");
...
fn skills_list_rejects_unsupported_profiles() {
    for profile in ["production", "migration-dry-run"] {
        ...
        let output = Command::new(reborn_bin())
            ...
            .arg("list")
            .env_clear()
            .env("T3CLAW_REBORN_HOME", temp.path().join("reborn-home"))
            .env("T3CLAW_REBORN_PROFILE", profile)
            .output()
            .expect("t3claw-reborn skills list should run");
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(
            stderr.contains("t3claw-reborn skills currently supports profile=local-dev"),
            "stderr: {stderr}"
        );
        assert!(
...
fn logs_reports_unwired_surface_without_reborn_home() {
    assert_empty_not_wired_surface(&["logs"], "T3Claw Reborn logs", "logs", "entries");
}
...
fn models_list_reports_reborn_provider_catalog_without_v1_state() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("HOME", temp.path())
        .output()
        .expect("t3claw-reborn models list should run");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("T3Claw Reborn LLM providers"),
        "stdout: {stdout}"
    );
...
fn models_status_json_reports_routes_not_configured_without_v1_state() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("HOME", temp.path())
        .output()
        .expect("t3claw-reborn models status --json should run");
...
fn models_set_provider_writes_reborn_config_without_v1_state() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .output()
        .expect("t3claw-reborn models set-provider should run");
    ...
    assert!(
        !temp.path().join(".t3claw").join(".env").exists(),
...
"#,
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .output()
        .expect("t3claw-reborn models set should run");
...
fn models_list_no_default_features_does_not_resolve_reborn_home() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("T3Claw Reborn model slots"),
        "stdout: {stdout}"
    );
    assert!(stdout.contains("v1_state: not-used"), "stdout: {stdout}");
}
...
fn models_status_no_default_features_does_not_resolve_reborn_home() {
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .output()
        .expect("t3claw-reborn models status should run");
...
fn models_write_commands_report_root_llm_provider_required_without_default_features() {
    for args in [
        ...
        let output = Command::new(reborn_bin())
            ...
            .env_clear()
            .output()
            .expect("t3claw-reborn models write command should run");
...
fn assert_empty_not_wired_surface(
    ...
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .output()
        .expect("t3claw-reborn command should run");
    ...
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .output()
        .expect("t3claw-reborn JSON command should run");
    assert!(
        output.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&output.stderr)
...
fn assert_verbose_detail(args: &[&str], expected_detail: &str) {
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .output()
        .expect("t3claw-reborn verbose command should run");
...
fn assert_json_verbose_detail(
    ...
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .output()
        .expect("t3claw-reborn JSON verbose command should run");
...
fn config_path_reports_reborn_home_without_touching_v1_state() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .env("T3CLAW_REBORN_PROFILE", "production")
        .env("T3CLAW_BASE_DIR", &v1_base_dir)
        .output()
        .expect("t3claw-reborn config path should run");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("T3Claw Reborn config path"),
        "stdout: {stdout}"
    );
    assert!(
    ...
    assert!(
        stdout.contains("home_source: T3CLAW_REBORN_HOME"),
        "stdout: {stdout}"
    );
...
fn config_path_reports_default_reborn_home_without_creating_directories() {
    let temp = tempfile::tempdir().expect("tempdir");
    let reborn_home = temp.path().join(".t3claw").join("reborn");

    let output = Command::new(reborn_bin())
        .arg("config")
        .arg("path")
        .env_remove("T3CLAW_REBORN_HOME")
        .env("HOME", temp.path())
        .env_remove("USERPROFILE")
        .env_remove("T3CLAW_REBORN_PROFILE")
        .output()
        .expect("t3claw-reborn config path should run");
...
fn completion_generates_zsh_script_without_reborn_home() {
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .output()
        .expect("t3claw-reborn completion should run");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("#compdef t3claw-reborn"),
        "stdout: {stdout}"
    );
    assert!(stdout.contains("_t3claw-reborn"), "stdout: {stdout}");
    assert!(
...
fn serve_help_mentions_host_and_port() {
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .output()
        .expect("t3claw-reborn serve --help should run");
...
fn serve_fails_closed_when_env_bearer_token_var_is_unset() {
    ...
    // (defaulting to T3CLAW_REBORN_WEBUI_TOKEN). When that var is
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", temp.path().join("reborn-home"))
        .env_remove("T3CLAW_REBORN_PROFILE")
        .env_remove("T3CLAW_REBORN_WEBUI_TOKEN")
        .env_remove("T3CLAW_REBORN_WEBUI_USER_ID")
        .output()
        .expect("t3claw-reborn serve should run");
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("T3CLAW_REBORN_WEBUI_TOKEN must be set"),
...
fn serve_fails_closed_when_env_user_id_var_is_unset() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", temp.path().join("reborn-home"))
        .env_remove("T3CLAW_REBORN_PROFILE")
        .env("T3CLAW_REBORN_WEBUI_TOKEN", "any-non-empty-token")
        .env_remove("T3CLAW_REBORN_WEBUI_USER_ID")
        .output()
        .expect("t3claw-reborn serve should run");
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("T3CLAW_REBORN_WEBUI_USER_ID must be set"),
...
fn serve_with_env_auth_seeds_reborn_config_before_binding() {
    ...
    let mut child = Command::new(reborn_bin())
        ...
        .env("HOME", &home)
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .env("T3CLAW_REBORN_WEBUI_TOKEN", "test-token")
        .env("T3CLAW_REBORN_WEBUI_USER_ID", "test-user")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("t3claw-reborn serve should start");
    ...
    loop {
        ...
        match stderr_rx.recv_timeout(std::time::Duration::from_millis(100)) {
            Ok(Ok(line)) => {
                ...
                stderr_text.push('\n');
                if stderr_text.contains("t3claw-reborn: WebChat v2 listener") {
                    break;
                }
            }
    ...
    assert!(
        config.contains("api_version = \"t3claw.runtime/v1\""),
...
// `t3claw_reborn_webui_ingress::tests` rather than from the binary
...
fn run_reports_runtime_readiness_snapshot_without_touching_v1_state() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env_remove("T3CLAW_REBORN_PROFILE")
        .output()
        .expect("t3claw-reborn run should run");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("T3Claw Reborn runtime readiness snapshot"),
        "stdout: {stdout}"
    );
    assert!(
...
fn doctor_uses_reborn_home_override_without_touching_v1_state() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env_remove("T3CLAW_REBORN_PROFILE")
        .output()
        .expect("t3claw-reborn doctor should run");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("T3Claw Reborn doctor"), "stdout: {stdout}");
    assert!(
        stdout.contains(reborn_home.to_str().expect("utf8 path")),
        "stdout: {stdout}"
    );
...
fn repl_help_mentions_composed_runtime() {
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .output()
        .expect("t3claw-reborn repl --help should run");
...
fn repl_exit_command_seeds_reborn_config() {
    ...
    let mut child = Command::new(reborn_bin())
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("t3claw-reborn repl should start");
    child
        .stdin
        .as_mut()
    ...
    let output = child
        .wait_with_output()
        .expect("t3claw-reborn repl should finish");
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("t3claw-reborn: runtime started"),
        "stderr: {stderr}"
    );
    assert!(
        !home_dir.join(".t3claw").exists(),
...
fn run_rejects_codex_backend_when_auth_file_is_missing() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .output()
        .expect("t3claw-reborn run should not crash");
    assert!(
        !output.status.success(),
...
fn run_help_command_prints_repl_commands_and_exits_on_quit() {
    ...
    let mut child = Command::new(reborn_bin())
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("t3claw-reborn run should start");
    child
        .stdin
        .as_mut()
        .expect("stdin should be piped")
    ...
    let output = child
        .wait_with_output()
        .expect("t3claw-reborn run should finish");
...
fn run_message_exits_nonzero_when_runtime_does_not_produce_reply() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("HOME", temp.path().join("home"))
        .output()
        .expect("t3claw-reborn run --message should run");
...
fn run_reports_explicit_profile() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .arg("--dry-run")
        .env("T3CLAW_REBORN_HOME", temp.path().join("reborn-home"))
        .env("T3CLAW_REBORN_PROFILE", "migration-dry-run")
        .output()
        .expect("t3claw-reborn run should run");
...
fn doctor_rejects_invalid_profile() {
    ...
    let output = Command::new(reborn_bin())
        .arg("doctor")
        .env("T3CLAW_REBORN_HOME", temp.path().join("reborn-home"))
        .env("T3CLAW_REBORN_PROFILE", "prod")
        .output()
        .expect("t3claw-reborn doctor should run");
...
fn run_rejects_reborn_home_equal_to_explicit_v1_base_dir() {
    ...
    let output = Command::new(reborn_bin())
        .arg("run")
        .env("T3CLAW_REBORN_HOME", &v1_root)
        .env("T3CLAW_BASE_DIR", &v1_root)
        .output()
        .expect("t3claw-reborn run should run");
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("T3CLAW_REBORN_HOME must not point at the v1 T3Claw state root"),
        "stderr: {stderr}"
    );
}
...
fn doctor_rejects_empty_reborn_home_override() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("T3CLAW_REBORN_HOME must not be empty"),
        "stderr: {stderr}"
    );
}
...
fn doctor_rejects_relative_reborn_home_override() {
    let output = Command::new(reborn_bin())
        .arg("doctor")
        .env_clear()
        .env("T3CLAW_REBORN_HOME", "relative/reborn")
        .output()
        .expect("t3claw-reborn doctor should run");
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("T3CLAW_REBORN_HOME must be an absolute path"),
        "stderr: {stderr}"
    );
}
...
fn config_init_writes_both_files() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .output()
        .expect("t3claw-reborn config init should run");
    assert!(
        output.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&output.stderr)
    ...
    assert!(
        config_text.contains("api_version = \"t3claw.runtime/v1\""),
...
fn config_init_with_force_overwrites() {
    ...
    assert!(config_text.contains("api_version = \"t3claw.runtime/v1\""));
...
fn onboard_bootstraps_reborn_home_without_touching_v1_state() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_BASE_DIR", &v1_home)
        .output()
        .expect("t3claw-reborn onboard should run");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("T3Claw Reborn onboarding"),
        "stdout: {stdout}"
    );
    assert!(stdout.contains("v1_state: not-used"), "stdout: {stdout}");
    assert!(
    ...
    assert_eq!(marker["schema_version"], "t3claw.reborn.onboarding/v1");
...
fn onboard_dry_run_is_read_only() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .output()
        .expect("t3claw-reborn onboard --dry-run should run");
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("T3Claw Reborn onboarding dry run"),
        "stdout: {stdout}"
    );
    assert!(
...
fn onboard_import_history_records_pending_step() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .output()
        .expect("t3claw-reborn onboard --import-history should run");
...
fn onboard_with_force_overwrites_existing_files_and_marker() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .output()
        .expect("t3claw-reborn onboard --force should run");
...
fn run_confirm_host_access_requires_home_or_userprofile() {
    ...
    let output = Command::new(reborn_bin())
        ...
        .env_clear()
        .env("T3CLAW_REBORN_HOME", &reborn_home)
        .env("T3CLAW_REBORN_PROFILE", "local-dev-yolo")
        .output()
        .expect("t3claw-reborn run should not crash");
...
fn repl_confirm_host_access_flag_gates_local_dev_yolo() {
    ...
    let missing = local_yolo_command(&temp, &["repl"])
        .stdin(Stdio::null())
        .output()
        .expect("t3claw-reborn repl should not crash");
    assert!(!missing.status.success(), "missing confirmation must fail");
    let missing_stderr = String::from_utf8_lossy(&missing.stderr);
    assert!(
        missing_stderr.contains("requires explicit disclosure acknowledgement"),
...
fn serve_confirm_host_access_flag_gates_local_dev_yolo() {
    ...
    let missing = local_yolo_command(&temp, &["serve"])
        .output()
        .expect("t3claw-reborn serve should not crash");
    assert!(!missing.status.success(), "missing confirmation must fail");
    let missing_stderr = String::from_utf8_lossy(&missing.stderr);
    assert!(
        missing_stderr.contains("requires explicit disclosure acknowledgement"),
    ...
    assert!(
        confirmed_stderr.contains("T3CLAW_REBORN_WEBUI_TOKEN"),
