...
use t3claw_reborn_composition::host_api::{AgentId, TenantId, UserId};
#[cfg(feature = "webui-v2-beta")]
...
pub(crate) fn init_tracing() {
    ...
    let filter = EnvFilter::try_from_env("T3CLAW_REBORN_LOG").unwrap_or_else(|_| {
        EnvFilter::new("info,t3claw_reborn=info,t3claw_reborn_composition=info")
    });
    let _ = fmt()
...
fn print_runtime_banner(config: &RebornBootConfig) {
    eprintln!("t3claw-reborn: runtime started");
...
async fn send_once(
    ...
    conversation: &t3claw_reborn_composition::ConversationId,
    text: &str,
...
fn print_reply(reply: &t3claw_reborn_composition::AssistantReply) {
...
pub(crate) fn build_runtime_input_with_options(
    ...
    {
        match t3claw_reborn_composition::resolve_reborn_runtime_llm(
            config,
            runtime_services.config_file.as_ref(),
        )? {
            Some(llm) => {
...
pub(crate) struct RuntimeServicesInput {
    ...
    config_file: Option<t3claw_reborn_config::RebornConfigFile>,
}
...
pub(crate) fn build_services_input_with_options(
    ...
    // (operator may not have run `t3claw-reborn config init` yet);
    ...
    let mut services_input = match profile {
        RebornProfile::LocalDev | RebornProfile::LocalDevYolo => {
            ...
            .with_context(|| {
                format!(
                    "t3claw-reborn run currently supports profile=local-dev or profile=local-dev-yolo; \
                     got profile={profile}."
                )
            })?
...
fn build_production_services_input(
    profile: RebornProfile,
    _owner_id: &str,
    _config_file: Option<&t3claw_reborn_config::RebornConfigFile>,
) -> anyhow::Result<RebornBuildInput> {
    anyhow::bail!(
        "profile={profile} requires a binary built with the `postgres` feature for production \
         storage; the default PostgreSQL URL env var is T3CLAW_REBORN_POSTGRES_URL"
    )
}
...
fn resolve_google_oauth_config(
    ...
    let reborn_client_id = lookup("T3CLAW_REBORN_GOOGLE_CLIENT_ID");
    let reborn_redirect_uri = lookup("T3CLAW_REBORN_GOOGLE_OAUTH_REDIRECT_URI");
    let reborn_client_secret = lookup("T3CLAW_REBORN_GOOGLE_CLIENT_SECRET");
    let reborn_hosted_domain_hint = lookup("T3CLAW_REBORN_GOOGLE_HOSTED_DOMAIN_HINT");
    ...
    let client_id = reborn_client_id.or(legacy_client_id).ok_or_else(|| {
        anyhow::anyhow!(
            "T3CLAW_REBORN_GOOGLE_CLIENT_ID or GOOGLE_CLIENT_ID is required for Google OAuth setup"
        )
    })?;
    let redirect_uri = reborn_redirect_uri.or(legacy_redirect_uri).ok_or_else(|| {
        anyhow::anyhow!(
            "T3CLAW_REBORN_GOOGLE_OAUTH_REDIRECT_URI or GOOGLE_OAUTH_REDIRECT_URI is required for Google OAuth setup"
        )
    })?;
    ...
    if client_secret.is_none() {
        tracing::warn!(
            target = "t3claw::reborn::cli::google_oauth",
...
///
/// **Not** for operator-control knobs like `T3CLAW_TRIGGER_POLLER_*` —
...
pub(crate) fn read_config_file(
    config: &RebornBootConfig,
) -> anyhow::Result<Option<t3claw_reborn_config::RebornConfigFile>> {
    use t3claw_reborn_config::RebornConfigFile;
...
mod tests {
    ...
    use t3claw_reborn_composition::RebornCompositionProfile;
    #[cfg(feature = "webui-v2-beta")]
    use t3claw_reborn_composition::{LocalTriggerAccessRole, LocalTriggerAccessSource};
    use t3claw_reborn_config::RebornBootConfig;
    ...
    fn clear_trigger_poller_env() -> (EnvGuard, EnvGuard) {
        (
            EnvGuard::clear("T3CLAW_TRIGGER_POLLER_ENABLED"),
            EnvGuard::clear("T3CLAW_TRIGGER_POLLER_INTERVAL_SECS"),
        )
    }
...
"#,
    ...
    fn build_runtime_input_production_requires_storage_section() {
        let _lock = lock_trigger_env();
        let (_enabled, _interval) = clear_trigger_poller_env();
        let _postgres_url = EnvGuard::clear("T3CLAW_REBORN_POSTGRES_URL");
        let _secret_master_key = EnvGuard::clear("T3CLAW_REBORN_SECRET_MASTER_KEY");
...
"#,
        ...
        assert!(
            err.to_string().contains("T3CLAW_REBORN_POSTGRES_URL"),
...
"#,
    ...
    fn build_runtime_input_production_rejects_whitespace_only_postgres_url() {
        let _lock = lock_trigger_env();
        let (_enabled, _interval) = clear_trigger_poller_env();
        let _postgres_url = EnvGuard::set("T3CLAW_REBORN_POSTGRES_URL", "   ");
        let _secret_master_key =
            EnvGuard::set("T3CLAW_REBORN_SECRET_MASTER_KEY", "test-master-key");
        let (_temp, config) = boot_config_with_config_toml(
            "production",
            r#"
[storage]
...
"#,
    ...
    fn build_runtime_input_production_preserves_whitespace_secret_master_key() {
        ...
        let _postgres_url = EnvGuard::set(
            "T3CLAW_REBORN_POSTGRES_URL",
            "postgres://localhost/t3claw_reborn_cli_test",
        );
        let _secret_master_key = EnvGuard::set("T3CLAW_REBORN_SECRET_MASTER_KEY", "   ");
        let (_temp, config) = boot_config_with_config_toml(
            "production",
            r#"
[storage]
...
"#,
    ...
    fn build_runtime_input_production_uses_custom_url_env_name() {
        let _lock = lock_trigger_env();
        let (_enabled, _interval) = clear_trigger_poller_env();
        let _default_postgres_url = EnvGuard::clear("T3CLAW_REBORN_POSTGRES_URL");
        let _custom_postgres_url = EnvGuard::set(
            "T3CLAW_REBORN_CUSTOM_POSTGRES_URL",
            "postgres://localhost/t3claw_reborn_cli_test",
        );
        let _secret_master_key =
            EnvGuard::set("T3CLAW_REBORN_SECRET_MASTER_KEY", "test-master-key");
...
[storage]
backend = "postgres"
url_env = "T3CLAW_REBORN_CUSTOM_POSTGRES_URL"
secret_master_key_env = "T3CLAW_REBORN_SECRET_MASTER_KEY"
...
"#,
        ...
        assert!(
            rendered.contains("T3CLAW_REBORN_SECRET_MASTER_KEY"),
...
"#,
    ...
    fn build_runtime_input_production_constructs_postgres_services_input() {
        ...
        let secret_master_key = EnvGuard::set("T3CLAW_REBORN_SECRET_MASTER_KEY", "test-master-key");
...
"#,
        ...
        let tenant_id = t3claw_reborn_composition::host_api::TenantId::new("run-trigger-tenant")
            .expect("tenant id");
        let user_id =
            t3claw_reborn_composition::host_api::UserId::new("run-trigger-user").expect("user id");
        let stale_user_id = t3claw_reborn_composition::host_api::UserId::new("run-trigger-stale")
            .expect("stale user id");
        let agent_id = t3claw_reborn_composition::host_api::AgentId::new("run-trigger-agent")
            .expect("agent id");
        let project_id = t3claw_reborn_composition::host_api::ProjectId::new("run-trigger-project")
            .expect("project id");
        let user_store_path = config
            .home()
            .path()
            ...
            .join("reborn-local-dev.db");
        let access_store =
            t3claw_reborn_composition::open_local_trigger_access_store(&user_store_path)
                .await
                .expect("open local trigger access store");
        access_store
            .seed_local_access(t3claw_reborn_composition::LocalTriggerAccessSeed {
                tenant_id: &tenant_id,
        ...
        let allowed = checker
            .check_trigger_fire_access(t3claw_reborn_composition::TriggerFireAccessCheck {
                tenant_id: tenant_id.clone(),
                creator_user_id: user_id,
                agent_id: Some(agent_id.clone()),
                project_id: None,
                trigger_id: t3claw_reborn_composition::TriggerId::new(),
        ...
        assert_eq!(
            allowed,
            t3claw_reborn_composition::TriggerFireAccessDecision::Allowed
        );

        let project_scoped_decision = checker
            .check_trigger_fire_access(t3claw_reborn_composition::TriggerFireAccessCheck {
                tenant_id: tenant_id.clone(),
                creator_user_id: t3claw_reborn_composition::host_api::UserId::new(
                    "run-trigger-user",
                )
                .expect("user id"),
                agent_id: Some(agent_id.clone()),
        ...
        assert_eq!(
            ...
            t3claw_reborn_composition::TriggerFireAccessDecision::Denied {
...
"#,
    ...
    fn build_runtime_input_env_enables_trigger_poller_with_no_config_section() {
        ...
        let _lock = lock_trigger_env();
        let _enabled = EnvGuard::set("T3CLAW_TRIGGER_POLLER_ENABLED", "true");
        let _interval = EnvGuard::clear("T3CLAW_TRIGGER_POLLER_INTERVAL_SECS");
        ...
        assert!(
            input.trigger_poller.enabled,
            "T3CLAW_TRIGGER_POLLER_ENABLED=true must reach input.trigger_poller.enabled through build_runtime_input"
        );
    }
    ...
    fn build_runtime_input_env_interval_overrides_config_interval() {
        ...
        let _lock = lock_trigger_env();
        let _enabled = EnvGuard::clear("T3CLAW_TRIGGER_POLLER_ENABLED");
        let _interval = EnvGuard::set("T3CLAW_TRIGGER_POLLER_INTERVAL_SECS", "45");
...
"#,
        ...
        assert_eq!(
            ...
            "env T3CLAW_TRIGGER_POLLER_INTERVAL_SECS=45 must override config poll_interval_secs=15 through build_runtime_input"
        );
    }
    ...
    fn build_runtime_input_rejects_invalid_trigger_poller_enabled_env() {
        ...
        let _lock = lock_trigger_env();
        let _enabled = EnvGuard::set("T3CLAW_TRIGGER_POLLER_ENABLED", "yes");
        let _interval = EnvGuard::clear("T3CLAW_TRIGGER_POLLER_INTERVAL_SECS");
        ...
        let err = match build_runtime_input(&config, RuntimeInputCaller::Run) {
            Ok(_) => panic!(
                "invalid T3CLAW_TRIGGER_POLLER_ENABLED must propagate as Err through build_runtime_input"
            ),
            Err(e) => e,
        };
        assert!(
            err.to_string().contains("T3CLAW_TRIGGER_POLLER_ENABLED"),
    ...
    fn resolve_google_oauth_config_errors_when_client_id_missing() {
        let vars = HashMap::from([(
            "T3CLAW_REBORN_GOOGLE_OAUTH_REDIRECT_URI",
    ...
    fn resolve_google_oauth_config_prefers_reborn_prefixed_vars() {
        let vars = HashMap::from([
            (
                "T3CLAW_REBORN_GOOGLE_CLIENT_ID",
                "reborn-client.apps.googleusercontent.com",
            ),
            ("T3CLAW_REBORN_GOOGLE_CLIENT_SECRET", "reborn-client-secret"),
            (
                "T3CLAW_REBORN_GOOGLE_OAUTH_REDIRECT_URI",
                "http://127.0.0.1:3000/api/reborn/product-auth/oauth/google/callback",
            ),
            (
                "T3CLAW_REBORN_GOOGLE_HOSTED_DOMAIN_HINT",
                "reborn.example.com",
            ),
            (
                "GOOGLE_CLIENT_ID",
