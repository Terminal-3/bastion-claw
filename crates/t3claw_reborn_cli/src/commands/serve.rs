...
use t3claw_reborn_composition::build_openai_compat_route_mount;
#[cfg(not(feature = "slack-v2-host-beta"))]
use t3claw_reborn_composition::build_webui_services;
use t3claw_reborn_composition::host_api::{AgentId, ProjectId, TenantId, UserId};
...
use t3claw_reborn_config::{IdentitySection, seed_default_config_file_if_missing};
...
const DEFAULT_ENV_TOKEN_VAR: &str = "T3CLAW_REBORN_WEBUI_TOKEN";
const DEFAULT_ENV_USER_ID_VAR: &str = "T3CLAW_REBORN_WEBUI_USER_ID";
...
impl ServeCommand {
    pub(crate) fn execute(self, context: RebornCliContext) -> anyhow::Result<()> {
        ...
        let config_file =
            t3claw_reborn_config::RebornConfigFile::load(&boot_config.home().config_file_path())
...
fn reject_non_loopback_privileged_local_runtime(
    ...
    anyhow::bail!(
        "`t3claw-reborn serve` refuses non-loopback listener {host} because the selected \
...
fn with_notion_dcr_oauth_backend(
    ...
    services
        .with_notion_dcr_oauth_backend(callback_origin, "T3Claw")
...
fn print_serve_banner(
    ...
    eprintln!("t3claw-reborn: WebChat v2 listener");
    eprintln!("  binary    : t3claw-reborn");
...
mod tests {
    ...
    async fn trigger_poller_bootstrap_seeds_local_access_checker() {
        ...
        .with_trigger_poller_settings(t3claw_reborn_composition::TriggerPollerSettings::enabled());
    ...
    async fn webui_serve_wires_notion_dcr_into_runtime_services() {
        ...
        let services = t3claw_reborn_composition::build_reborn_services(services_input)
            .await
