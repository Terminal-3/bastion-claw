...
//! ([`RebornIdentityResolver`](t3claw_reborn_composition::RebornIdentityResolver),
...
impl UserDirectory for WebuiUserDirectory {
    async fn resolve(
        ...
        let Some(admitted_email) = self.admitted_email(profile) else {
            ...
            // to T3CLAW_REBORN_WEBUI_ALLOWED_EMAIL_DOMAINS. Logs only the
            ...
            let candidate_domains: std::collections::BTreeSet<String> = profile
                ...
                .collect();
            tracing::warn!(
                target: "t3claw::reborn::webui_ingress::auth",
...
mod tests {
    ...
    fn shared_resolver() -> Arc<dyn RebornIdentityResolver> {
        ...
        t3claw_reborn_composition::open_reborn_identity_resolver(
    ...
    async fn sso_user_directory_seeds_local_trigger_access_for_admitted_user() {
        ...
        let access_store = t3claw_reborn_composition::open_local_trigger_access_store(&path)
            .await
