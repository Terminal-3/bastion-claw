...
use clap::Args;
use t3claw_reborn_config::RebornHome;
...
impl OnboardCommand {
    pub(crate) fn execute(self, context: RebornCliContext) -> anyhow::Result<()> {
        ...
        println!("T3Claw Reborn onboarding");
        ...
        println!("- run `t3claw-reborn models set-provider <provider> --model <model>` as needed");
...
fn print_dry_run(home: &RebornHome, marker_path: &Path, force: bool, import_history: bool) {
    println!("T3Claw Reborn onboarding dry run");
    println!("reborn_home: {}", home.path().display());
    println!("home_source: {}", home.source_label());
