...
use t3claw_reborn_config::{RebornBootConfig, RebornProfile};
...
fn build_skill_list_config(config: &RebornBootConfig) -> anyhow::Result<SkillListConfig> {
    ...
    match profile {
        ...
        RebornProfile::Production | RebornProfile::MigrationDryRun => {
            anyhow::bail!(
                "t3claw-reborn skills currently supports profile=local-dev or profile=local-dev-yolo; got profile={profile}"
            );
        }
    }
    Ok(SkillListConfig {
