...
use clap::Args;
use t3claw_reborn_config::{REBORN_CONFIG_API_VERSION, RebornHome};
...
///
/// Mirrors v1's `t3claw config init` ergonomics: refuses to clobber
...
impl ConfigInitCommand {
    pub(crate) fn execute(self, context: RebornCliContext) -> anyhow::Result<()> {
        ...
        println!();
        println!("edit them, then run `t3claw-reborn run`.");
        Ok(())
    }
}
...
fn config_stub() -> String {
    format!(
        r#"# T3Claw Reborn boot configuration.
#
# Layout:
...
#
# Regenerate with `t3claw-reborn config init --force`.
...
# # `sslmode=require` to T3CLAW_REBORN_POSTGRES_URL.
# backend = "postgres"
...
# [slack]
# # Host-beta Slack Events API route for `t3claw-reborn serve`.
