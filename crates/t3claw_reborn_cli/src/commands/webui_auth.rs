//! WebChat v2 auth-surface assembly for `t3claw-reborn serve`.
//!
...
use t3claw_reborn_composition::host_api::{AgentId, ProjectId, TenantId};
use t3claw_reborn_composition::{
    PublicRouteMount, RebornIdentityResolver, WebuiAuthenticator, open_local_trigger_access_store,
};
use t3claw_reborn_webui_ingress::{SignedSessionLoginConfig, build_signed_session_login};
...
pub(crate) async fn build_webui_auth_surface(
    ...
    eprintln!(
        "t3claw-reborn: WebChat v2 SSO login mounted — \
...
mod tests {
    ...
    use t3claw_reborn_composition::WebuiAuthentication;
    ...
    async fn sso_with_local_trigger_access_bootstrap_builds_surface() {
        ...
        let surface = build_webui_auth_surface(
            Some(sso),
            Some(t3claw_reborn_composition::open_reborn_identity_resolver(
