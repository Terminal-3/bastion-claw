...
impl ModelsListCommand {
    fn execute(self) -> anyhow::Result<()> {
        ...
        let admin =
            t3claw_reborn_composition::RebornProviderAdmin::new(context.boot_config().clone());
        let list = admin.list(
...
impl ModelsListCommand {
    fn execute(self) -> anyhow::Result<()> {
        let slots = t3claw_reborn_composition::reborn_model_slot_names();
        ...
        println!("T3Claw Reborn model slots");
        for slot in slots {
            println!("- {}", slot);
        }
...
impl ModelsStatusCommand {
    fn execute(self) -> anyhow::Result<()> {
        ...
        println!("T3Claw Reborn model status");
        println!("routes: not-configured");
        for slot in slots {
...
#[cfg(feature = "root-llm-provider")]
fn print_provider_list(list: &t3claw_reborn_composition::RebornProviderList, verbose: bool) {
    println!("T3Claw Reborn LLM providers");
...
#[cfg(feature = "root-llm-provider")]
fn print_provider_detail(list: &t3claw_reborn_composition::RebornProviderList) {
...
#[cfg(feature = "root-llm-provider")]
fn print_status(status: &t3claw_reborn_composition::RebornProviderStatus) {
    println!("T3Claw Reborn model status");
...
fn print_write_outcome(
    ...
    outcome: &t3claw_reborn_composition::RebornProviderWriteOutcome,
) {
    match kind {
