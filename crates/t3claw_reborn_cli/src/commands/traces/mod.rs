...
use t3claw_reborn_traces::client::{TraceClientHost, TraceClientScope};
use t3claw_reborn_traces::contribution::{
...
enum TracesSubcommand {
    ...
    OptIn {
        ...
        #[arg(long, default_value = "T3CLAW_TRACE_SUBMIT_TOKEN")]
        ...
        #[arg(long, default_value_t = t3claw_reborn_traces::contribution::TRACE_UPLOAD_CLAIM_DEFAULT_TIMEOUT_MS)]
    ...
    Preview {
        /// Recorded trace JSON file produced by T3CLAW_RECORD_TRACE
...
struct TraceQueueStatusDiagnostics {
    ...
    queue: t3claw_reborn_traces::contribution::TraceQueueDiagnostics,
...
fn show_policy_status(json: bool, user_scope: Option<&str>) -> anyhow::Result<()> {
    ...
    let queued_count = match normalized_scope.as_deref() {
        Some(scope) => {
            t3claw_reborn_traces::contribution::queued_trace_envelope_paths_for_scope(Some(scope))?
                .len()
        }
...
/// trims `T3CLAW_OWNER_ID` and falls back to the legacy "default" scope.
fn resolve_runtime_owner_scope() -> String {
    let env_owner_id = std::env::var("T3CLAW_OWNER_ID")
        .ok()
...
async fn preview_recorded_trace(options: PreviewOptions) -> anyhow::Result<()> {
    ...
    let recorded_trace: t3claw_reborn_traces::recording::TraceFile =
...
fn trace_contribution_dir() -> PathBuf {
    t3claw_reborn_traces::paths::t3claw_base_dir().join("trace_contributions")
}
...
fn write_policy(policy: &StandingTraceContributionPolicy) -> anyhow::Result<()> {
    ...
    // user-only access via t3claw_base_dir.
    #[cfg(unix)]
    {
        use std::io::Write;
