...
This crate owns the standalone `t3claw-reborn` command surface. Keep it small, explicit, and safe for agents to extend.
...
- Use `T3CLAW_REBORN_HOME` / `~/.t3claw/reborn`; do not write current v1 state.
- no v1 runtime imports: do not depend on root `t3claw`, `src/agent`, channels, worker, DB, setup, service, sandbox, or `t3claw_engine`.
- Do not add workspace dependencies beyond `t3claw_reborn_composition`, `t3claw_reborn_config`, `t3claw_reborn_traces`, and `t3claw_reborn_webui_ingress` (host-owned WebUI serve lifecycle) without an architecture test update and explicit PR rationale. Provider registry/auth/model UX should enter through the Reborn composition provider-admin facade, not a separate CLI-only path.
...
5. Add a binary smoke test in `tests/smoke.rs` that invokes `env!("CARGO_BIN_EXE_t3claw-reborn")`.
...
subcommand (`t3claw-reborn serve`). It is **off by default** so a
...
```bash
cargo install --path crates/t3claw_reborn_cli --features webui-v2-beta
# or, from a workspace checkout
cargo build -p t3claw_reborn_cli --features webui-v2-beta --release
```

When the feature is off, `t3claw-reborn --help` does not list `serve`
and `t3claw-reborn serve …` returns `error: unrecognized subcommand`.
...
themselves feature-gated so default `cargo test -p t3claw_reborn_cli`
...
`crates/t3claw_reborn_composition/tests/webui_v2_serve.rs`
