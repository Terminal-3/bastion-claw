...
//! HTTP gateway.
//!
//! `t3claw_reborn_composition::webui_v2_app` returns a fully composed
...
use t3claw_reborn_composition::WebuiAuthenticator;
use thiserror::Error;
...
pub async fn serve_webui_v2(opts: RebornWebuiServeOptions) -> Result<(), RebornWebuiServeError> {
    ...
    tracing::info!(
        target = "t3claw::reborn::webui_ingress",
        %bound,
...
/// variable. Intended for the standalone `t3claw-reborn` deployment
...
impl WebuiAuthenticator for EnvBearerAuthenticator {
    async fn authenticate(
        &self,
        candidate: &str,
    ) -> Option<t3claw_reborn_composition::WebuiAuthentication> {
        ...
        if expected.ct_eq(candidate).into() {
            Some(t3claw_reborn_composition::WebuiAuthentication::operator(
                self.user_id.clone(),
            ))
        } else {
            None
