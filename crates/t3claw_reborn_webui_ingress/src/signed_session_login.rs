...
//! `t3claw-reborn serve` binary needs to mount the OAuth login
...
use t3claw_reborn_composition::{WebuiAuthentication, WebuiAuthenticator};
use uuid::Uuid;
...
impl SignedTokenSessionStore {
    ...
    fn from_operator_secret(secret: &SecretString, tenant_id: &TenantId) -> Self {
        ...
        hasher.update(b"t3claw-reborn-webui-session-v1::");
