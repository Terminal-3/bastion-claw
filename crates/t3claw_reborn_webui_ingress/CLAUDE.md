# t3claw_reborn_webui_ingress guardrails

Host-owned counterpart to `t3claw_reborn_composition::webui_v2_app`.
...
behind the product/API seam in `t3claw_reborn_composition`.
...
- **Session transport** is one-time login ticket in the callback
  ...
  `t3claw_reborn_composition/CLAUDE.md` → "Session transport
...
```bash
cargo test -p t3claw_reborn_webui_ingress --all-features
cargo clippy -p t3claw_reborn_webui_ingress --all-features --tests -- -D warnings
```
