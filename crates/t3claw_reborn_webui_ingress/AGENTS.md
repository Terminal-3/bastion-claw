...
held in `t3claw_reborn_composition` (route descriptors + the
...
- Provide concrete `WebuiAuthenticator` implementations the standalone
  `t3claw-reborn` binary can wire (env-bearer first; DB / OIDC are
...
- Do not depend on v1's `src/`, `t3claw_engine`, channel code, or
...
- `t3claw_reborn_composition` (consumes the composed `Router` +
  `WebuiAuthenticator` trait + `WebuiServeConfig`)
- `t3claw_host_api` (identity types: `TenantId`, `UserId`)
...
2. Implement `WebuiAuthenticator` from `t3claw_reborn_composition`.
