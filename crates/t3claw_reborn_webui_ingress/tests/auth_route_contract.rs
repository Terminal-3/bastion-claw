...
//! 7. An `t3claw_session` cookie carrying a valid bearer does not
...
async fn cookie_session_not_honored_on_protected_route() {
    // v1 accepted an `t3claw_session` cookie as a credential and set
    // one on the OAuth callback; v2 never reads or writes cookies (#4116).
    // A request carrying a VALID session bearer in the `t3claw_session`
    ...
    let request = with_peer(
        Request::builder()
            ...
            .uri("/api/webchat/v2/threads")
            .header(header::CONTENT_TYPE, "application/json")
            .header(header::COOKIE, format!("t3claw_session={bearer}"))
            .body(Body::from(r#"{"client_action_id":"act-1"}"#))
            .expect("request"),
    );
    let response = app.oneshot(request).await.expect("oneshot");
    assert_eq!(
        response.status(),
        StatusCode::UNAUTHORIZED,
        "an `t3claw_session` cookie must not authenticate a v2 route (cookie transport dropped)",
    );
    assert_eq!(callers_len(&services), 0, "facade must not be reached");
}
