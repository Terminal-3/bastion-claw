//! Host runtime facade for T3Claw Reborn.
//!
//! `t3claw_host_runtime` is the narrow boundary upper Reborn services build
...
//!   [`t3claw_capabilities::CapabilityHost`] (which itself coordinates
...
pub(crate) const UNSAFE_RAW_HTTP_EGRESS_ERRORS_ENV: &str = "T3CLAW_UNSAFE_RAW_HTTP_EGRESS_ERRORS";
...
pub enum RuntimeWorkId {
    Invocation(t3claw_host_api::InvocationId),
    Process(ProcessId),
    Gate(RuntimeGateId),
}
