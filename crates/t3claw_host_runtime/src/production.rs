...
//! [`t3claw_capabilities::CapabilityHost`] with neutral kernel services —
...
//! [`TrustDecision`](t3claw_trust::TrustDecision) instead of caller-supplied
...
use t3claw_authorization::{CapabilityLeaseStore, TrustAwareCapabilityDispatchAuthorizer};
...
use t3claw_extensions::{ExtensionPackage, ExtensionRegistry, SharedExtensionRegistry};
...
use t3claw_trust::{HostTrustPolicy, TrustDecision, TrustError, TrustPolicy, TrustProvenance};
use t3claw_turns::run_profile::LoopSafeSummary;
...
impl DefaultHostRuntime {
    ...
    async fn fail_matching_blocked_resume_on_preflight_error(
        &self,
        context: &t3claw_host_api::ExecutionContext,
        capability_id: &CapabilityId,
...
) -> Result<t3claw_trust::TrustPolicyInput, TrustEvaluationError> {
    package
        .trust_policy_input(
...
fn auth_required_outcome(
    ...
    credential_requirements: Vec<t3claw_host_api::RuntimeCredentialAuthRequirement>,
) -> RuntimeCapabilityOutcome {
...
pub(crate) fn failure_kind_from(error: &CapabilityInvocationError) -> RuntimeFailureKind {
    match error {
        ...
        CapabilityInvocationError::ObligationFailed { kind, .. } => match kind {
            t3claw_capabilities::CapabilityObligationFailureKind::Audit => {
                RuntimeFailureKind::Backend
            }
            t3claw_capabilities::CapabilityObligationFailureKind::Mount => {
                RuntimeFailureKind::Authorization
            }
            t3claw_capabilities::CapabilityObligationFailureKind::Network => {
                RuntimeFailureKind::Network
            }
            t3claw_capabilities::CapabilityObligationFailureKind::Output => {
                RuntimeFailureKind::OutputTooLarge
            }
            t3claw_capabilities::CapabilityObligationFailureKind::Resource => {
                RuntimeFailureKind::Resource
            }
            t3claw_capabilities::CapabilityObligationFailureKind::Secret => {
                RuntimeFailureKind::Authorization
            }
        },
...
mod tests {
    ...
    //! [`t3claw_host_api::DispatchFailureKind`] values. Their display
    ...
    use super::*;
    use t3claw_capabilities::CapabilityInvocationError;
