...
async fn host_runtime_http_egress_port_executes_with_host_staged_credentials() {
    ...
    assert!(matches!(
        error,
        t3claw_host_api::RuntimeHttpEgressError::Credential { .. }
    ));
...
async fn host_http_egress_treats_expired_staged_secret_as_missing() {
    ...
    assert!(matches!(
        error,
        t3claw_host_api::RuntimeHttpEgressError::Credential { ref reason }
...
async fn host_runtime_services_with_security_audit_sink_records_leak_block() {
    use t3claw_events::{InMemorySecurityAuditSink, SecurityBoundary, SecurityDecision};
...
fn test_descriptor(runtime: RuntimeKind, effects: Vec<EffectKind>) -> CapabilityDescriptor {
    CapabilityDescriptor {
        ...
        provider: t3claw_host_api::ExtensionId::new("test").unwrap(),
        runtime,
...
fn policy_with(
    ...
    EffectiveRuntimePolicy {
        ...
        network_mode,
        secret_mode,
        approval_policy: t3claw_host_api::runtime_policy::ApprovalPolicy::AskDestructive,
        audit_mode: t3claw_host_api::runtime_policy::AuditMode::LocalMinimal,
    }
}
...
fn stage_secret_error_maps_auth_and_backend_variants() {
    ...
    use t3claw_host_api::CredentialStageError::{AuthRequired, Backend};
