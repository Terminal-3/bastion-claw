...
- Low-level mediation by composing `t3claw_network`/`t3claw_secrets`/`t3claw_resources` (egress, redaction, secret leases, accounting) — never duplicating that logic in runtime crates.
...
- Fast local check: `cargo test -p t3claw_host_runtime`
