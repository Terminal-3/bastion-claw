# t3claw_host_runtime guardrails
...
- Compose low-level services such as `t3claw_network` and `t3claw_secrets`; do not duplicate URL parsing, DNS checks, private-IP filtering, HTTP clients, secret stores, or redaction logic in runtime crates.
...
- Keep readiness checks near the runtime service they validate; driver/product
  readiness belongs in `t3claw_reborn`.
