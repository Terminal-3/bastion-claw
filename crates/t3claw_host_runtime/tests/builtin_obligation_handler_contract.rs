...
use serde_json::json;
use t3claw_authorization::TrustAwareCapabilityDispatchAuthorizer;
...
use t3claw_secrets::{InMemorySecretStore, SecretMaterial, SecretStore};
use t3claw_trust::{AuthorityCeiling, EffectiveTrustClass, TrustDecision, TrustProvenance};

fn local_test_runtime_policy() -> t3claw_host_api::runtime_policy::EffectiveRuntimePolicy {
    t3claw_runtime_policy::resolve(t3claw_runtime_policy::ResolveRequest::new(
        t3claw_host_api::runtime_policy::DeploymentMode::LocalSingleUser,
        t3claw_host_api::runtime_policy::RuntimeProfile::LocalDev,
    ))
    .unwrap()
}
...
async fn builtin_obligation_handler_removes_network_policy_on_abort() {
    ...
    let outcome = handler
        ...
        .unwrap();
    handler
        .abort(t3claw_capabilities::CapabilityObligationAbortRequest {
            phase: CapabilityObligationPhase::Invoke,
            context: &context,
            capability_id: &capability_id,
            estimate: &estimate,
...
impl RuntimeCredentialAccountResolver for AlwaysAuthRequiredResolver {
    ...
    ) -> Result<RuntimeCredentialAccessSecret, t3claw_host_api::CredentialStageError> {
        Err(t3claw_host_api::CredentialStageError::AuthRequired)
    }
}

#[derive(Debug)]
struct FixedHandleResolver(t3claw_host_api::SecretHandle);
...
struct SourceScopedHandleResolver {
    ...
    handle: t3claw_host_api::SecretHandle,
}
...
async fn inject_credential_account_once_fails_when_no_resolver_wired() {
    ...
    let secret_store = Arc::new(t3claw_secrets::InMemorySecretStore::new());
    let governor = Arc::new(t3claw_resources::InMemoryResourceGovernor::new());
    let services = BuiltinObligationServices::new(
        Arc::new(t3claw_events::InMemoryAuditSink::new()),
        secret_store,
        governor,
    );
    let handler = services.obligation_handler();
    ...
    let obligations = vec![Obligation::InjectCredentialAccountOnce {
        handle: t3claw_host_api::SecretHandle::new("github_runtime_token").unwrap(),
        provider: t3claw_host_api::RuntimeCredentialAccountProviderId::new("github").unwrap(),
        provider_scopes: Vec::new(),
        requester_extension: t3claw_host_api::ExtensionId::new("github").unwrap(),
    }];
...
async fn inject_credential_account_once_resolves_and_stages_secret() {
    ...
    let access_handle = t3claw_host_api::SecretHandle::new("github_access_secret").unwrap();
    let injection_slot = t3claw_host_api::SecretHandle::new("github_runtime_token").unwrap();
    let secret_store = Arc::new(t3claw_secrets::InMemorySecretStore::new());
    ...
    secret_store
        .put(
            execution_context(CapabilitySet::default())
                ...
                .clone(),
            access_handle.clone(),
            t3claw_secrets::SecretMaterial::from("test-github-token"),
        )
        .await
        .unwrap();
    let governor = Arc::new(t3claw_resources::InMemoryResourceGovernor::new());
...
async fn inject_credential_account_once_maps_unknown_resolved_secret_to_auth_required() {
    ...
    let access_handle = t3claw_host_api::SecretHandle::new("missing_secret").unwrap();
    let secret_store = Arc::new(t3claw_secrets::InMemorySecretStore::new()); // empty store
    let governor = Arc::new(t3claw_resources::InMemoryResourceGovernor::new());
    let services = BuiltinObligationServices::new(
        Arc::new(t3claw_events::InMemoryAuditSink::new()),
        secret_store,
