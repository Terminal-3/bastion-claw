...
use serde_json::json;
use t3claw_approvals::LeaseApproval;
use t3claw_authorization::{
...
#[cfg(feature = "libsql")]
use t3claw_filesystem::ScopedFilesystem;
...
use t3claw_mcp::{McpError, McpExecutionRequest, McpExecutionResult, McpExecutor};
use t3claw_network::{
    NetworkHttpEgress, NetworkHttpError, NetworkHttpRequest, NetworkHttpResponse, NetworkUsage,
};
use t3claw_processes::{
...
use t3claw_secrets::{InMemoryCredentialBroker, InMemorySecretStore, SecretMaterial, SecretStore};
use t3claw_triggers::InMemoryTriggerRepository;
use t3claw_trust::{
...
use t3claw_turns::{NoopTurnRunWakeNotifier, TurnRunWake, TurnRunWakeNotifier};
use t3claw_wasm::{
...
async fn with_filesystem_resource_governor_persists_reservations_across_handles() {
    use t3claw_filesystem::{InMemoryBackend, ScopedFilesystem};
...
// graph restart over `LocalFilesystem`) and the `t3claw_run_state`
// contract suite.
...
impl TurnRunWakeNotifier for RecordingTurnRunWakeNotifier {
    fn notify_queued_run(
        &self,
        wake: TurnRunWake,
    ) -> Result<(), t3claw_turns::TurnRunWakeNotifyError> {
...
async fn production_turn_coordinator_uses_configured_store_and_notifier() {
    ...
    let state = reopened
        .get_run_state(t3claw_turns::GetRunStateRequest {
            scope: request.scope,
            run_id,
        })
        .await
...
async fn host_runtime_services_jsonl_approval_audit_projection_rejects_foreign_cursor_without_leaks()
 {
    ...
    .with_approval_requests(Arc::clone(&approval_requests))
    .with_capability_leases(Arc::clone(&capability_leases))
    .with_audit_sink(Arc::new(t3claw_events::DurableAuditSink::new(Arc::clone(
        &audit_log,
    ))))
    .with_script_runtime(Arc::new(ScriptRuntime::new(
        ScriptRuntimeConfig::for_testing(),
...
async fn host_runtime_spawn_process_sandbox_rejects_invalid_plan_before_executor() {
    ...
    match error {
        t3claw_host_runtime::HostRuntimeError::InvalidRequest { reason } => {
...
async fn spawned_obligation_lifecycle_abort_cleans_up_when_process_start_fails() {
    ...
    assert!(matches!(
        err,
        t3claw_capabilities::CapabilityInvocationError::Process { .. }
    ));
    assert!(matches!(
        fixture.governor.release(reservation_id).unwrap_err(),
        ResourceError::ReservationClosed {
...
) -> t3claw_host_runtime::RuntimeApprovalGate {
    let outcome = runtime
        .invoke_capability(RuntimeCapabilityRequest::new(
            context,
            script_capability_id(),
...
) -> t3claw_authorization::CapabilityLease {
    services
        .approval_resolver()
...
impl ScriptExecutor for RecordingScriptExecutor {
    fn execute_extension_json(
    ...
    ) -> Result<ScriptExecutionResult, t3claw_scripts::ScriptError> {
        ...
        Ok(ScriptExecutionResult {
            result: t3claw_scripts::ScriptCapabilityResult {
...
impl DurableAuditLog for FailingDurableAuditLog {
    ...
    ) -> Result<t3claw_events::EventLogEntry<AuditEnvelope>, EventError> {
...
impl SpawnObligationFixture {
    async fn spawn(&self) -> t3claw_processes::ProcessRecord {
        let host = CapabilityHost::new(
...
impl ResourceGovernor for FailingCleanupResourceGovernor {
    ...
    fn reserve_with_outcome(
        ...
        scope: ResourceScope,
        estimate: ResourceEstimate,
    ) -> Result<t3claw_resources::ReservationOutcome, ResourceError> {
        Ok(t3claw_resources::ReservationOutcome {
    ...
    fn account_snapshot(
        &self,
        _account: &ResourceAccount,
    ) -> Result<Option<t3claw_resources::AccountSnapshot>, ResourceError> {
        Ok(None)
    }
}
...
impl ProcessStore for FailingTerminalProcessStore {
    async fn start(
        &self,
        start: ProcessStart,
    ) -> Result<t3claw_processes::ProcessRecord, ProcessError> {
    ...
    async fn get(
        ...
        scope: &ResourceScope,
        process_id: ProcessId,
    ) -> Result<Option<t3claw_processes::ProcessRecord>, ProcessError> {
    ...
    async fn records_for_scope(
        &self,
        scope: &ResourceScope,
    ) -> Result<Vec<t3claw_processes::ProcessRecord>, ProcessError> {
...
impl ProcessExecutor for BackgroundExecutor {
    ...
    ) -> Result<ProcessExecutionResult, t3claw_processes::ProcessExecutionError> {
        match &self.outcome {
            BackgroundExecutorOutcome::Success(output) => Ok(ProcessExecutionResult {
                output: output.clone(),
            }),
            BackgroundExecutorOutcome::Failure(kind) => {
                Err(t3claw_processes::ProcessExecutionError::new(kind.clone()))
            }
...
#[async_trait]
impl t3claw_processes::ProcessManager for FailingSpawnManager {
    async fn spawn(
        &self,
        _start: ProcessStart,
    ) -> Result<t3claw_processes::ProcessRecord, ProcessError> {
