...
use t3claw_filesystem::{InMemoryBackend, LocalFilesystem, RootFilesystem, ScopedFilesystem};
use t3claw_host_api::{
...
use t3claw_scripts::{ScriptHostHttpRequest, ScriptRuntimeHttpAdapter};
use t3claw_secrets::{InMemorySecretStore, SecretMaterial, SecretStore};
use t3claw_wasm::{WasmHostHttp, WasmHttpRequest, WasmRuntimeHttpAdapter};
...
async fn host_http_egress_removes_staged_secret_before_network_errors() {
    ...
    assert!(matches!(
        error,
        t3claw_host_api::RuntimeHttpEgressError::Network { .. }
    ));
...
async fn host_http_egress_fails_closed_when_real_scoped_filesystem_write_fails() {
    ...
    root.mount_local(
        VirtualPath::new("/projects/workspace").unwrap(),
        t3claw_host_api::HostPath::from_path_buf(temp.path().to_path_buf()),
    )
    .unwrap();
    let scoped_filesystem = Arc::new(ScopedFilesystem::with_fixed_view(
        Arc::new(root),
...
async fn host_http_egress_blocks_credential_shaped_response_body() {
    ...
    assert!(matches!(
        error,
        t3claw_host_api::RuntimeHttpEgressError::Response { ref reason, .. }
...
async fn host_http_egress_blocks_percent_encoded_credential_path_before_network() {
    ...
    assert!(matches!(
        error,
        t3claw_host_api::RuntimeHttpEgressError::Request { ref reason, .. }
...
async fn host_http_egress_blocks_runtime_supplied_sensitive_headers_before_network() {
    ...
    assert!(matches!(
        error,
        t3claw_host_api::RuntimeHttpEgressError::Request { .. }
    ));
