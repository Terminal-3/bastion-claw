...
impl TurnRunTransitionPort for FailingClaimTransitions {
    ...
    ) -> Result<t3claw_turns::EventCursor, TurnError> {
...
impl TurnStateStore for DurableLikeTurnStore {
    async fn submit_turn(
        ...
        admission_policy: &dyn t3claw_turns::TurnAdmissionPolicy,
        run_profile_resolver: &dyn t3claw_turns::RunProfileResolver,
    ...
    for _ in 0..8 {
        handle
            ...
            .notify_queued_run(TurnRunWake {
                ...
                run_id: t3claw_turns::TurnRunId::new(),
                status: TurnStatus::Queued,
                event_cursor: t3claw_turns::EventCursor::default(),
            })
            .unwrap();
    }
...
async fn wake_notifier_drains_submitted_run() {
    ...
    assert_eq!(
        store
            .get_run_state(t3claw_turns::GetRunStateRequest { scope, run_id })
            .await
            .unwrap()
            .status,
...
async fn expired_lease_reconciler_fails_running_run() {
    ...
    store
        .claim_next_run(ClaimRunRequest {
            runner_id: t3claw_turns::TurnRunnerId::new(),
            lease_token: t3claw_turns::TurnLeaseToken::new(),
...
fn accepted_run_id(response: SubmitTurnResponse) -> t3claw_turns::TurnRunId {
