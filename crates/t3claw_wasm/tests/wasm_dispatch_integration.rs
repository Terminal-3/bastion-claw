...
fn preserved_wasm_error_usage(error: &t3claw_wasm::WasmError) -> Option<ResourceUsage> {
    if let t3claw_wasm::WasmError::ExecutionFailed { usage, .. } = error
...
fn registry_with_package(manifest: &str) -> t3claw_extensions::ExtensionRegistry {
    let mut registry = t3claw_extensions::ExtensionRegistry::new();
...
fn wasm_error_kind(error: &t3claw_wasm::WasmError) -> RuntimeDispatchErrorKind {
    match error {
        t3claw_wasm::WasmError::EngineCreationFailed(_) => RuntimeDispatchErrorKind::Executor,
        t3claw_wasm::WasmError::CompilationFailed(_) => RuntimeDispatchErrorKind::Manifest,
        t3claw_wasm::WasmError::StoreConfiguration(_) => RuntimeDispatchErrorKind::Executor,
        t3claw_wasm::WasmError::LinkerConfiguration(_) => RuntimeDispatchErrorKind::Executor,
        t3claw_wasm::WasmError::InstantiationFailed(_) => RuntimeDispatchErrorKind::MethodMissing,
        t3claw_wasm::WasmError::ExecutionFailed { .. } => RuntimeDispatchErrorKind::Guest,
        t3claw_wasm::WasmError::InvalidSchema(_) => RuntimeDispatchErrorKind::Manifest,
    }
}
