# `t3claw_trust` — cross-crate contract

**Status:** V1 contract
**Crate:** `crates/t3claw_trust`
**Depends on:** `t3claw_host_api` (vocabulary)
**Consumed by:** `t3claw_authorization` (and any future grant-store /
...
`t3claw_trust` evaluates host trust policy for any manifest-bearing
...
`TrustClass` (in `t3claw_host_api`) names the four ceilings; the
...
|---|---|---|---|
| `RequestedTrustClass` | `t3claw_host_api::trust` | manifest / registry entry | freely deserializable; not authority |
| `TrustClass` | `t3claw_host_api::runtime` | wire vocabulary | `FirstParty`, `System` reject deserialization (`#[serde(skip_deserializing)]`) |
| `EffectiveTrustClass` | `t3claw_trust::decision` | `HostTrustPolicy::evaluate` | privileged constructors are crate-private; serializable for audit but **not** deserializable |
| `HostTrustAssignment` | `t3claw_trust::decision` | host-controlled bundle/admin loaders | production seeding token for policy entries; **not** an authorization input and not deserializable |
...
`t3claw_host_api::capability`) carries the manifest's *declarative
...
Trust class on its own grants nothing. `t3claw_authorization`
...
| Grant / lease | per-tool `PermissionMode` decision; no formal `CapabilityGrant` | `CapabilityGrant` / `CapabilityLease` (`t3claw_authorization`, see `capability-access.md`) | Unified — grants for both |
| Approval | per-tool `approval` setting + UnlessAutoApproved gate | gate via `t3claw_authorization` + obligation handlers | Unified — see `approvals.md` |
...
| Network | per-tool allowlist (HTTP tool's `allowed_hosts`) | network obligations (`network.md`) | Unified through `t3claw_network` |
...
Those should be added as follow-on slices once `t3claw_authorization`
