//! Host-controlled trust-class policy engine for T3Claw Reborn.
//!
//! `t3claw_trust` is the bridge between the *requested* trust an untrusted
...
//!
//! See `crates/t3claw_trust/CONTRACT.md` for the full cross-crate
...
#[cfg(test)]
mod tests {
    //! Lib-level smoke tests that run on bare `cargo test -p t3claw_trust`.
    ...
    fn fail_closed_policy_returns_default_for_local_manifest() {
        use t3claw_host_api::{PackageId, PackageIdentity, PackageSource, RequestedTrustClass};
        ...
        assert_eq!(
            ...
            t3claw_host_api::TrustClass::Sandbox
        );
