...
- Provide libSQL and PostgreSQL-backed implementations of
  `t3claw_product_workflow::IdempotencyLedger`.
- Persist product inbound action reservations and terminal outcomes through
  `t3claw_filesystem::RootFilesystem`.
...
```bash
cargo test -p t3claw_product_workflow_storage --features libsql
cargo test -p t3claw_product_workflow_storage --features postgres --no-run
cargo check -p t3claw_product_workflow_storage --features "libsql postgres"
cargo clippy -p t3claw_product_workflow_storage --all-targets --features "libsql postgres" -- -D warnings
```

PostgreSQL runtime tests require `T3CLAW_PRODUCT_WORKFLOW_POSTGRES_URL`; when
