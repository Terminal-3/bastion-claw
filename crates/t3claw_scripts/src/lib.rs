//! Script runner contracts for T3Claw Reborn.
//!
//! `t3claw_scripts` executes declared script/CLI capabilities through a
...
use t3claw_extensions::{ExtensionPackage, ExtensionRuntime};
use t3claw_host_api::{
...
use t3claw_resources::{ResourceError, ResourceGovernor, ResourceReceipt};
use thiserror::Error;
...
pub struct ScriptHostHttpRequest {
    pub scope: ResourceScope,
    pub capability_id: CapabilityId,
    pub method: t3claw_host_api::NetworkMethod,
    pub url: String,
    pub headers: Vec<(String, String)>,
    pub body: Vec<u8>,
    pub network_policy: t3claw_host_api::NetworkPolicy,
    pub credential_injections: Vec<t3claw_host_api::RuntimeCredentialInjection>,
...
mod tests {
    ...
    use t3claw_host_api::{CapabilityId, InvocationId, ResourceScope, TenantId, UserId};
    ...
    fn docker_backend_disables_ambient_network_before_image_and_manifest_args() {
        ...
        let old_args_file = std::env::var_os("T3CLAW_DOCKER_ARGS_FILE");
        let _restore = EnvRestore(vec![
            ("PATH", old_path.clone()),
            ("T3CLAW_DOCKER_ARGS_FILE", old_args_file),
        ]);
        ...
        unsafe {
            ...
            std::env::set_var("T3CLAW_DOCKER_ARGS_FILE", &args_file);
        }
    ...
    fn sample_docker_request() -> ScriptBackendRequest {
        ScriptBackendRequest {
            provider: t3claw_host_api::ExtensionId::new("script").unwrap(),
