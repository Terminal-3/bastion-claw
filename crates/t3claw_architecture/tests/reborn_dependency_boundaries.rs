...
fn reborn_virtual_roots_match_storage_placement_contract() {
    let root = workspace_root();
    let path_source = std::fs::read_to_string(root.join("crates/t3claw_host_api/src/path.rs"))
    ...
    assert_eq!(
        implemented, storage,
        "t3claw_host_api VIRTUAL_ROOTS must match storage-placement.md canonical roots"
    );
    assert_eq!(
        filesystem, storage,
...
fn reborn_crate_dependency_boundaries_hold() {
    ...
    assert_no_normal_workspace_deps(
        &dependencies,
        "t3claw_host_api",
        workspace_t3claw_crates(&dependencies)
            .into_iter()
            .filter(|name| *name != "t3claw_host_api")
            .collect::<Vec<_>>(),
    );
...
fn conversation_trusted_trigger_submitter_stays_conversation_or_composition_owned() {
    ...
    let allowed = BTreeSet::from([
        "crates/t3claw_architecture/tests/reborn_dependency_boundaries.rs",
        "crates/t3claw_conversations/src/inbound.rs",
    ]);
    let violations = uses
        .into_iter()
...
fn conversation_trusted_trigger_submitter_stays_out_of_root_exports() {
    let root = workspace_root();
    let lib_source = std::fs::read_to_string(root.join("crates/t3claw_conversations/src/lib.rs"))
        .expect("conversation lib source must be readable");

    assert!(
        !lib_source.contains("ConversationTrustedTriggerSubmitter"),
        "ConversationTrustedTriggerSubmitter must not be re-exported from t3claw_conversations; \
...
fn conversation_trusted_trigger_classifier_stays_out_of_root_exports() {
    ...
    assert!(
        ...
        "classify_trusted_trigger_inbound_error is submitter policy and must not be re-exported \
         from t3claw_conversations; composition-owned materialization should classify its own \
         local errors"
    );
    assert!(
        !lib_source.contains("classify_inbound_error"),
        "trusted trigger inbound classification must not be re-exported from \
         t3claw_conversations; keep it private to conversations-owned submitter policy"
    );
    assert!(
        ...
        "trusted trigger inbound classification types must not be re-exported from \
         t3claw_conversations; keep them private to conversations-owned submitter policy"
    );
    assert!(
...
fn trusted_trigger_submit_request_minting_stays_worker_owned() {
    ...
    let allowed_struct_literals = BTreeSet::from([
        "crates/t3claw_architecture/tests/reborn_dependency_boundaries.rs",
        "crates/t3claw_triggers/src/worker/ports.rs",
    ]);
...
fn retired_host_trusted_ingress_token_crate_stays_removed() {
    let root = workspace_root();
    let retired_crate_name = ["t3claw", "trusted", "ingress"].join("_");
    assert!(
        !root
            .join("crates")
        ...
        "a separate trusted ingress crate must stay absent; trusted trigger \
         submission is sealed by t3claw_triggers and privately converted inside \
    ...
    assert!(
        ...
        "a separate trusted ingress crate must not be introduced as a production dependency; \
         trusted trigger submission is now sealed by t3claw_triggers and privately \
         converted inside t3claw_conversations. Unexpected dependents:\n{}",
        violations.join("\n")
    );
}
...
fn untrusted_ingress_paths_cannot_submit_host_trusted_inbound() {
    ...
    let untrusted_src_roots = [
        "crates/t3claw_capabilities/src",
        "crates/t3claw_first_party_extension_ports/src",
        "crates/t3claw_first_party_extensions/src",
        "crates/t3claw_host_api/src",
        "crates/t3claw_host_runtime/src",
        "crates/t3claw_product_adapters/src",
        "crates/t3claw_product_adapter_registry/src",
        "crates/t3claw_product_workflow/src",
        "crates/t3claw_product_workflow_storage/src",
        "crates/t3claw_reborn_webui_ingress/src",
        "crates/t3claw_wasm_product_adapters/src",
        "crates/t3claw_webui_v2/src",
        "crates/t3claw_telegram_v2_adapter/src",
        "crates/t3claw_slack_v2_adapter/src",
    ];
...
fn reborn_cli_binary_crate_stays_separate_from_v1_root() {
    ...
    let root = workspace_root();
    let manifest_path = root.join("crates/t3claw_reborn_cli/Cargo.toml");
    assert!(
    ...
    assert!(
        manifest.contains("name = \"t3claw_reborn_cli\""),
        "Reborn CLI crate package name should be t3claw_reborn_cli"
    );
    assert!(
        manifest.contains("[[bin]]") && manifest.contains("name = \"t3claw-reborn\""),
        "Reborn CLI crate must declare the t3claw-reborn binary explicitly"
    );

    let command_module_paths = [
        "crates/t3claw_reborn_cli/AGENTS.md",
        "crates/t3claw_reborn_cli/src/commands/mod.rs",
        "crates/t3claw_reborn_cli/src/commands/completion.rs",
        "crates/t3claw_reborn_cli/src/commands/doctor.rs",
        "crates/t3claw_reborn_cli/src/commands/repl.rs",
        "crates/t3claw_reborn_cli/src/commands/run.rs",
        "crates/t3claw_reborn_cli/src/commands/serve.rs",
        "crates/t3claw_reborn_cli/src/context.rs",
    ];
    ...
    let agent_contract = std::fs::read_to_string(root.join("crates/t3claw_reborn_cli/AGENTS.md"))
    ...
    assert_workspace_deps_exactly(
        ...
        [
            ...
            "t3claw_reborn_config",
            "t3claw_reborn_traces",
            "t3claw_reborn_webui_ingress",
        ],
        "t3claw_reborn_cli should enter Reborn through t3claw_reborn_composition (assembled-runtime and provider-admin facade), t3claw_reborn_config (boot-config contract), t3claw_reborn_traces (contributor-side TraceCommons client extracted from the legacy monolith), and t3claw_reborn_webui_ingress (host-owned WebUI serve lifecycle) only. Adding any other workspace crate here re-opens speculative public API access to internal Reborn types.",
    );
    assert_workspace_deps_exactly(
        &dependencies_all_kinds,
        "t3claw_reborn_config",
        [],
        "t3claw_reborn_config must remain a standalone boot contract crate with no T3Claw workspace dependencies of any dependency kind",
    );

    let runtime_dir = root.join("crates/t3claw_reborn_cli/src/runtime");
    ...
    assert!(
        ...
        "Reborn CLI should enter the assembled runtime through t3claw_reborn_composition::build_reborn_runtime"
    );
    for forbidden in [
        ...
        assert!(
            ...
            "Reborn CLI runtime/ must not wire lower-level Reborn runtime pieces directly via `{forbidden}`; keep REPL as a UX shell over t3claw_reborn_composition."
        );
    }
}
...
fn reborn_host_runtime_services_do_not_expose_lower_substrate_handles() {
    let root = workspace_root();
    let lib = std::fs::read_to_string(root.join("crates/t3claw_host_runtime/src/lib.rs"))
        .expect("host runtime lib.rs must be readable");
    let services = std::fs::read_to_string(root.join("crates/t3claw_host_runtime/src/services.rs"))
        .expect("host runtime services.rs must be readable");
    let obligations =
        std::fs::read_to_string(root.join("crates/t3claw_host_runtime/src/obligations.rs"))
    ...
    let scripts = std::fs::read_to_string(root.join("crates/t3claw_scripts/src/lib.rs"))
        .expect("script runtime lib.rs must be readable");
    let scripts_manifest = std::fs::read_to_string(root.join("crates/t3claw_scripts/Cargo.toml"))
        .expect("script runtime Cargo.toml must be readable");
    let mcp = std::fs::read_to_string(root.join("crates/t3claw_mcp/src/lib.rs"))
        .expect("MCP runtime lib.rs must be readable");
    let mcp_manifest = std::fs::read_to_string(root.join("crates/t3claw_mcp/Cargo.toml"))
    ...
    for export in forbidden_lib_exports {
        assert!(
            !lib.contains(export),
            "t3claw_host_runtime must not re-export lower substrate handle `{export}`; upper Reborn code should enter through HostRuntimeServices::host_runtime / Arc<dyn HostRuntime>"
        );
    }
    ...
    for export in forbidden_obligation_exports {
        assert!(
            ...
            "t3claw_host_runtime must not re-export lower substrate handoff store `{export}`; upper Reborn code should enter through HostRuntimeServices::host_runtime / Arc<dyn HostRuntime>"
        );
    }
    ...
    for pattern in forbidden_script_lane_surface {
        assert!(
            ...
            "t3claw_scripts must not expose host-runtime dispatcher composition surface `{pattern}`; compose script dispatch adapters inside t3claw_host_runtime"
        );
    }

    assert!(
        !scripts_manifest.contains("t3claw_dispatcher"),
        "t3claw_scripts must not depend on t3claw_dispatcher; script dispatcher adapters are host-runtime-private composition"
    );
    ...
    for pattern in forbidden_mcp_lane_surface {
        assert!(
            ...
            "t3claw_mcp must not expose host-runtime dispatcher composition surface `{pattern}`; compose MCP dispatch adapters inside t3claw_host_runtime"
        );
    }
    assert!(
        !mcp_manifest.contains("t3claw_dispatcher"),
        "t3claw_mcp must not depend on t3claw_dispatcher; MCP dispatcher adapters are host-runtime-private composition"
    );
}
...
fn reborn_turns_public_surface_keeps_runner_api_explicit() {
    let root = workspace_root();
    let lib = std::fs::read_to_string(root.join("crates/t3claw_turns/src/lib.rs"))
    ...
    for pattern in forbidden_public_exports {
        assert!(
            !lib.contains(pattern),
            "t3claw_turns public prelude must not re-export trusted runner transition API `{pattern}`; adapters must import t3claw_turns::runner explicitly"
        );
    }
}
...
fn reborn_loop_support_llm_wiring_stays_out_of_root_src() {
    ...
    assert!(
        ...
        "Reborn loop LLM wiring must live under crates/t3claw_reborn, not root src/lib.rs"
    );
    assert!(
    ...
    let reborn_gateway = root.join("crates/t3claw_reborn/src/model_gateway.rs");
    assert!(
    ...
    assert!(
        ...
        "Reborn LLM gateway wiring should expose LlmProviderModelGateway from crates/t3claw_reborn"
    );

    let reborn_manifest = std::fs::read_to_string(root.join("crates/t3claw_reborn/Cargo.toml"))
    ...
    assert!(
        ...
        "t3claw_reborn may reuse root LLM code only behind an explicit feature, without enabling the root app's default postgres/libsql/tui feature set"
    );

    // The composition root — the only crate that should pull `t3claw_reborn`
    // (and through it `t3claw_llm`) for the assembled runtime — must mirror
    // the same feature-gated discipline. Both `t3claw_reborn` (transitive)
    // and `t3claw_llm` (direct) live behind a `root-llm-provider` feature
    // on the composition crate, so a default build of composition stays
    // substrate-only.
    let composition_manifest =
        std::fs::read_to_string(root.join("crates/t3claw_reborn_composition/Cargo.toml"))
            .expect("Reborn composition manifest must be readable");
    assert!(
        composition_manifest.contains("root-llm-provider")
            && composition_manifest.contains("t3claw_llm")
            && composition_manifest.contains("optional = true")
            && composition_manifest.contains("default-features = false"),
        "t3claw_reborn_composition must gate `t3claw_llm` behind the same `root-llm-provider` feature with `optional = true, default-features = false`"
    );
}

/// Lock the narrowed `t3claw_reborn` public surface in place.
///
/// `t3claw_reborn` previously exposed ~25 types as a wall of `pub use`
...
/// (`t3claw_reborn::driver_registry::DriverRegistry`, etc.). The wall was
/// pure speculative public API.
///
/// This test pins the cleanup: `crates/t3claw_reborn/src/lib.rs` must be a
...
/// taking a normal cargo dep on `t3claw_reborn`.
#[test]
fn reborn_internal_crate_keeps_directory_of_modules_lib_rs() {
    let root = workspace_root();
    let lib = std::fs::read_to_string(root.join("crates/t3claw_reborn/src/lib.rs"))
        .expect("t3claw_reborn lib.rs must be readable");
    ...
    // path or (preferably) consume them through `t3claw_reborn_composition`.
    let forbidden_reexports = [
        "pub use t3claw_loop_support::",
    ...
    for forbidden in forbidden_reexports {
        assert!(
            ...
            "t3claw_reborn/src/lib.rs must not re-export internal items via `{forbidden}`. \
             Reach them through the `pub mod` path or through t3claw_reborn_composition. \
             See `reborn_internal_crate_keeps_directory_of_modules_lib_rs` for context."
        );
    }

    // The composition root is the sanctioned consumer of `t3claw_reborn`'s
    ...
    // forbid).
    let composition_runtime = root.join("crates/t3claw_reborn_composition/src/runtime.rs");
    let composition_local_dev_runtime =
        root.join("crates/t3claw_reborn_composition/src/runtime/local_dev.rs");
    assert!(
    ...
    for required in [
        ...
        assert!(
            ...
            "composition runtime.rs missing `{required}` -- the runtime assembly slice \
             must live in `t3claw_reborn_composition` so the CLI and other \
             ingress points can avoid importing `t3claw_reborn` directly."
        );
    }
    assert!(
        composition_runtime_sources.contains("use t3claw_loop_support::")
        ...
        "composition runtime module set missing loop-support capability factory wiring -- \
         ...
         `t3claw_reborn_composition` rather than the CLI or other ingress points."
    );
}

/// Lock the boot-config TOML + provider-catalog layering for the
/// standalone `t3claw-reborn` binary.
///
/// Three properties:
///
/// 1. `t3claw_reborn_config` continues to expose the boot-time parser
...
///    same filename as v1's `~/.t3claw/providers.json` so operator
...
fn reborn_boot_config_file_layout_is_pinned() {
    ...
    let config_lib = std::fs::read_to_string(root.join("crates/t3claw_reborn_config/src/lib.rs"))
    ...
    for required_export in [
        ...
        assert!(
            ...
            "t3claw_reborn_config/src/lib.rs must export `{required_export}`; \
             see reborn_boot_config_file_layout_is_pinned for context"
        );
    }

    let home_src = std::fs::read_to_string(root.join("crates/t3claw_reborn_config/src/home.rs"))
    ...
    // File names — these match v1's `~/.t3claw/providers.json` so the
    ...
    let config_file_src =
        std::fs::read_to_string(root.join("crates/t3claw_reborn_config/src/config_file.rs"))
    ...
    // composition without forcing `t3claw_reborn_config` to depend
    // on `t3claw_llm` (which would violate _config's standalone
    // boundary). The composition crate is the legitimate consumer.
    let llm_catalog = root.join("crates/t3claw_reborn_composition/src/llm_catalog.rs");
    assert!(
        llm_catalog.exists(),
    ...
    // `t3claw_llm` must expose the path-overridable loader so the
    // catalog file location is selectable per-deployment (the
    // standalone Reborn binary points at $T3CLAW_REBORN_HOME/providers.json,
    // not v1's ~/.t3claw/providers.json).
    let llm_registry = std::fs::read_to_string(root.join("crates/t3claw_llm/src/registry.rs"))
        .expect("t3claw_llm registry.rs must be readable");
    assert!(
        llm_registry.contains("pub fn load_from_path"),
        "t3claw_llm::ProviderRegistry must expose `load_from_path` so callers can \
         override the user-overlay catalog path; v1 hardcoded ~/.t3claw/providers.json \
...
fn reborn_turns_public_surface_uses_turn_ids_not_runtime_or_process_ids() {
    let root = workspace_root();
    let turns_src = root.join("crates/t3claw_turns/src");
    let mut violations = Vec::new();
    collect_forbidden_turns_identifier_uses(&turns_src, &root, &mut violations);

    assert!(
        violations.is_empty(),
        "t3claw_turns public API must use TurnId/TurnRunId instead of lower runtime/process identifiers:\n{}",
        violations.join("\n")
    );
}
...
fn wasm_sandbox_core_is_standalone_v1_parity_kernel() {
    let root = workspace_root().join("crates/t3claw_wasm_sandbox_core");
    assert!(
    ...
    let package = packages
        .iter()
        .find(|package| package["name"] == "t3claw_wasm_sandbox_core")
        .expect("t3claw_wasm_sandbox_core must be a workspace package");
    let workspace_deps = workspace_dependency_names(package)
        .filter_map(|dependency| dependency["name"].as_str())
        .collect::<Vec<_>>();
    assert!(
        workspace_deps.is_empty(),
        "WASM sandbox core should stay independent of T3Claw domain crates; got {workspace_deps:?}"
    );
}

#[test]
fn wasm_product_adapter_crate_has_local_guardrails() {
    let guardrails = workspace_root().join("crates/t3claw_wasm_product_adapters/CLAUDE.md");
    assert!(
        guardrails.exists(),
        "t3claw_wasm_product_adapters needs local CLAUDE.md guardrails before becoming a Reborn boundary crate"
    );
}
...
fn wasm_product_adapter_crate_keeps_minimal_host_glue_dependencies() {
    ...
    let package = packages
        .iter()
        .find(|package| package["name"] == "t3claw_wasm_product_adapters")
        .expect("t3claw_wasm_product_adapters must be a workspace package");
    ...
    //   * t3claw_wasm_sandbox_core — shared v1-style minimal WASI sandbox kernel.
    ...
    assert_eq!(
        deps, expected,
        "t3claw_wasm_product_adapters should stay thin host glue; add runtime/workflow dependencies only when a call-site proves they are required"
    );
}
...
fn wasm_product_adapter_runtime_uses_v1_style_minimal_wasi() {
    let root = workspace_root();
    let core = std::fs::read_to_string(root.join("crates/t3claw_wasm_sandbox_core/src/lib.rs"))
        .expect("WASM sandbox core must be readable");
    let adapter_store =
        std::fs::read_to_string(root.join("crates/t3claw_wasm_product_adapters/src/store.rs"))
            .expect("ProductAdapter WASM store must be readable");
    let adapter_runtime = std::fs::read_to_string(
        root.join("crates/t3claw_wasm_product_adapters/src/component_runtime.rs"),
    )
...
fn wasm_product_adapter_wit_preserves_product_adapter_trust_boundary() {
    let wit = std::fs::read_to_string(
        workspace_root().join("crates/t3claw_wasm_product_adapters/wit/product_adapter.wit"),
    )
...
fn wasm_product_adapter_wit_pins_json_shim_shape() {
    ...
    // covering a typed Rust DTO in `t3claw_product_adapters`.
    let shim_fields = [
...
fn reborn_runtime_http_egress_has_single_network_boundary() {
    let forbidden = [
        ForbiddenRuntimeNetworkUse {
            ...
            reason: "runtime crates must use t3claw_network for outbound HTTP transport",
        },
        ForbiddenRuntimeNetworkUse {
    ...
    let runtime_src_roots = [
        "crates/t3claw_wasm/src",
        "crates/t3claw_scripts/src",
        "crates/t3claw_mcp/src",
        "crates/t3claw_host_runtime/src",
    ];
    ...
    assert!(
        violations.is_empty(),
        "Reborn runtime HTTP must use the shared host egress service and t3claw_network only:\n{}",
        violations.join("\n")
    );
}
...
fn reborn_product_api_crates_do_not_bind_http_ingress() {
    ...
    let reborn_product_api_src_roots = [
        "crates/t3claw_reborn/src",
        "crates/t3claw_reborn_cli/src",
        "crates/t3claw_reborn_composition/src",
        "crates/t3claw_reborn_config/src",
        "crates/t3claw_reborn_event_store/src",
        "crates/t3claw_reborn_api/src",
        "crates/t3claw_reborn_openai_compat/src",
        "crates/t3claw_product_adapters/src",
        "crates/t3claw_product_adapter_registry/src",
        "crates/t3claw_product_workflow/src",
        "crates/t3claw_wasm_product_adapters/src",
        "crates/t3claw_telegram_v2_adapter/src",
        "crates/t3claw_slack_v2_adapter/src",
        "crates/t3claw_outbound/src",
        "crates/t3claw_conversations/src",
        "crates/t3claw_turns/src",
        "crates/t3claw_threads/src",
        "crates/t3claw_loop_support/src",
...
fn reborn_openai_compat_routes_do_not_depend_on_v1_gateway_or_legacy_streams() {
    ...
    let root = workspace_root();
    let compat_src = root.join("crates/t3claw_reborn_openai_compat/src");
    let mut violations = Vec::new();
...
fn reborn_product_auth_contract_stays_reborn_native() {
    ...
    let root = workspace_root();
    let manifest = std::fs::read_to_string(root.join("crates/t3claw_auth/Cargo.toml"))
        .expect("t3claw_auth manifest must be readable");
    assert!(
        !manifest.contains("reqwest"),
        "t3claw_auth must not depend on reqwest directly; provider transport belongs behind Reborn-native composition"
    );

    let auth_src = root.join("crates/t3claw_auth/src");
    assert!(
        auth_src.exists(),
    ...
    collect_forbidden_reborn_auth_file_uses(
        &root.join("crates/t3claw_reborn_composition/src/auth.rs"),
        &root,
        &forbidden,
        &mut violations,
    );
    collect_forbidden_reborn_auth_path_uses(
        &root.join("crates/t3claw_reborn_composition/src/product_auth_serve"),
        &root.join("crates/t3claw_reborn_composition/src/product_auth_serve.rs"),
        &root,
        &forbidden,
        &mut violations,
    );
...
fn boundary_rules() -> Vec<BoundaryRule> {
    vec![
        BoundaryRule {
            crate_name: "t3claw_product_workflow",
            forbidden: vec![
                "t3claw_dispatcher",
                "t3claw_extensions",
                "t3claw_host_runtime",
        ...
        BoundaryRule {
            ...
            forbidden: vec![
                ...
                "t3claw_processes",
                "t3claw_product_adapters",
                "t3claw_product_adapter_registry",
        ...
        BoundaryRule {
            ...
            forbidden: vec![
                ...
                // `t3claw_product_workflow` facade plus the ingress + error
        ...
        BoundaryRule {
            ...
            crate_name: "t3claw_reborn_openai_compat",
            forbidden: vec![
                "t3claw",
                "t3claw_capabilities",
                "t3claw_conversations",
        ...
        BoundaryRule {
            ...
            crate_name: "t3claw_reborn_openai_compat_storage",
            forbidden: vec![
                "t3claw",
                "t3claw_capabilities",
                "t3claw_conversations",
        ...
        BoundaryRule {
            ...
            crate_name: "t3claw_product_adapter_registry",
            forbidden: vec![
                "t3claw",
                "t3claw_authorization",
                "t3claw_approvals",
        ...
        BoundaryRule {
            ...
            // runtime handles.
            crate_name: "t3claw_first_party_extensions",
            forbidden: vec![
                "t3claw",
                "t3claw_approvals",
                "t3claw_authorization",
        ...
        BoundaryRule {
            ...
            // product composition.
            crate_name: "t3claw_first_party_extension_ports",
            forbidden: vec![
                "t3claw",
                "t3claw_approvals",
                "t3claw_authorization",
        ...
        BoundaryRule {
            crate_name: "t3claw_reborn_config",
            forbidden: vec![
                "t3claw",
                "t3claw_approvals",
                "t3claw_authorization",
        ...
        BoundaryRule {
            ...
            // `t3claw_reborn_composition` facades. Adding any of the
        ...
        BoundaryRule {
            ...
            // Reaches Reborn through t3claw_reborn_composition's facade
            // only (Router + WebuiAuthenticator trait + WebuiServeConfig).
            crate_name: "t3claw_reborn_webui_ingress",
            forbidden: vec![
                "t3claw",
                "t3claw_authorization",
                "t3claw_capabilities",
        ...
        BoundaryRule {
            ...
            forbidden: vec![
                ...
                "t3claw_events",
                "t3claw_extensions",
                // t3claw_filesystem is permitted: FilesystemResourceGovernorStore
        ...
        BoundaryRule {
            ...
            // `t3claw_turns` is intentionally allowed here for
            ...
            // pending-gate types.
            crate_name: "t3claw_event_projections",
            forbidden: vec![
                "t3claw",
                "t3claw_authorization",
                "t3claw_approvals",
        ...
        BoundaryRule {
            crate_name: "t3claw_event_streams",
            forbidden: vec![
                "t3claw",
                "t3claw_authorization",
                "t3claw_approvals",
        ...
        BoundaryRule {
            ...
            // the adapter crate.
            crate_name: "t3claw_slack_v2_adapter",
            forbidden: vec![
                "t3claw",
                "t3claw_authorization",
                "t3claw_approvals",
        ...
        BoundaryRule {
            ...
            forbidden: vec![
                ...
                "t3claw_dispatcher",
                "t3claw_extensions",
                // t3claw_filesystem is permitted: FilesystemOutboundStateStore
        ...
        BoundaryRule {
            crate_name: "t3claw_wasm_product_adapters",
            forbidden: vec![
                "t3claw",
                "t3claw_authorization",
                "t3claw_approvals",
        ...
        BoundaryRule {
            crate_name: "t3claw_reborn_event_store",
            // t3claw_filesystem is permitted: FilesystemEventLog routes the
        ...
        BoundaryRule {
            ...
            forbidden: vec![
                ...
                "t3claw_events",
                "t3claw_extensions",
                // t3claw_filesystem is permitted: FilesystemSecretStore /
        ...
        BoundaryRule {
            crate_name: "t3claw_authorization",
            forbidden: vec![
                "t3claw_approvals",
                "t3claw_capabilities",
                "t3claw_dispatcher",
        ...
        BoundaryRule {
            ...
            forbidden: vec![
                ...
                "t3claw_events",
                "t3claw_extensions",
                // t3claw_filesystem is permitted: FilesystemSessionThreadService
                ...
                "t3claw_resources",
                "t3claw_run_state",
                // t3claw_safety is permitted: thread/transcript storage
        ...
        BoundaryRule {
            ...
            forbidden: vec![
                ...
                "t3claw_dispatcher",
                "t3claw_extensions",
                // t3claw_filesystem is permitted: FilesystemTurnStateStore
        ...
        // The hooks framework depends on `t3claw_turns` and host primitives
        ...
        // contracts in `t3claw_turns`. It must not import host runtime crates,
        ...
        BoundaryRule {
            crate_name: "t3claw_capabilities",
            forbidden: vec![
                "t3claw_dispatcher",
                "t3claw_host_runtime",
                "t3claw_secrets",
...
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("architecture crate must live under crates/t3claw_architecture")
        .to_path_buf()
}
...
fn workspace_dependency_names(package: &Value) -> impl Iterator<Item = &Value> {
    package["dependencies"]
        ...
        .filter(|dependency| {
            dependency["name"]
                .as_str()
                .is_some_and(|name| name == "t3claw" || name.starts_with("t3claw_"))
        })
}
...
fn workspace_t3claw_crates(dependencies: &HashMap<String, Vec<String>>) -> Vec<&str> {
    dependencies
        .keys()
        .filter_map(|name| {
            (name == "t3claw" || name.starts_with("t3claw_")).then_some(name.as_str())
        })
        .collect()
}
...
fn assert_no_normal_workspace_deps<'a>(
    ...
    for forbidden in forbidden {
        assert!(
            ...
            "{crate_name} must not have a normal dependency on {forbidden}; actual normal t3claw deps: {actual:?}"
        );
    }
}
...
fn is_reborn_tracing_target_line(line: &str) -> bool {
    line.contains("target: \"t3claw::reborn::") || line.contains("target = \"t3claw::reborn::")
}

#[test]
fn collect_forbidden_reborn_auth_file_uses_detects_violation() {
    let root = std::env::temp_dir().join(format!(
        "t3claw-reborn-auth-boundary-test-{}",
        std::process::id()
    ));
    let src = root.join("crates/t3claw_reborn_composition/src");
    ...
    assert!(
        violations[0].contains("crates/t3claw_reborn_composition/src/auth.rs"),
...
fn collect_forbidden_reborn_auth_file_uses_allows_reborn_tracing_targets() {
    let root = std::env::temp_dir().join(format!(
        "t3claw-reborn-auth-boundary-tracing-test-{}",
        std::process::id()
    ));
    let src = root.join("crates/t3claw_reborn_composition/src");
    std::fs::create_dir_all(&src).expect("test source directory must be created");
    ...
    std::fs::write(
        &auth_rs,
        "fn allowed() { tracing::warn!(target: \"t3claw::reborn::product_auth::oauth\"); }\n",
    )
    .expect("test auth.rs must be written");
...
fn collect_forbidden_uses_allows_reborn_tracing_targets() {
    let root = std::env::temp_dir().join(format!(
        "t3claw-reborn-auth-boundary-dir-tracing-test-{}",
        std::process::id()
    ));
    let src = root.join("crates/t3claw_reborn_composition/src/product_auth_serve");
    std::fs::create_dir_all(&src).expect("test source directory must be created");
