...
`t3claw_product_workflow::RebornServicesApi`. Handlers are the only
...
`crates/t3claw_product_workflow/tests/reborn_services_contract.rs`.
...
`t3claw_dispatcher`, `t3claw_extensions`, `t3claw_host_runtime`,
`t3claw_mcp`, `t3claw_wasm`, `t3claw_scripts`, `t3claw_network`,
`t3claw_engine`, `t3claw_gateway`, `t3claw_run_state`,
`t3claw_capabilities`, or any DB/storage crate. The architecture
...
```bash
cargo test -p t3claw_webui_v2 --features webui-v2-beta
cargo clippy -p t3claw_webui_v2 --all-features --tests -- -D warnings
