...
T3Claw build does **not** need `libclang`.
...
and produces `target/release/t3claw-silk-decoder` (relative to the
crate). It is excluded from the top-level workspace so it does not
participate in `cargo build --workspace`.

## Install

The T3Claw host looks for the binary in this order:

1. `T3CLAW_SILK_DECODER` environment variable (a path to the binary)
2. A sibling of the running `t3claw` executable named
   `t3claw-silk-decoder` (with `.exe` on Windows)
3. `t3claw-silk-decoder` on `$PATH`
