...
- This crate is excluded from the workspace so the main T3Claw build does not require libclang.

## What This Crate Owns

- Standalone helper binary `t3claw-silk-decoder`.
- WeChat raw SILK v3 voice-note decoding to WAV.
- CLI/protocol behavior for the helper process.

## Do Not Move In Here

- Main workspace dependencies or root T3Claw runtime behavior.
- Channel/product workflow logic beyond the decoder protocol.
- Secrets, network access, database access, or agent state.

## Validation

- Build/check from repo root: `cargo build --manifest-path crates/t3claw_silk_decoder/Cargo.toml`
