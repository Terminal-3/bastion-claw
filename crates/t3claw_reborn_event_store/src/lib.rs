...
//! substrate. `t3claw_events` owns the durable log traits and redacted record
...
//! an `invocation_id`, but `ReadScope` (defined in `t3claw_events`) does
...
//! invocation boundary. Adding it requires changes to `t3claw_events`,
...
use t3claw_host_api::{AgentId, AuditEnvelope};
#[cfg(any(feature = "libsql", feature = "postgres"))]
...
pub enum RebornEventStoreConfig {
    ...
    /// [`PostgresRootFilesystem`](t3claw_filesystem::PostgresRootFilesystem)
    ...
    /// [`LibSqlRootFilesystem`](t3claw_filesystem::LibSqlRootFilesystem)
    ...
    mod tests {
        ...
        fn local_postgres_urls_are_recognised() {
            for url in [
                ...
                "host=/var/run/postgresql user=t3claw dbname=t3claw",
            ] {
        ...
        fn libpq_keyword_strings_to_remote_hosts_require_tls() {
            ...
            for url in [
                "host=db.example.com user=event_user dbname=t3claw",
                "host=10.0.0.5 port=5432 user=t3claw",
                "user=t3claw host=managed-pg.internal",
            ] {
                assert!(
                    !is_local(url),
        ...
        fn libpq_keyword_strings_without_host_or_with_socket_path_are_local() {
            for url in [
                ...
                "user=t3claw dbname=t3claw",
                // Socket directory.
                "host=/var/run/postgresql user=t3claw dbname=t3claw",
                // Localhost literal.
                "host=localhost user=t3claw",
                "host=127.0.0.1 user=t3claw",
            ] {
                assert!(is_local(url), "expected `{url}` to be detected as local");
            }
        }
        ...
        fn libpq_hostaddr_to_remote_address_requires_tls() {
            ...
            assert!(!is_local("hostaddr=10.0.0.5 user=t3claw"));
            assert!(!is_local("hostaddr=2001:db8::1 user=t3claw"));
        }

        #[test]
        fn libpq_hostaddr_to_loopback_is_local() {
            assert!(is_local("hostaddr=127.0.0.1 user=t3claw"));
            assert!(is_local("hostaddr=::1 user=t3claw"));
        }
        ...
        fn libpq_mixed_socket_and_remote_host_list_requires_tls() {
            ...
            assert!(!is_local(
                "host=/var/run/postgresql,db.example.com user=t3claw"
            ));
        }
        ...
        fn url_with_empty_authority_and_query_host_uses_query_host() {
            ...
            assert!(!is_local(
                "postgresql:///db?host=db.example.com&user=t3claw"
            ));
        }
        ...
        fn libpq_quoted_socket_path_is_local() {
            ...
            let url = "host='/var/run/postgresql' user=t3claw dbname=t3claw";
            assert!(
                is_local(url),
...
struct TrustedRuntimeJsonlEntry {
    cursor: EventCursor,
    #[serde(deserialize_with = "t3claw_events::deserialize_trusted_runtime_event")]
    record: RuntimeEvent,
}
...
/// the function returns. Cross-process safe: two T3Claw processes that race
