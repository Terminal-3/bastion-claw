...
use t3claw_reborn_event_store::{RebornEventStoreConfig, RebornProfile, build_reborn_event_stores};
...
async fn postgres_replay_advances_next_cursor_past_trailing_filtered_records() {
    let Ok(url) = std::env::var("T3CLAW_REBORN_EVENT_STORE_POSTGRES_URL") else {
        eprintln!(
            "skipping postgres event-store cursor contract: T3CLAW_REBORN_EVENT_STORE_POSTGRES_URL not set"
        );
        return;
    };
...
async fn postgres_runtime_and_audit_logs_survive_rebuild_with_filtered_cursor_semantics() {
    let Ok(url) = std::env::var("T3CLAW_REBORN_EVENT_STORE_POSTGRES_URL") else {
        eprintln!(
            "skipping postgres event-store contract: T3CLAW_REBORN_EVENT_STORE_POSTGRES_URL not set"
        );
        return;
    };
    let suffix = std::time::SystemTime::now()
