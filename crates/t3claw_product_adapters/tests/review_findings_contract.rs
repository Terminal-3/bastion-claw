...
use t3claw_turns::{AcceptedMessageRef, ReplyTargetBindingRef, TurnRunId};
...
fn failed_auth_evidence_cannot_create_trusted_context() {
    let evidence =
        ProtocolAuthEvidence::failed(t3claw_product_adapters::ProtocolAuthFailure::Missing);
    assert!(
        TrustedInboundContext::from_verified_evidence(
            adapter_id(),
            installation_id(),
...
fn command_payload_is_bounded_and_serde_validated() {
    ...
    assert!(
        serde_json::from_value::<t3claw_product_adapters::ApprovalResolutionPayload>(
            oversized_gate
        )
        .is_err()
    );
    ...
    assert!(
        serde_json::from_value::<t3claw_product_adapters::ApprovalResolutionPayload>(newline_gate)
            .is_err()
    );
    ...
    assert!(
        serde_json::from_value::<t3claw_product_adapters::AuthResolutionPayload>(oversized_auth)
            .is_err()
    );
...
fn ack_and_rejection_types_are_unambiguous_and_typed() {
    ...
    assert_eq!(
        ...
        t3claw_product_adapters::InboundRetryDisposition::DoNotRetry
    );
    ...
    assert!(t3claw_product_adapters::fakes::ensure_durable_outcome(
        &ProductInboundAck::Accepted {
            accepted_message_ref: AcceptedMessageRef::new("msg:2").expect("valid"),
            submitted_run_id: TurnRunId::new(),
        }
    ));
    assert!(!t3claw_product_adapters::fakes::ensure_noop_outcome(
        &ProductInboundAck::Accepted {
...
fn transient_egress_errors_are_retryable_adapter_errors() {
    let timeout: t3claw_product_adapters::ProductAdapterError =
