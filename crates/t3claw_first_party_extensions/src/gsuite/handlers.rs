...
pub enum GsuiteCredentialDispatchReason {
    ...
    MissingAccessSecret,
    AuthRequired {
        required_secrets: Vec<t3claw_host_api::SecretHandle>,
    },
    BackendAuth,
    HostApi,
}
...
impl GsuiteDispatchError {
    ...
    pub fn auth_requirement(&self) -> Option<Vec<t3claw_host_api::SecretHandle>> {
...
pub struct GsuiteCredentialStageRequest<'a> {
    ...
    pub capability_id: &'a CapabilityId,
    pub access_secret: &'a t3claw_host_api::SecretHandle,
}

/// Alias for [`t3claw_host_api::CredentialStageError`].
///
/// The shared type lives in `t3claw_host_api` so that both the GSuite staging
/// trait and the host-runtime staging layer use the same type without a
/// cross-crate conversion step.
pub type GsuiteCredentialStageError = t3claw_host_api::CredentialStageError;
...
enum CapabilityExecutionOutcome {
    Response {
        response: t3claw_host_api::RuntimeHttpEgressResponse,
...
) -> Result<t3claw_host_api::RuntimeHttpEgressResponse, GsuiteDispatchError> {
...
fn is_google_auth_expired_response(response: &t3claw_host_api::RuntimeHttpEgressResponse) -> bool {
    response.status == 401
}
...
fn runtime_request(
    request: &GsuiteDispatchRequest<'_>,
    access_secret: t3claw_host_api::SecretHandle,
    method: NetworkMethod,
    url: String,
    body: Vec<u8>,
...
fn map_stage_error(
    ...
    required_secret: t3claw_host_api::SecretHandle,
) -> GsuiteDispatchError {
    match error {
...
#[cfg(test)]
mod tests {
    use t3claw_host_api::{HostApiError, RuntimeHttpEgressResponse};
    ...
    fn map_credential_error_tests() {
        assert_eq!(
            map_credential_error(GoogleCredentialError::Recovery(
                t3claw_auth::CredentialRecoveryProjection::setup_required(
                    t3claw_auth::AuthProviderId::new("google").unwrap(),
                    t3claw_auth::CredentialRecoveryReason::NoAccount,
                    Vec::new(),
                ),
            ))
            .kind(),
        ...
        assert_eq!(
            map_credential_error(GoogleCredentialError::Recovery(
                t3claw_auth::CredentialRecoveryProjection::account_selection_required(
                    t3claw_auth::AuthProviderId::new("google").unwrap(),
                    Vec::new(),
                ),
            ))
        ...
        let backend_error = map_credential_error(GoogleCredentialError::Auth(
            t3claw_auth::AuthProductError::BackendUnavailable,
        ));
        ...
        let configured_recovery = map_credential_error(GoogleCredentialError::Recovery(
            t3claw_auth::CredentialRecoveryProjection::configured(
                t3claw_auth::AuthProviderId::new("google").unwrap(),
                t3claw_auth::CredentialAccountProjection {
                    id: t3claw_auth::CredentialAccountId::new(),
                    provider: t3claw_auth::AuthProviderId::new("google").unwrap(),
                    label: t3claw_auth::CredentialAccountLabel::new("Google").unwrap(),
                    status: t3claw_auth::CredentialAccountStatus::Configured,
                    ownership: t3claw_auth::CredentialOwnership::UserReusable,
    ...
    fn auth_requirement_classifies_each_credential_dispatch_reason() {
        ...
        let configured_proj = t3claw_auth::CredentialRecoveryProjection::configured(
            t3claw_auth::AuthProviderId::new("google").unwrap(),
            t3claw_auth::CredentialAccountProjection {
                id: t3claw_auth::CredentialAccountId::new(),
                provider: t3claw_auth::AuthProviderId::new("google").unwrap(),
        ...
        let handle = t3claw_host_api::SecretHandle::new("google-access-token").unwrap();
    ...
    fn map_egress_error_tests() {
        let cases = [
            ...
            (
                RuntimeHttpEgressError::Network {
                    reason: t3claw_host_api::RUNTIME_HTTP_REASON_RESPONSE_BODY_LIMIT_EXCEEDED
                        .to_string(),
                    request_bytes: 15,
                    response_bytes: 1024,
                },
