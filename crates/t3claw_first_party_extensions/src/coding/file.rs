...
use t3claw_filesystem::{FileType, FilesystemOperation};
use t3claw_host_api::RuntimeDispatchErrorKind;
...
fn filesystem_error_with_summary(
    operation: &str,
    scoped_path: &str,
    error: t3claw_filesystem::FilesystemError,
) -> CodingCapabilityError {
    let scoped_path = safe_summary_path(scoped_path);
    let summary = match &error {
        t3claw_filesystem::FilesystemError::NotFound { .. } => {
            format!("{operation} failed for {scoped_path}: file not found")
        }
        t3claw_filesystem::FilesystemError::PermissionDenied { .. }
        | t3claw_filesystem::FilesystemError::MountNotFound { .. }
        | t3claw_filesystem::FilesystemError::PathOutsideMount { .. }
        | t3claw_filesystem::FilesystemError::SymlinkEscape { .. }
        | t3claw_filesystem::FilesystemError::MountConflict { .. }
        | t3claw_filesystem::FilesystemError::VersionMismatch { .. }
        | t3claw_filesystem::FilesystemError::Unsupported { .. }
        | t3claw_filesystem::FilesystemError::IndexConflict { .. } => {
            format!("{operation} failed for {scoped_path}: permission denied or unsupported path")
        }
        t3claw_filesystem::FilesystemError::Backend { .. }
        | t3claw_filesystem::FilesystemError::BackendInfrastructure { .. } => {
            format!("{operation} failed for {scoped_path}: filesystem backend error")
        }
        t3claw_filesystem::FilesystemError::Contract(_) => {
...
async fn existing_text_for_preview(
    ...
    stat: Option<&t3claw_filesystem::FileStat>,
) -> Option<String> {
