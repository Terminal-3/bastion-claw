...
fn mcp_initialize_params() -> Value {
    json!({
        ...
        "clientInfo": {"name": "t3claw", "version": env!("CARGO_PKG_VERSION")}
    })
}
...
async fn execute_runtime_http(
    ...
    egress: Arc<dyn RuntimeHttpEgress>,
) -> Result<t3claw_host_api::RuntimeHttpEgressResponse, RuntimeHttpEgressError> {
...
mod tests {
    ...
    use t3claw_host_api::{InvocationId, RuntimeHttpEgressResponse, UserId};
