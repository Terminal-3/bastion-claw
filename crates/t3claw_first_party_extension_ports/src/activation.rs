...
use t3claw_turns::run_profile::{LoopRunContext, SkillVisibility};
use t3claw_turns::{AcceptedMessageRef, TurnRunId, TurnScope};
use thiserror::Error;
...
struct ActivationCandidateCacheKey {
    ...
    content_hash: String,
    trust: Option<t3claw_skills::SkillTrust>,
...
mod tests {
    use super::*;
    use t3claw_host_api::{AgentId, ProjectId, TenantId};
    use t3claw_loop_support::{SkillBundleId, SkillFilePath};
    ...
    impl ChangingSkillBundleSource {
        fn new(name: &str, first: String, second: String) -> Self {
            ...
            .with_provenance(
                t3claw_loop_support::SkillBundleProvenance::new(SkillSourceKind::User)
    ...
    async fn run_context_for(thread_id: &str, accepted_message: &str) -> LoopRunContext {
        ...
        LoopRunContext::new(
            TurnScope::new(
                ...
                t3claw_host_api::ThreadId::new(thread_id).unwrap(),
            ),
            TurnId::new(),
            TurnRunId::new(),
            resolved,
        ...
        .with_actor(TurnActor::new(
            t3claw_host_api::UserId::new("user-a").unwrap(),
        ))
    }
    ...
    fn explicit_name_extraction_matches_valid_dotted_skill_names() {
        ...
        assert!(t3claw_skills::validate_skill_name("skill.v2"));
    }
}
