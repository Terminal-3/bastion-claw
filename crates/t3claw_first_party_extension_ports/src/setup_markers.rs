...
use t3claw_filesystem::{FilesystemError, RootFilesystem, ScopedFilesystem};
...
mod tests {
    ...
    use t3claw_host_api::{AgentId, MountView, ProjectId, TenantId};
    use t3claw_turns::{
    ...
    async fn run_context() -> LoopRunContext {
        ...
        LoopRunContext::new(
            TurnScope::new(
                ...
                t3claw_host_api::ThreadId::new("thread-a").expect("valid thread"),
            ),
            TurnId::new(),
            TurnRunId::new(),
            resolved,
        ...
        .with_actor(TurnActor::new(
            t3claw_host_api::UserId::new("user-a").expect("valid user"),
        ))
    }
