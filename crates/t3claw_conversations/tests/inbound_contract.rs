...
async fn first_bind_does_not_trust_unvalidated_requested_scope_hints() {
    ...
    let resolution = services
        .resolve_or_create_binding(t3claw_conversations::ResolveConversationRequest {
            tenant_id: tenant(),
            adapter_kind: web(),
            adapter_installation_id: default_installation(),
...
impl ConversationBindingService for DriftBindingService {
    ...
    async fn lookup_binding(
        &self,
        _request: t3claw_conversations::ResolveConversationRequest,
    ) -> Result<ConversationBindingResolution, InboundTurnError> {
...
struct UntrustedOnlyBindingService {
    ...
    resolve_requests: Arc<Mutex<Vec<t3claw_conversations::ResolveConversationRequest>>>,
...
impl UntrustedOnlyBindingService {
    ...
    fn resolve_requests(&self) -> Vec<t3claw_conversations::ResolveConversationRequest> {
...
impl TurnCoordinator for CapacityFailureTurnCoordinator {
    ...
    async fn submit_turn(
        ...
        Err(TurnError::capacity_exceeded(
            t3claw_turns::TurnCapacityResource::SubmitTurn,
            1,
        ))
    }
    ...
    async fn submit_turn(
        ...
        if submissions.len() == 1 {
            return Err(TurnError::ThreadBusy(ThreadBusy {
                ...
                event_cursor: t3claw_turns::events::EventCursor(1),
            }));
        }
