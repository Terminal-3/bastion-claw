# t3claw_capabilities guardrails

- Own caller-facing `CapabilityHost` invoke/resume/spawn workflow.
- Use the neutral `CapabilityDispatcher` port; do not add a normal dependency on concrete `t3claw_dispatcher` or runtime crates.
...
- Do not absorb process lifecycle/result APIs; those belong in `t3claw_processes::ProcessHost`.
