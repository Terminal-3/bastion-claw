...
- Fast local check: `cargo test -p t3claw_triggers`
- Backend check: `cargo test -p t3claw_triggers --features libsql`
- Lint check: `cargo clippy -p t3claw_triggers --all-targets --all-features -- -D warnings`
- Boundary check after dependency changes: `cargo test -p t3claw_architecture reborn_crate_dependency_boundaries_hold`
