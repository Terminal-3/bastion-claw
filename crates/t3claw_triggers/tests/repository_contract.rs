...
#[cfg(feature = "libsql")]
use {libsql::params, std::sync::Arc, t3claw_triggers::LibSqlTriggerRepository, tempfile::tempdir};

#[cfg(feature = "postgres")]
use t3claw_triggers::PostgresTriggerRepository;
...
async fn postgres_pool_or_skip() -> Option<(
    ...
    if std::env::var("T3CLAW_SKIP_POSTGRES_TESTS").is_ok() {
        eprintln!("skipping Postgres trigger repository tests: T3CLAW_SKIP_POSTGRES_TESTS is set");
        return None;
    }
