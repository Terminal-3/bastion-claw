...
Product-facing workflow facade for T3Claw Reborn (issue #3280).
...
| `ProductConversationBindingService` | Adapter from product workflow bindings to `t3claw_conversations` with trusted installation→tenant mapping |
...
- `t3claw_product_adapters` — trait definitions, envelope/ack types, `ProjectionStream` for SSE
- `t3claw_approvals` / `t3claw_authorization` — canonical approval resolution and scoped lease issue ports used by approval interactions
- `t3claw_auth` — typed product-auth continuation events consumed by the workflow auth bridge
- `t3claw_conversations` — canonical actor/conversation binding and thread route ownership
- `t3claw_run_state` — approval request store contract surfaced through approval resolution/read-model ports
- `t3claw_turns` — turn coordinator, scope, IDs
- `t3claw_threads` — session thread service contract
- `t3claw_host_api` — canonical identifiers (TenantId, UserId, etc.)

## Boundary rules

Must NOT depend on: `t3claw_dispatcher`, `t3claw_extensions`,
`t3claw_host_runtime`, `t3claw_mcp`, `t3claw_wasm`, `t3claw_scripts`,
`t3claw_network`, `t3claw_engine`, `t3claw_gateway`.
...
Outbound delivery orchestration starts only after `t3claw_outbound` resolves
