...
//!
//! This module consumes the `t3claw_auth` continuation vocabulary and routes
...
use t3claw_auth::{AuthContinuationEvent, AuthContinuationRef, AuthProductError};
use t3claw_turns::{
...
fn auth_continuation_binding_id(
    flow_id: t3claw_auth::AuthFlowId,
    run_id: &TurnRunId,
    gate_ref: &str,
) -> String {
    format!(
...
mod tests {
    ...
    fn workflow_error_kind_capacity_exceeded_returns_expected_strings() {
        ...
        let resume = ProductWorkflowError::TurnResumeDenied {
            error: TurnError::capacity_exceeded(t3claw_turns::TurnCapacityResource::SubmitTurn, 7),
        };
        assert_eq!(
