...
use t3claw_host_api::{AgentId, ExtensionId, ProjectId, TenantId, ThreadId, UserId};
...
impl RebornServices {
    async fn list_visible_threads_for_scope(
        ...
        while visible_threads.len() < visible_limit {
            ...
            let response = self
                .thread_service
                .list_threads_for_scope(t3claw_threads::ListThreadsForScopeRequest {
                    scope: scope.clone(),
...
impl RebornServices {
    ...
    async fn gate_resolution_route(
        ...
        {
            Ok(state) => state,
            Err(error) if error.category() == t3claw_turns::TurnErrorCategory::ScopeNotFound => {
    ...
    async fn resolve_generic_gate(
        ...
        match resolution {
            ...
            WebUiGateResolution::Denied | WebUiGateResolution::Cancelled => {
                ...
                let response = self
                    .turn_coordinator
                    .cancel_run(t3claw_turns::CancelRunRequest {
                        scope,
                        actor,
                        run_id,
...
fn thread_scope_from_turn_scope(
    scope: &TurnScope,
    owner_user_id: Option<t3claw_host_api::UserId>,
...
fn webui_source_binding_ref_from_raw(
    prefix: &str,
    raw: &str,
) -> Result<t3claw_turns::SourceBindingRef, RebornServicesError> {
...
fn webui_reply_target_binding_ref_from_raw(
    prefix: &str,
    raw: &str,
) -> Result<t3claw_turns::ReplyTargetBindingRef, RebornServicesError> {
...
fn paginate_timeline_messages(
    mut messages: Vec<t3claw_threads::ThreadMessageRecord>,
    limit: usize,
    cursor: Option<TimelineCursor>,
) -> (Vec<t3claw_threads::ThreadMessageRecord>, Option<String>) {
...
fn cap_summary_artifacts(
    mut artifacts: Vec<t3claw_threads::SummaryArtifact>,
) -> Vec<t3claw_threads::SummaryArtifact> {
...
fn gate_ref_string(gate_ref: &t3claw_turns::GateRef) -> String {
...
fn map_turn_error(error: TurnError) -> RebornServicesError {
    let (code, kind, status_code, retryable) = match error.category() {
        t3claw_turns::TurnErrorCategory::ThreadBusy => (
            RebornServicesErrorCode::Conflict,
            RebornServicesErrorKind::Busy,
            409,
            false,
        ),
        t3claw_turns::TurnErrorCategory::Conflict => (
            RebornServicesErrorCode::Conflict,
            RebornServicesErrorKind::Conflict,
            409,
            false,
        ),
        t3claw_turns::TurnErrorCategory::AdmissionRejected => (
            RebornServicesErrorCode::RateLimited,
            RebornServicesErrorKind::Busy,
            429,
            true,
        ),
        t3claw_turns::TurnErrorCategory::CapacityExceeded => (
            RebornServicesErrorCode::RateLimited,
            RebornServicesErrorKind::Busy,
            429,
            false,
        ),
        t3claw_turns::TurnErrorCategory::ScopeNotFound => (
            RebornServicesErrorCode::NotFound,
            RebornServicesErrorKind::NotFound,
            404,
            false,
        ),
        t3claw_turns::TurnErrorCategory::Unauthorized => (
            RebornServicesErrorCode::Forbidden,
            RebornServicesErrorKind::ParticipantDenied,
            403,
            false,
        ),
        t3claw_turns::TurnErrorCategory::InvalidRequest => (
            RebornServicesErrorCode::InvalidRequest,
            RebornServicesErrorKind::Validation,
            400,
            false,
        ),
        t3claw_turns::TurnErrorCategory::Unavailable => (
            RebornServicesErrorCode::Unavailable,
...
fn create_thread_metadata_json(
    client_action_id: &t3claw_turns::IdempotencyKey,
    ...
    let seed = format!(
        ...
        segment(
            ...
            caller
                .project_id
                .as_ref()
                .map(t3claw_host_api::ProjectId::as_str)
                .unwrap_or("")
        ),
