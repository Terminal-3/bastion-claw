//! Adapter from product workflow binding requests to `t3claw_conversations`.
...
pub enum ProductActorBindingPolicy {
    ...
    ResolveActor {
        ...
        actor_pairings: Arc<dyn t3claw_conversations::ConversationActorPairingService>,
    },
}
...
pub struct ProductConversationBindingService {
    conversations: Arc<dyn t3claw_conversations::ConversationBindingService>,
...
fn ensure_resolved_actor_matches_expected_user(
    ...
    resolution: &t3claw_conversations::ConversationBindingResolution,
) -> Result<(), ProductWorkflowError> {
    ...
    async fn resolve_binding(
                ...
                .is_some()
        {
                ...
                .await
            {
                ...
                Ok(_) | Err(t3claw_conversations::InboundTurnError::BindingRequired { .. }) => {}
...
fn conversation_request(
    request: &ResolveBindingRequest,
    tenant_id: TenantId,
) -> Result<t3claw_conversations::ResolveConversationRequest, ProductWorkflowError> {
    Ok(t3claw_conversations::ResolveConversationRequest {
        tenant_id,
...
fn conversation_adapter_kind(
    adapter_id: &ProductAdapterId,
) -> Result<t3claw_conversations::AdapterKind, ProductWorkflowError> {
    t3claw_conversations::AdapterKind::new(adapter_id.as_str()).map_err(map_conversation_error)
}

fn conversation_installation_id(
    installation_id: &AdapterInstallationId,
) -> Result<t3claw_conversations::AdapterInstallationId, ProductWorkflowError> {
    t3claw_conversations::AdapterInstallationId::new(installation_id.as_str())
        .map_err(map_conversation_error)
}

fn conversation_event_id(
    event_id: &t3claw_product_adapters::ExternalEventId,
) -> Result<t3claw_conversations::ExternalEventId, ProductWorkflowError> {
    t3claw_conversations::ExternalEventId::new(event_id.as_str()).map_err(map_conversation_error)
}

fn conversation_actor_ref(
    actor_ref: &t3claw_product_adapters::ExternalActorRef,
) -> Result<t3claw_conversations::ExternalActorRef, ProductWorkflowError> {
    t3claw_conversations::ExternalActorRef::new(actor_ref.kind(), actor_ref.id())
        .map_err(map_conversation_error)
}

fn conversation_conversation_ref(
    conversation_ref: &t3claw_product_adapters::ExternalConversationRef,
) -> Result<t3claw_conversations::ExternalConversationRef, ProductWorkflowError> {
    t3claw_conversations::ExternalConversationRef::new(
...
fn conversation_route_kind(
    route_kind: ProductConversationRouteKind,
) -> t3claw_conversations::ConversationRouteKind {
    match route_kind {
        ProductConversationRouteKind::Direct => t3claw_conversations::ConversationRouteKind::Direct,
        ProductConversationRouteKind::Shared => t3claw_conversations::ConversationRouteKind::Shared,
    }
}

fn map_conversation_error(error: t3claw_conversations::InboundTurnError) -> ProductWorkflowError {
    match error {
        t3claw_conversations::InboundTurnError::InvalidExternalRef { reason, .. }
        | t3claw_conversations::InboundTurnError::InvalidCanonicalRef { reason } => {
            ProductWorkflowError::InvalidBindingRequest { reason }
        }
        t3claw_conversations::InboundTurnError::BindingRequired { .. } => {
            ProductWorkflowError::BindingRequired {
                reason: "external actor is not paired with a canonical user".into(),
            }
        }
        t3claw_conversations::InboundTurnError::AccessDenied { .. }
        | t3claw_conversations::InboundTurnError::BindingConflict { .. }
        | t3claw_conversations::InboundTurnError::ThreadNotFound { .. } => {
            ProductWorkflowError::BindingAccessDenied
        }
        t3claw_conversations::InboundTurnError::StatePoisoned
        | t3claw_conversations::InboundTurnError::DurableState { .. } => {
            ProductWorkflowError::Transient {
                reason: "conversation binding store unavailable".into(),
            }
        }
        t3claw_conversations::InboundTurnError::TurnSubmissionFailed { error } => {
