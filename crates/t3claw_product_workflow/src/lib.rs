//! Product-facing workflow facade for T3Claw Reborn.
//!
//! `t3claw_product_workflow` sits between product adapters and host-layer
...
//!   [`t3claw_product_adapters::ProductWorkflow`].
...
//!   `t3claw_conversations` using trusted installation configuration for
...
// so `t3claw_webui_v2` consumes them via the facade crate and does not need
// a direct dependency on `t3claw_product_adapters` — the single-facade
// boundary is enforced by `t3claw_architecture`.
...
pub use t3claw_product_adapters::{
