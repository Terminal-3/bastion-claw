...
use t3claw_approvals::{ApprovalResolutionError, ApprovalResolver, DenyApproval, LeaseApproval};
use t3claw_authorization::{CapabilityLeaseStatus, CapabilityLeaseStore};
use t3claw_events::AuditSink;
use t3claw_host_api::{Action, ApprovalRequestId, CapabilityId, ResourceScope};
use t3claw_run_state::{ApprovalRequestStore, ApprovalStatus, RunStateError};
...
mod tests {
    ...
    use t3claw_run_state::{ApprovalRequestStore, InMemoryApprovalRequestStore};
    ...
    fn approval_request(
        invocation_fingerprint: Option<t3claw_host_api::InvocationFingerprint>,
    ) -> ApprovalRequest {
        ApprovalRequest {
