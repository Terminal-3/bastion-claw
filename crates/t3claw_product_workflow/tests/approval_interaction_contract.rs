...
use chrono::Utc;
use t3claw_approvals::{DenyApproval, LeaseApproval};
...
use t3claw_run_state::{ApprovalRequestStore, ApprovalStatus, InMemoryApprovalRequestStore};
use t3claw_turns::{
...
impl ApprovalInteractionReadModel for FakeReadModel {
    ...
    ) -> Result<Vec<ApprovalGateRecord>, t3claw_product_workflow::ProductWorkflowError> {
        Ok(self
            .gates
            .lock()
            .expect("lock")
    ...
    async fn approval_gate(
        ...
        gate_ref: &GateRef,
    ) -> Result<Option<ApprovalGateRecord>, t3claw_product_workflow::ProductWorkflowError> {
        Ok(self
            .gates
            .lock()
            .expect("lock")
...
impl ApprovalLeaseTermsProvider for FixedLeaseTermsProvider {
    ...
    ) -> Result<LeaseApproval, t3claw_product_workflow::ProductWorkflowError> {
...
impl ApprovalResolutionPort for RecordingApprovalResolver {
    ...
    ) -> Result<(), t3claw_product_workflow::ProductWorkflowError> {
...
async fn unsupported_approval_action_returns_invalid_request_without_resolution() {
    ...
    assert!(matches!(
        err,
        t3claw_product_workflow::ProductWorkflowError::ApprovalInteractionRejected {
