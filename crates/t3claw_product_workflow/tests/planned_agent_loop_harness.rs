...
use t3claw_product_workflow::InboundTurnOutcome;
use t3claw_reborn::planned_driver_factory::PLANNED_DEFAULT_PROFILE_ID;
use t3claw_threads::MessageStatus;
...
async fn product_live_harness_invokes_builtin_echo_through_host_runtime() {
    let harness = ProductLiveAgentLoopHarness::new(ProductLiveAgentLoopHarnessConfig {
        ...
        host_runtime_capability: Some(HostRuntimeCapabilityConfig {
            capability_id: t3claw_host_runtime::ECHO_CAPABILITY_ID.to_string(),
    ...
    assert_eq!(
        ...
        t3claw_host_runtime::ECHO_CAPABILITY_ID
    );
    assert_eq!(
