...
//!   reachable server via `T3CLAW_HOOKS_POSTGRES_URL` / `DATABASE_URL`;
...
use t3claw_hooks::predicate_state::{
...
mod libsql_cluster {
    use super::*;
    use t3claw_hooks_libsql::LibSqlPredicateStateBackend;
    ...
    impl Cluster {
        ...
        pub async fn distinct_invocation_scopes(&self, tenant: &str) -> usize {
            let scope = t3claw_hooks_libsql::test_support::scope_hash_bytes(tenant);
...
mod postgres_cluster {
    ...
    pub(super) async fn distinct_invocation_scopes(url: &str, tenant: &str) -> usize {
        ...
        let scope = t3claw_hooks_postgres::test_support::scope_hash_bytes(tenant);
        let row = client
            .query_one(
                "SELECT count(DISTINCT key_hash) FROM hooks_predicate_invocations \
                 WHERE scope_hash = $1",
        ...
        ($url:ident) => {
            match prepare().await {
                ...
                None => {
                    if require_postgres() {
                        panic!(
                            "T3CLAW_REQUIRE_POSTGRES=1 but Postgres multi-host parity \
                             could not run (T3CLAW_HOOKS_POSTGRES_URL / DATABASE_URL \
