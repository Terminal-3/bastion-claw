...
pub(crate) async fn postgres_backend() -> Result<Option<Arc<dyn PredicateStateBackend>>, String> {
    ...
    let Ok(url) =
        std::env::var("T3CLAW_HOOKS_POSTGRES_URL").or_else(|_| std::env::var("DATABASE_URL"))
    else {
        return Ok(None);
    };
...
/// When `T3CLAW_REQUIRE_POSTGRES=1`, a missing/unreachable Postgres backend is
...
pub(crate) fn require_postgres_or_skip(script_name: &str) {
    let required = std::env::var("T3CLAW_REQUIRE_POSTGRES")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false);
    if required {
        panic!(
            "[{script_name}] T3CLAW_REQUIRE_POSTGRES=1 but the Postgres parity leg \
             did not run (backend not compiled with --features postgres, or \
             T3CLAW_HOOKS_POSTGRES_URL / DATABASE_URL unset/unreachable). \
    ...
    eprintln!(
        "[{script_name}] Postgres leg SKIPPED (no --features postgres or no reachable \
         ...
         T3CLAW_REQUIRE_POSTGRES=1 to make this a hard failure in CI."
    );
}
