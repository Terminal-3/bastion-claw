# Agent Map — t3claw_wasm_sandbox_core
...
- Neighboring runtime crates:
  - `crates/t3claw_wasm/AGENTS.md`
  - `crates/t3claw_wasm_product_adapters/AGENTS.md`
...
- Fast local check: `cargo test -p t3claw_wasm_sandbox_core`
- Boundary check after dependency/API changes: `cargo test -p t3claw_architecture`
- Run downstream runtime tests when changing exported sandbox primitives: `cargo test -p t3claw_wasm` and `cargo test -p t3claw_wasm_product_adapters`.
