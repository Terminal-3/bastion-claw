    ...
    pub async fn process_verified_webhook_immediate_ack_with_observer(
        ...
        while let Some(result) = tasks.try_join_next() {
            if let Err(error) = result {
                tracing::debug!(
                    target = "t3claw::product_adapter::runner",
                    error = %error,
...
mod tests {
    ...
    #[async_trait]
    impl t3claw_product_adapters::ProductWorkflow for BlockingWorkflow {
        async fn submit_inbound(
            &self,
            _envelope: ProductInboundEnvelope,
        ) -> Result<ProductInboundAck, ProductAdapterError> {
    ...
    fn runner_with_workflow(
        parse_count: Arc<AtomicUsize>,
        workflow: Arc<dyn t3claw_product_adapters::ProductWorkflow>,
        max_in_flight: usize,
