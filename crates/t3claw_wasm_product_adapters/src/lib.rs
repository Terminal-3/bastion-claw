//! Host runtime glue for T3Claw Reborn WASM v2 product adapters.
//!
//! # Trust-model warning
//!
...
//!   [`t3claw_product_adapters::ProtocolAuthEvidence::Verified`] before any
//!   adapter parse step.
//! * `WebhookAuth` — bridge that returns a `Verified` evidence constructed via
//!   the public `mark_*_verified` helpers in `t3claw_product_adapters::auth`.
