# t3claw_wasm_product_adapters guardrails

Owns the thin WASM ProductAdapter host-glue boundary for T3Claw Reborn.
...
- Do not perform production HTTP, DNS/private-IP checks, redirect handling, response limits, secret injection, leak scanning, or redaction here. Actual network egress must delegate through host-runtime services backed by `t3claw_network` and secret leases.
...
- Runtime-specific WIT/request shapes may live here, but shared product-surface DTOs stay in `t3claw_product_adapters`.
...
- `t3claw_architecture` boundary tests pin crate dependencies, local guardrails, and WIT trust-boundary invariants.
