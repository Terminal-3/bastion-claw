# t3claw_product_adapter_registry Agent Notes

- This crate owns ProductAdapter host-api section projection for T3Claw Reborn. Generic extension manifests, installation state, activation state, health, and credential bindings live in `t3claw_extensions`.
...
- Do not add an env-var adapter declaration path. `t3claw_extensions` installation state is authoritative.
...
- Validation runs:
  - `cargo test -p t3claw_product_adapter_registry`
  - `cargo clippy -p t3claw_product_adapter_registry --all-targets -- -D warnings`
  - `cargo test -p t3claw_architecture reborn_crate_dependency_boundaries_hold`
