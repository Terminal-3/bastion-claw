...
//!
//! ```text
//! t3claw_extensions::ExtensionInstallationStore
//!   manifests/installations for any extension
//!
//! list_enabled_product_adapter_entries(store)
//!   → filter enabled installations whose manifest carries t3claw.product_adapter/v1
...
use t3claw_host_api::{ExtensionId, HostPortCatalog};
...
pub use t3claw_extensions::ManifestHash;

// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------

pub const PRODUCT_ADAPTER_HOST_API_ID: &str = "t3claw.product_adapter/v1";
...
/// `t3claw.product_adapter/v1` host-api section, then pairs each with its
...
fn project_product_adapter_sections(
    ...
    // `t3claw_extensions` validates host-api sections from its internal
    ...
    // API in `t3claw_extensions` instead of caching private parser state here.
