# t3claw_product_adapter_registry guardrails

Owns ProductAdapter host-api projection contracts for T3Claw Reborn.

- ProductAdapter declarations live in the single Extension Manifest v2 as
  `t3claw.product_adapter/v1` host-api sections. This crate owns the typed
  ...
  `t3claw_extensions`. This crate must not introduce a second ProductAdapter
...
- Do **not** depend on legacy `ChannelsConfig`, `ExtensionManager`,
  ...
  boundary test in `crates/t3claw_architecture` enforces this.
- Credential bindings store `t3claw_host_api::SecretHandle` only. Raw
...
- Health updates are generic extension installation state and use
  `t3claw_extensions::ExtensionHealthMessage` for redacted debug output.
