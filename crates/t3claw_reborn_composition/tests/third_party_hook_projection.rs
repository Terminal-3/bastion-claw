...
use t3claw_hooks::identity::{ExtensionId as HookExtensionId, HookId, HookLocalId, HookVersion};
use t3claw_hooks::registry::HookPointSpec;
use t3claw_host_api::{TenantId, VirtualPath};
...
async fn projection_for(
    ...
    build_hook_projection_registry(
        t3claw_extensions::ExtensionRegistry::new(),
...
async fn third_party_hook_is_projected_while_capability_stays_out_of_capability_surface() {
    ...
    // catalog or surface. (See the `t3claw_architecture` source assertion for
...
async fn matrix_before_capability_installed_deny_is_allowed_and_fires() {
    use t3claw_hooks::points::{BeforeCapabilityHookContext, SanitizedArguments};
    ...
    let ctx = BeforeCapabilityHookContext::new(
        ...
        Some(t3claw_host_api::ExtensionId::new("gate-ext").expect("ext")),
    );
...
async fn matrix_owning_extension_is_derived_not_spoofable() {
    ...
    assert_eq!(
        ...
        Some(&t3claw_host_api::ExtensionId::new("claimant").expect("ext")),
