...
use t3claw_threads::{InMemorySessionThreadService, SessionThreadService, ThreadScope};
...
async fn capability_io_rejects_cross_run_input_and_result_refs() {
    ...
    assert_eq!(
        input_error.kind,
        t3claw_turns::run_profile::AgentLoopHostErrorKind::ScopeMismatch
    );
...
async fn adapter_bundle_wires_required_product_live_components() {
    ...
    assert_eq!(
        readiness,
        t3claw_loop_support::ProductLiveCancellationReadiness::ExternallyControllable
    );
...
fn host_visible_capability_request(label: &str) -> HostVisibleCapabilityRequest {
    let mut context = ExecutionContext::local_default(
        ...
        t3claw_host_api::MountView::default(),
    )
    .unwrap();
