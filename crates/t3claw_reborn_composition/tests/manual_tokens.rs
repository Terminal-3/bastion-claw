...
use t3claw_host_api::{InvocationId, ResourceScope, SecretHandle, UserId};
...
impl AuthInteractionService for SuccessfulManualTokenInteractionService {
    ...
    async fn submit_manual_token(
        ...
        Ok(SecretSubmitResult {
            account_id: t3claw_auth::CredentialAccountId::new(),
...
fn update_binding(account: &t3claw_auth::CredentialAccount) -> CredentialAccountUpdateBinding {
...
async fn manual_token_facade_retries_completed_flow_when_continuation_dispatch_fails() {
    ...
    owner.resource.thread_id = Some(t3claw_host_api::ThreadId::new("thread-retry").unwrap());
