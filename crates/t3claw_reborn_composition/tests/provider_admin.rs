...
use t3claw_reborn_composition::{RebornProviderAdmin, RebornProviderAdminError, RebornV1State};
...
fn set_provider_nearai_uses_concrete_default_model() {
    ...
    assert_eq!(outcome.model, t3claw_llm::DEFAULT_MODEL);
    let config = std::fs::read_to_string(reborn_home.join("config.toml")).expect("read config");
    assert!(
        config.contains(&format!("model = \"{}\"", t3claw_llm::DEFAULT_MODEL)),
        "config: {config}"
    );
    assert!(
