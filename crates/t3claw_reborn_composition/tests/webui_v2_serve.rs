...
use serde_json::json;
use t3claw_host_api::{AgentId, NetworkMethod, ProjectId, TenantId, ThreadId, UserId};
...
use t3claw_threads::{SessionThreadRecord, ThreadScope};
use t3claw_turns::{EventCursor, RunProfileId, RunProfileVersion, TurnRunId, TurnStatus};
...
mod openai_compat_mount_tests {
    ...
    use t3claw_reborn_composition::ProtectedRouteMount;
    use t3claw_reborn_openai_compat::{
    ...
    use t3claw_turns::runner::{ClaimRunRequest, CompleteRunRequest, TurnRunTransitionPort};
    use t3claw_turns::{
    ...
    fn binding_error(
        field: &'static str,
    ) -> impl FnOnce(t3claw_host_api::HostApiError) -> ProductWorkflowError + 'static {
    ...
    impl OpenAiChatCompletionProjectionReader for StaticChatProjectionReader {
        async fn read_chat_completion_projection(
            ...
            t3claw_reborn_openai_compat::OpenAiCompatHttpError,
        > {
    ...
    impl OpenAiResponsesProjectionReader for StaticResponsesProjectionReader {
        ...
        ) -> Result<OpenAiResponseProjection, t3claw_reborn_openai_compat::OpenAiCompatHttpError>
        {
        ...
        ) -> Result<OpenAiResponseObject, t3claw_reborn_openai_compat::OpenAiCompatHttpError>
        {
...
mod slack_personal_binding_pairing_mount_tests {
    use super::*;
    use t3claw_product_adapters::AdapterInstallationId;
    use t3claw_reborn_composition::slack_serve::SlackUserId;
    use t3claw_reborn_composition::{
    ...
    async fn submit_turn(
        ...
        Ok(RebornSubmitTurnResponse::Submitted {
            thread_id: ThreadId::new(request.thread_id.clone().unwrap_or_default())
                .expect("thread id"),
            accepted_message_ref: t3claw_turns::AcceptedMessageRef::new("msg.fake").expect("ref"),
...
async fn ws_upgrade_uses_canonical_host_over_client_host_when_configured() {
    ...
    use t3claw_reborn_composition::WebuiServeConfig;
    ...
    let app = t3claw_reborn_composition::webui_v2_app(bundle, config).expect("app");
    let (addr, handle) = spawn_serve(app).await;
...
async fn every_webui_v2_descriptor_is_mounted_on_composed_app() {
    ...
    for descriptor in t3claw_webui_v2::webui_v2_routes() {
...
// ─── static SPA mount (`t3claw_webui_v2_static`) ────────────────────
//
...
// Caller") — the standalone router test in `t3claw_webui_v2_static`
...
async fn static_root_serves_index_with_substituted_csp_nonce() {
    ...
    assert!(
        !body.contains("__T3CLAW_CSP_NONCE__"),
...
async fn static_chat_hook_listens_for_oauth_callback_completion() {
    ...
    assert!(
        body.contains("t3claw:product-auth:oauth-complete"),
...
/// `t3claw_reborn_webui_ingress::webui_v2_auth_router`) must
...
async fn public_route_mount_is_merged_without_bearer_auth_and_keeps_descriptor_policy() {
    ...
    use t3claw_host_api::{IngressScopeSource, NetworkMethod};
