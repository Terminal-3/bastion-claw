...
async fn oauth_callback_handler_reports_completed_continuation_dispatch_failure_as_retryable() {
    ...
    assert_eq!(retry.status, t3claw_auth::AuthFlowStatus::Completed);
...
async fn oauth_callback_handler_routes_exchange_failures_through_provider_boundary() {
    ...
    let parsed: t3claw_reborn_composition::RebornOAuthCallbackError =
    ...
    let flow = services
        ...
        .expect("flow lookup")
        .expect("flow record");
    assert_eq!(flow.status, t3claw_auth::AuthFlowStatus::Failed);
