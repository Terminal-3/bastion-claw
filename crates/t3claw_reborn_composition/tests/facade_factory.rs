...
#[cfg(feature = "libsql")]
use t3claw_auth::{OAuthClientId, OAuthRedirectUri};
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_host_api::{
...
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_reborn_composition::RebornRuntimeProcessBinding;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_reborn_composition::{RebornBuildError, RebornCompositionProfile, RebornServices};
...
async fn invoke_trigger_management(
    runtime: &dyn t3claw_host_runtime::HostRuntime,
    capability: &str,
    input: Value,
) -> Value {
    let outcome = runtime
...
fn trigger_management_execution_context() -> ExecutionContext {
    let grants = CapabilitySet {
        grants: vec![
            local_dev_grant(
                t3claw_host_runtime::TRIGGER_CREATE_CAPABILITY_ID,
                vec![EffectKind::DispatchCapability, EffectKind::ExternalWrite],
            ),
            local_dev_grant(
                t3claw_host_runtime::TRIGGER_LIST_CAPABILITY_ID,
                vec![EffectKind::DispatchCapability],
            ),
            local_dev_grant(
                t3claw_host_runtime::TRIGGER_REMOVE_CAPABILITY_ID,
                vec![EffectKind::DispatchCapability, EffectKind::ExternalWrite],
            ),
        ],
    };
...
async fn start_postgres_container() -> Option<(
    ...
    Some((
        container,
        format!("postgres://postgres:postgres@{host}:{port}/t3claw_test"),
    ))
}
...
fn test_sandbox_process_binding() -> RebornRuntimeProcessBinding {
    let process_port = Arc::new(t3claw_host_runtime::TenantSandboxProcessPort::new(
...
#[cfg(any(feature = "libsql", feature = "postgres"))]
#[async_trait::async_trait]
impl t3claw_host_runtime::SandboxCommandTransport for ProductionReadySandboxTransport {
    async fn run_command(
        &self,
        _request: t3claw_host_runtime::CommandExecutionRequest,
    ) -> Result<t3claw_host_runtime::CommandExecutionOutput, t3claw_host_runtime::RuntimeProcessError>
    {
...
async fn local_dev_product_auth_entrypoint_redacts_manual_token_submit() {
    ...
    let provider = t3claw_auth::AuthProviderId::new("github").unwrap();
    let label = t3claw_auth::CredentialAccountLabel::new("work github").unwrap();
    ...
    let challenge = product_auth
        .request_manual_token_setup(RebornManualTokenSetupRequest {
            ...
            label: label.clone(),
            continuation: t3claw_auth::AuthContinuationRef::SetupOnly,
            update_binding: None,
    ...
    assert_eq!(
        result.status,
        t3claw_auth::CredentialAccountStatus::Configured
    );

    let accounts = product_auth
        .credential_account_service()
        .list_accounts(t3claw_auth::CredentialAccountListRequest::new(
            scope.clone(),
            provider,
        ))
        .await
...
fn auth_scope(user: &str) -> t3claw_auth::AuthProductScope {
    t3claw_auth::AuthProductScope::new(
        t3claw_host_api::ResourceScope::local_default(
            t3claw_host_api::UserId::new(user).unwrap(),
            t3claw_host_api::InvocationId::new(),
        )
        .unwrap(),
        t3claw_auth::AuthSurface::Web,
    )
    .with_session_id(t3claw_auth::AuthSessionId::new(format!("session-{user}")).unwrap())
}
...
async fn production_google_oauth_config_uses_factory_built_product_auth_ports() {
    ...
    let result = build_reborn_services(
        ...
        .with_google_oauth_backend(t3claw_reborn_composition::OAuthClientConfig {
...
async fn production_factory_built_product_auth_manual_token_round_trips() {
    ...
    let scope = auth_scope("alice");
    let provider = t3claw_auth::AuthProviderId::new("manual-provider").unwrap();
    let label = t3claw_auth::CredentialAccountLabel::new("manual production").unwrap();
    let challenge = product_auth
...
async fn production_rejects_local_only_runtime_policy() {
    ...
    assert!(
        report.contains(
            t3claw_host_runtime::ProductionWiringComponent::RuntimePolicy,
            t3claw_host_runtime::ProductionWiringIssueKind::LocalOnlyImplementation,
        ),
    ...
    assert!(!serialized.contains("t3claw_"));
...
async fn production_libsql_resolved_secret_master_key_rejects_invalid_env_key() {
    ...
    let _env = EnvVarGuard::set(
        t3claw_secrets::keychain::SECRETS_MASTER_KEY_ENV,
    ...
    assert!(matches!(
        ...
        Err(RebornBuildError::Secret(
            t3claw_secrets::SecretError::InvalidMasterKey
        ))
    ));
}
