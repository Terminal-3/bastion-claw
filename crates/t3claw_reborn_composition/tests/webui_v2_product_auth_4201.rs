...
use t3claw_auth::{AuthProviderId, CredentialAccountId, CredentialAccountService};
use t3claw_host_api::{AgentId, InvocationId, ProjectId, ResourceScope, TenantId, UserId};
...
fn caller_scope_with_invocation_and_thread(
    ...
    thread_id: t3claw_host_api::ThreadId,
) -> AuthProductScope {
    AuthProductScope::new(
        ResourceScope {
            tenant_id: TenantId::new(TENANT).expect("tenant"),
...
) -> t3claw_auth::CredentialAccountId {
    let account = shared
...
fn auth_prompt_view_serialises_optional_fields_when_present() {
    use t3claw_product_adapters::{AuthPromptChallengeKind, AuthPromptView};
...
fn auth_prompt_view_omits_optional_fields_when_absent() {
    use t3claw_product_adapters::AuthPromptView;
    use t3claw_turns::TurnRunId;
...
async fn challenge_for_gate_returns_oauth_url_view_for_seeded_flow() {
    use chrono::Utc;
    use std::sync::Arc;
    use t3claw_auth::AuthProviderId;
    use t3claw_auth::{
