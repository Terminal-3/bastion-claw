# t3claw_reborn_composition guardrails
...
- Do not depend on the root `t3claw` crate or `src/` modules.
...
- Product auth composition must use `t3claw_auth` trait-shaped ports. Do not
...
- The first WebUI-mounted OAuth route keeps raw PKCE verifiers in a bounded,
  expiring process-local cache because `t3claw_auth` durable records may
...
| `webui_v2_app(bundle, config) -> Router` | Build the fully-composed axum `Router`. This is the seam between this product/API crate and host-owned HTTP ingress: tests drive it via `tower::ServiceExt::oneshot`; the `t3claw-reborn serve` subcommand (follow-up PR) hands it to `axum::serve` from a host-owned listener |
...
5. **Descriptor-driven per-route body limit**
   ...
   `BodyLimitPolicy` from `t3claw_webui_v2::webui_v2_routes()` and,
...
8. **Descriptor-driven per-route rate limit**
   ...
   `t3claw_webui_v2::webui_v2_routes()` plus mounted product-auth
...
9. `webui_v2_router(WebUiV2State::new(bundle.api))` — the v2
   handlers from `t3claw_webui_v2` (create-thread, list-threads, delete-thread,
...
shipped by
`t3claw_reborn_webui_ingress::webui_v2_auth_router`, which
...
- **Logout actually revokes.** `POST /auth/logout` calls
  ...
  `crates/t3claw_reborn_webui_ingress/tests/session_round_trip.rs`
...
`Set-Cookie: t3claw_session=...; HttpOnly` on its OAuth
...
| SSO login (Google) | `GET /auth/providers`, `GET /auth/login/{p}`, `GET /auth/callback/{p}`, `POST /auth/logout` | Same paths on the v2 listener via `t3claw_reborn_webui_ingress::webui_v2_auth_router`, merged into `webui_v2_app` through [`WebuiServeConfig::with_public_route_mount`] (typed `{ router, descriptors }` so the per-route body-limit / rate-limit middleware applies) | Mapped (Google); GitHub + NEAR follow under #4116 |
...
- **Body limit** — descriptor-driven per-route via
  ...
  `t3claw_webui_v2::webui_v2_routes()`: `create_thread` 16 KiB,
...
- **Static security headers** — `nosniff`, `DENY`, CSP applied via
  ...
  base-uri 'self'`.
- **Connection limit (SSE)** — bounded by `t3claw_webui_v2`'s own
...
- **Caller construction** — `WebUiAuthenticatedCaller` is built from
  ...
  `crates/t3claw_product_workflow/CLAUDE.md`.
...
### How the standalone `t3claw-reborn serve` consumes this
...
the resulting router to the host-owned `t3claw_reborn_webui_ingress`
...
- `src/webui_rate_limit.rs::tests` — unit tests for the sliding-window
  ...
  `t3claw_webui_v2::webui_v2_routes()`, and
