...
//! - custom provider definitions  → `$T3CLAW_REBORN_HOME/providers.json`
...
use t3claw_llm::registry::{ProviderDefinition, ProviderProtocol, ProviderRegistry};
use t3claw_llm::{NearWalletSignedMessage, OpenAiCodexConfig, OpenAiCodexSessionManager};
...
use t3claw_reborn_config::{LlmSlotSelection, RebornBootConfig};
...
pub struct RebornLlmConfigService {
    ...
    nearai_session: Option<Arc<t3claw_llm::SessionManager>>,
...
impl RebornLlmConfigService {
    ...
    pub fn with_nearai_session(mut self, session: Arc<t3claw_llm::SessionManager>) -> Self {
    ...
    async fn build_snapshot(&self) -> Result<LlmConfigSnapshot, LlmConfigServiceError> {
        ...
        let builtin_registry = t3claw_llm::ProviderRegistry::try_load_from_path(None)
    ...
    async fn probe_provider(
    ...
    ) -> Result<Arc<dyn t3claw_llm::LlmProvider>, LlmConfigServiceError> {
        ...
        let session = t3claw_llm::create_session_manager(config.session.clone()).await;
        t3claw_llm::build_static_provider_chain(&config, session)
            .await
            .map_err(|_| LlmConfigServiceError::Unavailable)
    }
    ...
    async fn probe_matches_persisted_provider(
        ...
        let registry = tokio::task::spawn_blocking(move || {
            t3claw_llm::ProviderRegistry::try_load_from_path(Some(providers_path.as_path()))
        })
        .await
        .map_err(|_| LlmConfigServiceError::Unavailable)?
        .map_err(|_| LlmConfigServiceError::Unavailable)?;
...
/// `t3claw_llm::resolution` uses so the Codex login manager resolves the same
...
pub(crate) async fn apply_nearai_login(
    session: &t3claw_llm::SessionManager,
    boot: &RebornBootConfig,
...
mod tests {
    use super::*;
    use t3claw_host_api::{AgentId, ProjectId, ResourceScope, SecretHandle, TenantId, UserId};
    use t3claw_reborn_config::{RebornHome, RebornProfile};
    use t3claw_secrets::{
    ...
    fn editing_a_builtin_preserves_protocol_and_setup() {
        ...
        let registry = t3claw_llm::ProviderRegistry::try_load_from_path(None).expect("registry");
