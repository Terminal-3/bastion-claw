...
#[cfg(test)]
use t3claw_event_projections::CapabilityActivityProjection;
...
use t3claw_events::{DurableEventLog, EventCursor, EventStreamKey, ReadScope};
use t3claw_first_party_extension_ports::SkillActivationObserver;
...
impl RebornProjectionServices {
    ...
    pub(crate) fn with_model_failure_explainer_factory(
        ...
        system_inference: Arc<
            dyn Fn() -> Arc<dyn t3claw_turns::run_profile::SystemInferencePort> + Send + Sync,
        >,
    ) -> Self {
    ...
    match item {
        ...
        ProjectionStreamItem::Update(envelope) => {
            ...
            match envelope.as_ref() {
                t3claw_event_streams::ProductProjectionEnvelope::ThreadUpdates(replay) => {
                    replay_payloads(
                        scope,
                        display_previews,
                        replay,
                ...
                t3claw_event_streams::ProductProjectionEnvelope::ThreadLiveUpdate(update) => {
...
async fn runtime_payloads_for_item(
    ...
    let snapshot = ProjectionSnapshot {
        timeline: t3claw_event_projections::ThreadTimeline {
            entries: Vec::new(),
        },
        runs,
        capability_activities,
...
fn snapshot_from_envelope(
    envelope: t3claw_event_streams::ProductProjectionEnvelope,
) -> Result<ProjectionSnapshot, ProductAdapterError> {
    match envelope {
        t3claw_event_streams::ProductProjectionEnvelope::ThreadSnapshot(snapshot) => Ok(snapshot),
        _ => Err(internal_projection_error(
