...
//! `t3claw_reborn` (drivers, host factory, model gateway bridge),
//! `t3claw_threads` (session thread service), and (under the
//! `root-llm-provider` feature) `t3claw_llm` are composed into a running
//! agent.
//!
...
//! pinned by `crates/t3claw_architecture/tests/reborn_dependency_boundaries.rs`.
...
use t3claw_events::{DurableAuditLog, DurableEventLog, InMemoryAuditSink, RuntimeEvent};
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_filesystem::RootFilesystem;
use t3claw_first_party_extension_ports::{
...
use t3claw_product_adapters::ProjectionStream;
...
use t3claw_reborn::loop_exit_applier::{
    ApprovalGateEvidenceStore, ThreadCheckpointLoopExitEvidencePort,
};
use t3claw_reborn::milestone_events::{
...
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_reborn::subagent::goal_store::FilesystemSubagentGoalStore;
#[cfg(not(any(feature = "libsql", feature = "postgres")))]
use t3claw_reborn::subagent::goal_store::InMemoryBoundedSubagentGoalStore;
use t3claw_reborn::subagent::{
    flavors::StaticSubagentDefinitionResolver, gate_resolution::BoundedSubagentGateResolutionStore,
};
use t3claw_reborn::turn_runner::{
...
struct RuntimeStoreParts<'a> {
    ...
    checkpoint_state_store: Arc<dyn t3claw_turns::CheckpointStateStore>,
    loop_checkpoint_store: Arc<dyn t3claw_turns::LoopCheckpointStore>,
    thread_service: Arc<dyn SessionThreadService>,
    event_log: Arc<dyn DurableEventLog>,
    audit_log: Arc<dyn DurableAuditLog>,
    resource_governor: Arc<dyn t3claw_resources::ResourceGovernor>,
    ...
    trigger_repository: Option<Arc<dyn t3claw_triggers::TriggerRepository>>,
}
    ...
    F: RootFilesystem + 'static,
{
    RuntimeStoreParts {
        ...
        loop_checkpoint_store: Arc::clone(&graph.turn_state)
            as Arc<dyn t3claw_turns::LoopCheckpointStore>,
...
pub struct RebornRuntime {
    ...
    trigger_conversation_pairing:
        Option<Arc<dyn t3claw_conversations::ConversationActorPairingService>>,
    ...
    #[cfg(feature = "root-llm-provider")]
    boot: Option<t3claw_reborn_config::RebornBootConfig>,
...
struct TriggerPollerServices {
    materializer: Arc<dyn t3claw_triggers::TriggerPromptMaterializer>,
    trusted_submitter: Arc<dyn t3claw_triggers::TrustedTriggerFireSubmitter>,
    ...
    /// never submitted.
    #[cfg(any(test, feature = "test-support"))]
    pairing_service: Arc<dyn t3claw_conversations::ConversationActorPairingService>,
}
...
async fn build_trigger_poller_services(
    ...
    #[cfg(any(feature = "libsql", feature = "postgres"))]
    {
        ...
        let pairing_service: Arc<
            dyn t3claw_conversations::ConversationActorPairingService,
...
where
    B: t3claw_conversations::ConversationBindingService + Clone + 'static,
    S: t3claw_conversations::SessionThreadService + 'static,
{
    ...
    let trusted_submitter = t3claw_conversations::trusted_trigger_fire_submitter(
        binding_service,
...
) -> Arc<dyn t3claw_triggers::TriggerActiveRunLookup> {
...
impl LocalDevApprovalTurnRunLocator {
    ...
    async fn snapshot(
        &self,
    ) -> Result<TurnPersistenceSnapshot, t3claw_product_workflow::ProductWorkflowError> {
        #[cfg(feature = "libsql")]
        {
            self.turn_state
...
struct LocalDevApprovalGateEvidence {
    approval_requests: Arc<dyn t3claw_run_state::ApprovalRequestStore>,
}
...
impl ApprovalGateEvidenceStore for LocalDevApprovalGateEvidence {
    async fn pending_approval_gate(
        ...
        Ok(record
            .map(|record| record.status == t3claw_run_state::ApprovalStatus::Pending)
            .unwrap_or(false))
    }
}
...
impl ApprovalTurnRunLocator for LocalDevApprovalTurnRunLocator {
    ...
    ) -> Result<Vec<ApprovalBlockedTurnRun>, t3claw_product_workflow::ProductWorkflowError> {
    ...
    async fn approval_run_for_gate(
        &self,
        scope: &ApprovalInteractionScope,
        gate_ref: &t3claw_turns::GateRef,
    ) -> Result<Option<TurnRunId>, t3claw_product_workflow::ProductWorkflowError> {
        let turn_scope = TurnScope::new(
            scope.tenant_id.clone(),
            scope.agent_id.clone(),
            scope.project_id.clone(),
...
#[cfg(feature = "libsql")]
fn approval_turn_locator_unavailable() -> t3claw_product_workflow::ProductWorkflowError {
    t3claw_product_workflow::ProductWorkflowError::Transient {
...
async fn fold_legacy_webui_identities<R>(
    ...
    tenant_id: &TenantId,
    store: &R,
) -> Result<(), t3claw_reborn_identity::RebornIdentityError>
where
    R: t3claw_reborn_identity::RebornIdentityResolver + ?Sized,
{
    use t3claw_reborn_identity::{
    ...
    fn invalid_key(error: t3claw_reborn_identity::IdentityKeyError) -> RebornIdentityError {
        RebornIdentityError::Backend(error.to_string())
    }
...
impl RebornRuntime {
    ...
    #[cfg(feature = "root-llm-provider")]
    pub(crate) fn webui_boot_config(&self) -> Option<&t3claw_reborn_config::RebornBootConfig> {
        self.boot.as_ref()
    }
    ...
    #[cfg(feature = "root-llm-provider")]
    pub(crate) fn webui_llm_session(&self) -> Option<Arc<t3claw_llm::SessionManager>> {
        self.llm_reload
            .as_ref()
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn trigger_repository(&self) -> Option<Arc<dyn t3claw_triggers::TriggerRepository>> {
        self.services
            .local_runtime
            .as_ref()
    ...
    pub async fn open_reborn_identity_resolver(
        ...
        Result<
            Arc<dyn t3claw_reborn_identity::RebornIdentityResolver>,
            t3claw_reborn_identity::RebornIdentityError,
        >,
    > {
        ...
        let store = t3claw_reborn_identity::FilesystemRebornIdentityStore::new(
        ...
        Some(Ok(
            Arc::new(store) as Arc<dyn t3claw_reborn_identity::RebornIdentityResolver>
        ))
    }
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn budget_resource_governor(&self) -> Option<Arc<dyn t3claw_resources::ResourceGovernor>> {
        self.services
            .local_runtime
            .as_ref()
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn budget_event_sink(&self) -> Option<Arc<t3claw_resources::InMemoryBudgetEventSink>> {
        self.services
            .local_runtime
            .as_ref()
    ...
    pub fn broadcast_budget_event_sink(
        &self,
    ) -> Option<Arc<t3claw_resources::BroadcastBudgetEventSink>> {
        self.services
            .local_runtime
            .as_ref()
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn budget_gate_store(&self) -> Option<Arc<dyn t3claw_resources::BudgetGateStore>> {
        self.services
            .local_runtime
            .as_ref()
    ...
    pub fn apply_resolved_budget_gate(
        &self,
        scope: &t3claw_host_api::ResourceScope,
        gate_id: t3claw_resources::BudgetGateId,
    ) -> Result<t3claw_resources::BudgetApprovalGate, RebornRuntimeError> {
        ...
        if let t3claw_resources::BudgetGateStatus::Approved {
            increased_limit, ..
        } = &gate.status
        {
            local_runtime
...
pub async fn build_reborn_runtime(
    ...
    let llm_cost_table_arc: Option<Arc<dyn t3claw_loop_support::ModelCostTable>> =
        llm_cost_table.map(|table| Arc::new(table) as Arc<dyn t3claw_loop_support::ModelCostTable>);
    #[cfg(any(test, feature = "test-support"))]
    ...
    let model_budget_accountant: Option<
        Arc<dyn t3claw_turns::run_profile::LoopModelBudgetAccountant>,
    > = match (profile, resolved_cost_table) {
        ...
        (_, Some(cost_table)) => {
            let resolved_budget_defaults = match budget_defaults {
                ...
                None => {
                    let defaults = t3claw_reborn_config::BudgetDefaults::compiled_defaults()
                        .with_env()
                        .map_err(|error| RebornRuntimeError::InvalidArgument {
            ...
            let event_sink: Arc<dyn t3claw_resources::BudgetEventSink> =
                Arc::clone(&broadcast_budget_event_sink)
                    as Arc<dyn t3claw_resources::BudgetEventSink>;
    ...
    let mut loop_exit_evidence = ThreadCheckpointLoopExitEvidencePort::new_with_thread_scope(
        Arc::clone(&thread_service),
        Arc::clone(&turn_state_store) as Arc<dyn t3claw_turns::TurnStateStore>,
        Arc::clone(&loop_checkpoint_store) as Arc<dyn t3claw_turns::LoopCheckpointStore>,
        thread_scope.clone(),
    );
        ...
        loop_exit_evidence = loop_exit_evidence.with_approval_gate_evidence(Arc::new(
            LocalDevApprovalGateEvidence {
                approval_requests: Arc::clone(&local_runtime.approval_requests)
                    as Arc<dyn t3claw_run_state::ApprovalRequestStore>,
            },
        ));
    }
    ...
    let planned_runtime_parts = DefaultPlannedRuntimeParts {
        ...
        checkpoint_state_store: Arc::clone(&checkpoint_state_store)
            as Arc<dyn t3claw_turns::CheckpointStateStore>,
        ...
        safety_context: None,
        hook_security_audit_sink: Some(Arc::new(t3claw_events::TracingSecurityAuditSink)),
    ...
    let failure_explanation_inference = Arc::new(move || {
        ...
        )) as Arc<dyn t3claw_turns::run_profile::SystemInferencePort>
    });
    ...
    let trigger_conversation_pairing_value: Option<
        Arc<dyn t3claw_conversations::ConversationActorPairingService>,
    >;
...
fn local_dev_selector_config(
    ...
    SkillActivationSelectorConfig {
        ...
        selection_mode:
            t3claw_first_party_extension_ports::SkillActivationSelectionMode::ExplicitOnly,
...
async fn build_production_model_gateway(
    ...
    (
        Arc<dyn t3claw_loop_support::HostManagedModelGateway>,
        Option<t3claw_loop_support::StaticModelCostTable>,
...
struct LlmGatewayBundle {
    gateway: Arc<dyn t3claw_loop_support::HostManagedModelGateway>,
    ...
    policy: t3claw_reborn::model_gateway::LlmModelProfilePolicy,
...
pub(crate) struct RebornLlmReloadParts {
    pub(crate) reload_handle: Arc<t3claw_llm::LlmReloadHandle>,
    pub(crate) session: Arc<t3claw_llm::SessionManager>,
...
async fn build_llm_gateway(llm: ResolvedRebornLlm) -> Result<LlmGatewayBundle, RebornRuntimeError> {
    let session = t3claw_llm::create_session_manager(llm.config.session.clone()).await;
    let raw = t3claw_llm::build_static_provider_chain(&llm.config, Arc::clone(&session))
        .await
...
async fn build_placeholder_llm_gateway() -> Result<LlmGatewayBundle, RebornRuntimeError> {
    let session = t3claw_llm::create_session_manager(t3claw_llm::SessionConfig::default()).await;
    let raw: Arc<dyn t3claw_llm::LlmProvider> = Arc::new(PlaceholderLlmProvider);
    wrap_swappable_gateway(raw, session)
}
...
fn wrap_swappable_gateway(
    raw: Arc<dyn t3claw_llm::LlmProvider>,
    session: Arc<t3claw_llm::SessionManager>,
) -> Result<LlmGatewayBundle, RebornRuntimeError> {
    use t3claw_llm::{LlmProvider, LlmReloadHandle, SwappableLlmProvider};
    use t3claw_reborn::model_gateway::{LlmModelProfilePolicy, LlmProviderModelGateway};
    use t3claw_turns::run_profile::ModelProfileId;
...
#[cfg(feature = "root-llm-provider")]
#[async_trait::async_trait]
impl t3claw_llm::LlmProvider for PlaceholderLlmProvider {
    ...
    ) -> Result<t3claw_llm::CompletionResponse, t3claw_llm::LlmError> {
    ...
    ) -> Result<t3claw_llm::ToolCompletionResponse, t3claw_llm::LlmError> {
        Err(placeholder_unconfigured_error())
    }
}

#[cfg(feature = "root-llm-provider")]
fn placeholder_unconfigured_error() -> t3claw_llm::LlmError {
    t3claw_llm::LlmError::RequestFailed {
...
#[cfg(not(feature = "root-llm-provider"))]
fn build_stub_gateway() -> Arc<dyn t3claw_loop_support::HostManagedModelGateway> {
...
mod tests {
    ...
    use async_trait::async_trait;
    use chrono::Utc;
    use t3claw_auth::{GOOGLE_CALENDAR_EVENTS_SCOPE, GOOGLE_CALENDAR_READONLY_SCOPE};
    ...
    use t3claw_authorization::CapabilityLeaseStore;
    use t3claw_events::{EventStreamKey, ReadScope};
    use t3claw_host_api::{
    ...
    use t3claw_product_adapters::{ProductOutboundPayload, ProductProjectionItem};
    use t3claw_product_workflow::{
    ...
    use t3claw_run_state::ApprovalRequestStore;
    ...
    impl RuntimeEnvGuard {
        fn set(name: &'static str, value: &str) -> Self {
            let previous = t3claw_common::env_helpers::env_or_override(name);
            t3claw_common::env_helpers::set_runtime_env(name, value);
            Self { name, previous }
        }
    }
    ...
    impl Drop for RuntimeEnvGuard {
        fn drop(&mut self) {
            match &self.previous {
                Some(value) => t3claw_common::env_helpers::set_runtime_env(self.name, value),
                None => t3claw_common::env_helpers::remove_runtime_env(self.name),
            }
        }
    }
    ...
    fn nearai_gateway_test_request() -> HostManagedModelRequest {
        HostManagedModelRequest {
            model_profile_id: t3claw_turns::run_profile::ModelProfileId::new("interactive_model")
                .expect("model profile id"),
            messages: vec![t3claw_loop_support::HostManagedModelMessage {
                role: HostManagedModelMessageRole::User,
                content: "hello model".to_string(),
                content_ref: t3claw_turns::LoopMessageRef::new(
    ...
    #[cfg(feature = "root-llm-provider")]
    #[async_trait]
    impl t3claw_llm::LlmProvider for RecordingLlmProvider {
        fn model_name(&self) -> &str {
            "recording-provider"
        }
        ...
        async fn complete(
            &self,
            request: t3claw_llm::CompletionRequest,
        ) -> Result<t3claw_llm::CompletionResponse, t3claw_llm::LlmError> {
            self.requests
                .lock()
        ...
        async fn complete_with_tools(
            &self,
            request: t3claw_llm::ToolCompletionRequest,
        ) -> Result<t3claw_llm::ToolCompletionResponse, t3claw_llm::LlmError> {
            self.requests
                .lock()
                .expect("recording provider request lock poisoned")
        ...
        fn set_model(&self, model: &str) -> Result<(), t3claw_llm::LlmError> {
            *self
                .active_model
                .lock()
    ...
    async fn swappable_gateway_uses_current_active_model_for_requests() {
        ...
        let raw: Arc<dyn t3claw_llm::LlmProvider> = provider.clone();
        let session =
            t3claw_llm::create_session_manager(t3claw_llm::SessionConfig::default()).await;
    ...
    async fn root_llm_gateway_bootstraps_nearai_session_token_from_env() {
        ...
        let config = t3claw_llm::LlmConfig {
            backend: "nearai".to_string(),
            session: t3claw_llm::SessionConfig {
                auth_base_url: base_url.clone(),
                session_path: session_dir.path().join("session.json"),
            },
            nearai: t3claw_llm::NearAiConfig {
    ...
    async fn production_runtime_rejects_enabled_hooks_without_local_runtime() {
        ...
        let input = RebornRuntimeInput::from_services(
            RebornBuildInput::libsql(
                ...
                None,
                t3claw_secrets::SecretMaterial::from("01234567890123456789012345678901"),
            )
            ...
            .with_runtime_process_binding(RebornRuntimeProcessBinding::tenant_sandbox(Arc::new(
                t3claw_host_runtime::TenantSandboxProcessPort::new(Arc::new(
    ...
    #[async_trait]
    impl t3claw_host_runtime::SandboxCommandTransport for RecordingSandboxTransport {
        async fn run_command(
            &self,
            _request: t3claw_host_runtime::CommandExecutionRequest,
        ) -> Result<
            t3claw_host_runtime::CommandExecutionOutput,
            t3claw_host_runtime::RuntimeProcessError,
        > {
            Ok(t3claw_host_runtime::CommandExecutionOutput {
                output: String::new(),
                saved_output: None,
                exit_code: 0,
                sandboxed: true,
    ...
    async fn local_dev_runtime_rejects_invalid_trigger_poller_worker_config() {
        ...
        let trigger_poller = TriggerPollerSettings {
            enabled: true,
            worker: t3claw_triggers::TriggerPollerWorkerConfig {
    ...
    async fn local_dev_yolo_message_flow_ignores_model_budget_gate() {
        ...
        let cost_table = t3claw_loop_support::StaticModelCostTable::new().with_entry(
