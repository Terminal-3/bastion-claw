...
//! Reborn `$T3CLAW_REBORN_HOME/config.toml` and reads the shared provider
//! catalog through `t3claw_llm`.
...
impl RebornProviderAdmin {
    ...
    fn load_registry(&self) -> Result<t3claw_llm::ProviderRegistry, RebornProviderAdminError> {
        let providers_path = self.boot.home().providers_file_path();
        t3claw_llm::ProviderRegistry::try_load_from_path(Some(providers_path.as_path())).map_err(
...
pub enum RebornProviderAdminError {
    ...
    LoadConfig {
        path: PathBuf,
        source: Box<t3claw_reborn_config::RebornConfigFileError>,
    },
    ...
    UpdateConfig {
        path: PathBuf,
        source: Box<t3claw_reborn_config::RebornConfigFileUpdateError>,
    },
}
...
fn unique_provider_definitions(
    registry: &t3claw_llm::ProviderRegistry,
) -> Vec<&t3claw_llm::registry::ProviderDefinition> {
...
fn known_provider_ids(registry: &t3claw_llm::ProviderRegistry) -> Vec<String> {
    unique_provider_definitions(registry)
        .into_iter()
        .map(|def| def.id.clone())
        .collect()
        ...
        metadata: verbose.then(|| RebornProviderMetadata {
            ...
            accepts_api_key: def.api_key_env.is_some()
                || def
                    .setup
                    .as_ref()
                    .is_some_and(t3claw_llm::registry::SetupHint::accepts_api_key),
            can_list_models: def
                .setup
                .as_ref()
                .is_some_and(t3claw_llm::registry::SetupHint::can_list_models),
        }),
    }
}

fn provider_protocol_wire_name(protocol: t3claw_llm::registry::ProviderProtocol) -> String {
