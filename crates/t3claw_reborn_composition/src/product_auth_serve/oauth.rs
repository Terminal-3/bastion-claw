...
async fn start_google_oauth_flow(
    ...
    .map_err(ProductAuthRouteFailure::from)?;
    let opaque_state_hash = opaque_state_hash(opaque_state.as_str())?;
    let pkce_verifier_secret = SecretString::from(t3claw_common::pkce::generate_code_verifier());
...
fn oauth_callback_completion_html(response: &RebornOAuthCallbackResponse) -> Response {
    const CHANNEL: &str = "t3claw-product-auth";
    const STORAGE_KEY: &str = "t3claw:product-auth:oauth-complete";
    const MESSAGE_TYPE: &str = "t3claw:product-auth:oauth-complete";
...
mod tests {
    ...
    use t3claw_auth::{GOOGLE_CALENDAR_READONLY_SCOPE, InMemoryAuthProductServices};
    use t3claw_host_api::{RuntimeCredentialAccountProviderId, RuntimeCredentialAuthRequirement};
