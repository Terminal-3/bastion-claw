...
//!   point used by the standalone `t3claw-reborn` binary and any
...
pub use t3claw_triggers::TriggerId;
...
#[cfg(feature = "webui-v2-beta")]
pub mod host_api {
    pub use t3claw_host_api::{AgentId, ProjectId, TenantId, UserId};
}
...
/// direct `t3claw_reborn` dependency (the
...
#[cfg(feature = "webui-v2-beta")]
pub use t3claw_reborn::local_trigger_access::{
...
/// types are re-exported so host wiring (`t3claw-reborn serve`, the CLI
/// `UserDirectory` adapter) depends on the facade vocabulary, never on
/// `t3claw_reborn_identity` directly. The concrete filesystem-backed store
/// stays private to this composition layer (composition CLAUDE.md: "keep
/// lower substrate handles private").
#[cfg(feature = "webui-v2-beta")]
pub use t3claw_reborn_identity::{
...
pub fn open_reborn_identity_resolver(
    ...
    let root = std::sync::Arc::new(t3claw_filesystem::InMemoryBackend::default());
    ...
    .expect("mount view");
    let filesystem = std::sync::Arc::new(t3claw_filesystem::ScopedFilesystem::with_fixed_view(
        root, view,
    ));
    std::sync::Arc::new(t3claw_reborn_identity::FilesystemRebornIdentityStore::new(
        filesystem,
        tenant_id.clone(),
...
mod webui_user_access_checker_tests {
    ...
    use t3claw_host_api::{AgentId, ProjectId, TenantId, UserId};
...
/// the CLI mirror `t3claw_reborn::model_routes::ModelSlot`.
pub fn reborn_model_slot_names() -> Vec<&'static str> {
    t3claw_reborn::model_routes::ModelSlot::all()
        .iter()
...
pub fn reborn_runtime_readiness_snapshot() -> RebornRuntimeReadinessSnapshot {
    let mut registry = t3claw_reborn::driver_registry::DriverRegistry::new();
    let text_only_driver = RebornRuntimeComponentStatus::from_result(
        t3claw_reborn::planned_driver_factory::register_default_text_only_driver(
            &mut registry,
            t3claw_reborn::text_loop_driver::TextOnlyModelReplyDriverConfig::default(),
        ),
    );
    let family_registry = t3claw_reborn::app_loop_family::build_loop_family_registry();
    let planned_driver = match &family_registry {
        Ok(family_registry) => RebornRuntimeComponentStatus::from_result(
            t3claw_reborn::planned_driver_factory::register_default_planned_driver(
                &mut registry,
    ...
    let subagent_planned_driver = match family_registry {
        Ok(family_registry) => RebornRuntimeComponentStatus::from_result(
            t3claw_reborn::planned_driver_factory::register_subagent_planned_driver(
                &mut registry,
                family_registry,
            ),
        ),
...
use t3claw_authorization::CapabilityLeaseError;
#[cfg(feature = "libsql")]
use t3claw_filesystem::LibSqlRootFilesystem;
#[cfg(feature = "postgres")]
use t3claw_filesystem::PostgresRootFilesystem;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_filesystem::{RootFilesystem, ScopedFilesystem};
use t3claw_host_api::ProcessBackendKind;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_host_api::{
...
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_host_runtime::{CapabilitySurfaceVersion, HostRuntimeServices};
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_processes::{FilesystemProcessResultStore, FilesystemProcessStore};
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_reborn_event_store::RebornEventStoreConfig;
use t3claw_reborn_event_store::RebornEventStoreError;
use t3claw_resources::ResourceError;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_resources::{FilesystemResourceGovernorStore, PersistentResourceGovernor};
use t3claw_run_state::RunStateError;
use t3claw_secrets::SecretError;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_secrets::SecretMaterial;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_trust::TrustPolicy;
use t3claw_turns::TurnError;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_turns::TurnRunWakeNotifier;
use thiserror::Error;
...
/// [`t3claw_host_api::ResourceScope::system`]) routes records under
...
pub fn invocation_mount_view(
    scope: &ResourceScope,
) -> Result<MountView, t3claw_host_api::HostApiError> {
...
pub enum RebornCompositionError {
    ...
    Mount(#[from] t3claw_host_api::HostApiError),
    ...
    RunProfile(#[from] t3claw_turns::run_profile::RunProfileRegistryError),
    ...
    ProductionWiring {
        report: t3claw_host_runtime::ProductionWiringReport,
    },
}
...
pub fn open_reborn_postgres_pool(
    ...
    Ok(t3claw_reborn_event_store::open_postgres_pool(url)?)
}
...
pub fn open_reborn_postgres_pool_with_max_size(
    ...
    Ok(t3claw_reborn_event_store::open_postgres_pool_with_max_size(
        url, max_size,
    )?)
}
...
mod mount_view_tests {
    use super::*;
    use t3claw_filesystem::{FilesystemError, FilesystemOperation, InMemoryBackend};
    use t3claw_host_api::{
...
mod two_tenant_isolation_tests {
    ...
    use t3claw_host_api::{AgentId, InvocationId, ProjectId, SecretHandle, TenantId, UserId};
    use t3claw_secrets::{FilesystemSecretStore, SecretMaterial, SecretStore, SecretsCrypto};
