...
impl CredentialAccountRecordSource for UnsupportedCredentialAccountRecordSource {
    ...
    ) -> Result<Vec<t3claw_auth::CredentialAccount>, AuthProductError> {
...
impl RebornProductAuthServices {
    ...
    pub async fn request_manual_token_setup(
        ...
        match challenge {
            t3claw_auth::AuthChallenge::ManualTokenRequired {
                interaction_id,
                provider,
                label,
                expires_at,
...
fn auth_challenge_to_view(
    challenge: &t3claw_auth::AuthChallenge,
    flow: &t3claw_auth::AuthFlowRecord,
) -> AuthChallengeView {
    match challenge {
        t3claw_auth::AuthChallenge::OAuthUrl {
            authorization_url,
            expires_at,
        } => AuthChallengeView {
        ...
        t3claw_auth::AuthChallenge::AccountSelectionRequired { .. }
        | t3claw_auth::AuthChallenge::ReauthorizeRequired { .. }
        | t3claw_auth::AuthChallenge::SetupRequired { .. } => AuthChallengeView {
...
impl AuthChallengeProvider for RebornProductAuthServices {
    async fn challenge_for_gate(
        ...
        run_id: TurnRunId,
        gate_ref: &str,
        credential_requirements: &[t3claw_host_api::RuntimeCredentialAuthRequirement],
...
mod tests {
    ...
    impl AuthInteractionService for SharedAuthTestDouble {
        async fn request_secret_input(
            &self,
            _request: t3claw_auth::ManualTokenSetupRequest,
