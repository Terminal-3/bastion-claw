...
async fn token_sink_rolls_back_access_token_when_refresh_write_fails() {
    ...
    assert_eq!(
        ...
        t3claw_auth::AuthErrorCode::BackendUnavailable
    );
    assert!(
        store
            .deleted_handles()
...
async fn google_exchange_fails_closed_when_response_omits_scope() {
    ...
    assert_eq!(
        error.code(),
        t3claw_auth::AuthErrorCode::TokenExchangeFailed
    );
...
impl RuntimeHttpEgress for NoopEgress {
    ...
    ) -> Result<t3claw_host_api::RuntimeHttpEgressResponse, t3claw_host_api::RuntimeHttpEgressError>
    {
        Err(t3claw_host_api::RuntimeHttpEgressError::Network {
