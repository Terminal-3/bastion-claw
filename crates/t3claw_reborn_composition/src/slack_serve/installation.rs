...
use t3claw_product_adapters::{AdapterInstallationId, ProtocolAuthEvidence};
use t3claw_slack_v2_adapter::SlackPayloadParseError;
use t3claw_wasm_product_adapters::{ImmediateAckWorkflowObserver, RunnerError};
use thiserror::Error;
        ...
        if verified.is_empty() {
            return Err(auth_failure
                .unwrap_or(RunnerError::AuthenticationFailed {
                    failure: t3claw_product_adapters::ProtocolAuthFailure::Missing,
                })
                .into());
        }
        Ok(verified)
...
mod tests {
    ...
    use t3claw_wasm_product_adapters::{ImmediateAckWorkflowObserver, WebhookProcessOutcome};
