...
use t3claw_host_api::{AgentId, ApprovalRequestId, ProjectId, TenantId, UserId};
use t3claw_outbound::{
...
async fn build_harness_with_actor_user_resolver_and_auth_challenges(
    ...
    let adapter_id = t3claw_product_adapters::ProductAdapterId::new(ADAPTER).expect("adapter id"); // safety: static test adapter id is valid.
...
async fn slack_dm_delivers_final_reply_after_auth_completes_outside_slack() {
    ...
    assert_eq!(messages[1]["text"], "T3Claw is thinking...");
...
async fn slack_dm_posts_working_indicator_and_deletes_it_after_final_reply() {
    ...
    assert_eq!(messages.len(), 1);
    assert_eq!(messages[0]["channel"], CHANNEL);
    assert_eq!(messages[0]["text"], "T3Claw is thinking...");
...
fn turn_state(
    ...
    TurnRunState {
        ...
        status,
        accepted_message_ref,
        source_binding_ref: t3claw_turns::SourceBindingRef::new("slack:source")
