...
use t3claw_auth::{AuthProviderId, OAuthAuthorizationUrl};
use t3claw_event_projections::{CapabilityActivityProjection, ProjectionSnapshot, ThreadTimeline};
...
impl AuthChallengeProvider for FakeAuthChallengeProvider {
    async fn challenge_for_gate(
        ...
        run_id: TurnRunId,
        gate_ref: &str,
        _credential_requirements: &[t3claw_host_api::RuntimeCredentialAuthRequirement],
    ) -> Result<Option<AuthChallengeView>, t3claw_auth::AuthProductError> {
...
impl AuthChallengeProvider for FailingAuthChallengeProvider {
    async fn challenge_for_gate(
        ...
        Err(t3claw_auth::AuthProductError::BackendUnavailable)
    }
}
