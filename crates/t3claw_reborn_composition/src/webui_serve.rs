...
//!
//! The `t3claw_webui_v2` crate ships handlers that dispatch through
...
//! `t3claw-reborn serve` subcommand (on a follow-up PR) consumes the
//! same `Router` and owns the listener lifecycle on the host side.
//!
//! ### Why no serve-and-bind helper here
//!
//! `t3claw_reborn_composition` sits in the Reborn product/API
...
use t3claw_auth::GoogleOAuthRouteConfig;
use t3claw_host_api::ingress::IngressRouteDescriptor;
...
use serde::Serialize;
use t3claw_product_workflow::WebUiAuthenticatedCaller;
...
pub struct WebuiServeConfig {
    ...
    /// per-route [`BodyLimitPolicy`](t3claw_host_api::ingress::BodyLimitPolicy)
    /// declared in `t3claw_webui_v2::webui_v2_routes()`; that
    ...
    /// by `t3claw_reborn_webui_ingress::webui_v2_auth_router`
...
impl WebuiServeConfig {
    ...
    /// `t3claw_reborn_webui_ingress::webui_v2_auth_router` plugs
...
/// - WebChat v2 route set from `t3claw_webui_v2::webui_v2_router`
///
/// The returned [`Router`] is the seam between this composition crate
/// and host-owned ingress code: tests drive it via
/// `tower::ServiceExt::oneshot`, and the standalone `t3claw-reborn
...
pub fn webui_v2_app_with_lifecycle(
    ...
    let mut descriptors = t3claw_webui_v2::webui_v2_routes();
    ...
    let app = app
        ...
        // `t3claw_webui_v2_static` bundle. Routed AFTER the
        ...
        .merge(t3claw_webui_v2_static::mount_at_prefix("/v2"))
...
async fn authenticate_request(
    ...
    #[cfg(feature = "openai-compat-beta")]
    {
        let scope = t3claw_reborn_openai_compat::OpenAiCompatActorScope::new(
            state.tenant_id.clone(),
            openai_user_id.clone(),
            state.default_agent_id.clone(),
            state.default_project_id.clone(),
        );
        let auth_evidence = t3claw_product_adapters::mark_bearer_token_verified_for_tenant(
            openai_user_id.as_str(),
            state.tenant_id.clone(),
        );
        let caller = match t3claw_reborn_openai_compat::OpenAiCompatAuthenticatedCaller::new(
            scope,
            auth_evidence,
        ) {
            Ok(caller) => caller,
...
fn panic_handler(
    ...
    tracing::error!(
        target = "t3claw::reborn::webui_serve",
