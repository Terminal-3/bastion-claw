...
use t3claw_threads::{FinalizedAssistantMessageByRunRequest, SessionThreadService, ThreadScope};
use t3claw_turns::{
    GetRunStateRequest, ReplyTargetBindingRef, TurnActor, TurnCoordinator, TurnRunId, TurnRunState,
    TurnScope, TurnStatus,
};
use t3claw_wasm_product_adapters::ImmediateAckWorkflowObserver;
...
const SLACK_WORKING_MESSAGE: &str = "T3Claw is thinking...";
...
impl SlackFinalReplyDeliveryObserver {
    ...
    async fn deliver_run_notification(
        ...
        let delivery = t3claw_outbound::PrepareCommunicationDeliveryRequest {
        ...
        let _outcome = prepare_and_render_product_outbound(
            ...
            ProductOutboundDeliveryRequest {
                delivery,
                payload,
                projection_cursor: t3claw_product_adapters::ProjectionCursor::new(projection_id)
    ...
    async fn post_slack_working_message(
        ...
        .await
        {
            ...
            Err(error) => {
                tracing::warn!(
                    target = "t3claw::reborn::slack_delivery",
                    error = %error,
...
fn slack_auth_prompt_view(
    envelope: &ProductInboundEnvelope,
    mut view: t3claw_product_adapters::AuthPromptView,
) -> t3claw_product_adapters::AuthPromptView {
...
#[async_trait]
impl t3claw_outbound::ThreadProjectionAccessPolicy for AllowNoProjectionAccess {
    async fn authorize_projection_access(
        &self,
        _request: t3claw_outbound::ThreadProjectionAccessRequest,
    ) -> Result<t3claw_outbound::ThreadProjectionAccessClaim, OutboundError> {
...
fn should_deliver_after_ack(envelope: &ProductInboundEnvelope, ack: &ProductInboundAck) -> bool {
    ...
    !matches!(
        ...
        ProductInboundPayload::AuthResolution(payload)
            if matches!(
                &payload.result,
                t3claw_product_adapters::AuthResolutionResult::Denied
            )
    ) && !matches!(
        envelope.payload(),
        ProductInboundPayload::ApprovalResolution(payload)
            if payload.decision == t3claw_product_adapters::ApprovalDecision::Deny
    ) && !matches!(
        envelope.payload(),
...
mod tests {
    ...
    use t3claw_turns::AcceptedMessageRef;
    ...
    fn envelope(payload: ProductInboundPayload) -> ProductInboundEnvelope {
        let adapter_id =
            t3claw_product_adapters::ProductAdapterId::new("slack_v2").expect("adapter");
    ...
    fn auth_denial_ack_does_not_enter_slack_delivery_loop() {
        let payload = ProductInboundPayload::AuthResolution(
            t3claw_product_adapters::AuthResolutionPayload::new(
                "gate:auth-test",
                t3claw_product_adapters::AuthResolutionResult::Denied,
            )
    ...
    fn auth_completion_ack_still_enters_slack_delivery_loop() {
        let payload = ProductInboundPayload::AuthResolution(
            t3claw_product_adapters::AuthResolutionPayload::new(
                "gate:auth-test",
                t3claw_product_adapters::AuthResolutionResult::CallbackCompleted {
                    callback_ref: t3claw_auth::AuthFlowId::new().to_string(),
                },
            )
            .expect("auth resolution"),
        );
