...
mod tests {
    ...
    async fn automation_facade_maps_blocked_and_unknown_outcomes() {
        let capability_id =
            t3claw_host_api::CapabilityId::new(TRIGGER_LIST_CAPABILITY_ID).expect("capability");
        let cases = [
            (
            ...
            (
                RuntimeCapabilityOutcome::ResourceBlocked(RuntimeResourceGate {
                    ...
                    estimate: t3claw_host_api::ResourceEstimate::default(),
                }),
                RebornServicesErrorCode::Unavailable,
                RebornServicesErrorKind::BlockedResource,
                503,
    ...
    impl HostRuntime for RecordingHostRuntime {
        async fn invoke_capability(
            ...
            Ok(RuntimeCapabilityOutcome::Completed(Box::new(
                RuntimeCapabilityCompleted {
                    ...
                    usage: t3claw_host_api::ResourceUsage::default(),
                },
            )))
        }
        ...
        async fn resume_capability(
            &self,
            _request: t3claw_host_runtime::RuntimeCapabilityResumeRequest,
        ) -> Result<RuntimeCapabilityOutcome, HostRuntimeError> {
        ...
        async fn cancel_work(
            &self,
            _request: t3claw_host_runtime::CancelRuntimeWorkRequest,
        ) -> Result<t3claw_host_runtime::CancelRuntimeWorkOutcome, HostRuntimeError> {
            unreachable!("cancel work is not used in automation facade tests")
        }

        async fn runtime_status(
            &self,
            _request: t3claw_host_runtime::RuntimeStatusRequest,
        ) -> Result<t3claw_host_runtime::HostRuntimeStatus, HostRuntimeError> {
            unreachable!("runtime status is not used in automation facade tests")
        }

        async fn health(&self) -> Result<t3claw_host_runtime::HostRuntimeHealth, HostRuntimeError> {
