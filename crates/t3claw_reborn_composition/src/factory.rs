...
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_auth::AuthProviderClient;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_authorization::FilesystemCapabilityLeaseStore;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_authorization::GrantAuthorizer;
#[cfg(not(feature = "libsql"))]
use t3claw_authorization::InMemoryCapabilityLeaseStore;
#[cfg(not(any(feature = "libsql", feature = "postgres")))]
use t3claw_conversations::InMemoryConversationServices;
use t3claw_conversations::{
    AdapterInstallationId, AdapterKind, ConversationActorPairingService, ExternalActorRef,
};
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_conversations::{InboundTurnError, RebornFilesystemConversationServices};
use t3claw_events::{DurableAuditLog, DurableEventLog};
#[cfg(not(feature = "libsql"))]
use t3claw_events::{InMemoryDurableAuditLog, InMemoryDurableEventLog};
use t3claw_extensions::{ExtensionInstallationStore, ExtensionLifecycleService, ExtensionRegistry};
#[cfg(not(feature = "libsql"))]
use t3claw_filesystem::InMemoryBackend;
#[cfg(feature = "libsql")]
use t3claw_filesystem::LibSqlRootFilesystem;
#[cfg(feature = "postgres")]
use t3claw_filesystem::PostgresRootFilesystem;
...
use t3claw_filesystem::{LocalFilesystem, ScopedFilesystem};
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_host_api::runtime_policy::EffectiveRuntimePolicy;
use t3claw_host_api::runtime_policy::{FilesystemBackendKind, ProcessBackendKind, SecretMode};
use t3claw_host_api::{
    EffectKind, ExtensionId, HostPath, MountPermissions, MountView, PackageId, RuntimeHttpEgress,
    UserId, VirtualPath,
};
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_host_api::{HostApiError, MountAlias, MountGrant, ResourceScope};
...
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_loop_support::FilesystemCheckpointStateStore;
use t3claw_outbound::CommunicationPreferenceRepository;
#[cfg(feature = "libsql")]
use t3claw_outbound::FilesystemOutboundStateStore;
#[cfg(not(feature = "libsql"))]
use t3claw_outbound::InMemoryOutboundStateStore;
use t3claw_processes::ProcessServices;
use t3claw_product_workflow::ProductAuthTurnGateResumeDispatcher;
use t3claw_resources::InMemoryResourceGovernor;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_resources::{
...
#[cfg(feature = "libsql")]
use t3claw_run_state::{FilesystemApprovalRequestStore, FilesystemRunStateStore};
#[cfg(not(feature = "libsql"))]
use t3claw_run_state::{InMemoryApprovalRequestStore, InMemoryRunStateStore};
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_secrets::FilesystemCredentialBroker;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_secrets::FilesystemSecretStore;
use t3claw_secrets::SecretStore;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_threads::FilesystemSessionThreadService;
#[cfg(not(feature = "libsql"))]
use t3claw_threads::InMemorySessionThreadService;
use t3claw_threads::SessionThreadService;
use t3claw_triggers::{
...
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_turns::FilesystemTurnStateStore;
#[cfg(any(feature = "libsql", feature = "postgres"))]
use t3claw_turns::InMemoryRunProfileResolver;
use t3claw_turns::{CheckpointStateStore, DefaultTurnCoordinator, LoopCheckpointStore};
#[cfg(not(feature = "libsql"))]
use t3claw_turns::{
...
type LocalDevProcessServices = ProcessServices<
    t3claw_processes::FilesystemProcessStore<LocalDevRootFilesystem>,
    t3claw_processes::FilesystemProcessResultStore<LocalDevRootFilesystem>,
>;
#[cfg(not(feature = "libsql"))]
type LocalDevProcessServices = ProcessServices<
    t3claw_processes::InMemoryProcessStore,
    t3claw_processes::InMemoryProcessResultStore,
>;
...
where
    F: t3claw_filesystem::RootFilesystem + 'static,
    G: t3claw_resources::ResourceGovernor + 'static,
    S: t3claw_processes::ProcessStore + 'static,
    R: t3claw_processes::ProcessResultStore + 'static,
{
    match binding {
...
pub struct RebornServices {
    pub host_runtime: Option<Arc<dyn t3claw_host_runtime::HostRuntime>>,
    pub turn_coordinator: Option<Arc<dyn t3claw_turns::TurnCoordinator>>,
...
impl RebornServices {
    ...
    pub fn local_dev_approval_test_parts(&self) -> Option<RebornLocalDevApprovalTestParts> {
        ...
        let approval_requests: Arc<dyn t3claw_run_state::ApprovalRequestStore> =
            local_runtime.approval_requests.clone();
        let capability_leases: Arc<dyn t3claw_authorization::CapabilityLeaseStore> =
...
pub struct RebornLocalDevApprovalTestParts {
    pub approval_requests: Arc<dyn t3claw_run_state::ApprovalRequestStore>,
    pub capability_leases: Arc<dyn t3claw_authorization::CapabilityLeaseStore>,
}
...
pub(crate) struct RebornLocalRuntimeServices {
    ...
    pub(crate) resource_governor: Arc<dyn t3claw_resources::ResourceGovernor>,
    ...
    /// stream".
    #[allow(dead_code)]
    pub(crate) budget_event_sink: Arc<dyn t3claw_resources::BudgetEventSink>,
    ...
    #[allow(dead_code)]
    pub(crate) in_memory_budget_event_sink: Arc<t3claw_resources::InMemoryBudgetEventSink>,
    ...
    pub(crate) broadcast_budget_event_sink: Arc<t3claw_resources::BroadcastBudgetEventSink>,
    ...
    #[allow(dead_code)]
    pub(crate) budget_gate_store: Arc<dyn t3claw_resources::BudgetGateStore>,
...
impl RebornServices {
    pub fn disabled() -> Self {
        Self {
            ...
            #[cfg(feature = "root-llm-provider")]
            secret_store: Arc::new(t3claw_secrets::InMemorySecretStore::new()),
        }
    }
}
...
fn auth_continuation_dispatcher(
    turn_coordinator: Arc<dyn t3claw_turns::TurnCoordinator>,
...
fn production_config(
    required_runtime_backends: Vec<t3claw_host_api::RuntimeKind>,
    require_runtime_http_egress: bool,
    require_wasm_credentials: bool,
) -> t3claw_host_runtime::ProductionWiringConfig {
    let mut config = t3claw_host_runtime::ProductionWiringConfig::new(required_runtime_backends);
...
async fn build_local_dev(input: RebornBuildInput) -> Result<RebornServices, RebornBuildError> {
    ...
    let turn_coordinator: Arc<dyn t3claw_turns::TurnCoordinator> = Arc::new(
    ...
    #[cfg(not(any(feature = "libsql", feature = "postgres")))]
    let secret_store: Arc<dyn SecretStore> = Arc::new(t3claw_secrets::InMemorySecretStore::new());
    let local_dev_trust_policy = Arc::new(builtin_first_party_trust_policy()?);
    let local_dev_trust_invalidation_bus = Arc::new(t3claw_trust::InvalidationBus::new());
    ...
    .try_with_host_http_egress_with_body_store(
        t3claw_network::PolicyNetworkHttpEgress::new(
            t3claw_network::ReqwestNetworkTransport::default(),
        ),
    ...
    let host_runtime: Arc<dyn t3claw_host_runtime::HostRuntime> =
...
fn build_local_dev_store_graph(
    ...
    let local_runtime = Arc::new(RebornLocalRuntimeServices {
        ...
        resource_governor: Arc::clone(&resource_governor)
            as Arc<dyn t3claw_resources::ResourceGovernor>,
        budget_event_sink,
        in_memory_budget_event_sink,
        broadcast_budget_event_sink,
        budget_gate_store,
...
async fn local_dev_trigger_repository(
    ...
    let repository = t3claw_triggers::LibSqlTriggerRepository::new(database);
    repository
        .run_migrations()
        .await
        .map_err(|error| RebornBuildError::InvalidConfig {
...
fn local_dev_trigger_repository() -> Arc<dyn TriggerRepository> {
    Arc::new(t3claw_triggers::InMemoryTriggerRepository::default())
}
...
struct BudgetSinks {
    budget_event_sink: Arc<dyn t3claw_resources::BudgetEventSink>,
    in_memory_budget_event_sink: Arc<t3claw_resources::InMemoryBudgetEventSink>,
    broadcast_budget_event_sink: Arc<t3claw_resources::BroadcastBudgetEventSink>,
    budget_gate_store: Arc<dyn t3claw_resources::BudgetGateStore>,
}

fn build_budget_sinks() -> BudgetSinks {
    let in_memory_budget_event_sink = Arc::new(t3claw_resources::InMemoryBudgetEventSink::new());
    let broadcast_budget_event_sink =
        Arc::new(t3claw_resources::BroadcastBudgetEventSink::default());
    let budget_event_sink: Arc<dyn t3claw_resources::BudgetEventSink> =
        Arc::new(t3claw_resources::CompositeBudgetEventSink::new(vec![
            Arc::clone(&in_memory_budget_event_sink) as Arc<dyn t3claw_resources::BudgetEventSink>,
            Arc::clone(&broadcast_budget_event_sink) as Arc<dyn t3claw_resources::BudgetEventSink>,
        ]));
    let budget_gate_store: Arc<dyn t3claw_resources::BudgetGateStore> =
        Arc::new(t3claw_resources::InMemoryBudgetGateStore::new());
    BudgetSinks {
        budget_event_sink,
        in_memory_budget_event_sink,
        broadcast_budget_event_sink,
    ...
    F: RootFilesystem + 'static,
{
    ...
    let crypto = Arc::new(t3claw_secrets::SecretsCrypto::new(master_key)?);
...
fn resolve_local_dev_secret_master_key(
    root: &Path,
) -> Result<t3claw_secrets::SecretMaterial, RebornBuildError> {
    let key_path = root.join(LOCAL_DEV_SECRETS_MASTER_KEY_PATH);
    match std::fs::read_to_string(&key_path) {
        Ok(existing) => {
            return Ok(t3claw_secrets::SecretMaterial::from(
    ...
    let key = std::env::var(t3claw_secrets::keychain::SECRETS_MASTER_KEY_ENV)
        .ok()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(t3claw_secrets::keychain::generate_master_key_hex);
    write_local_dev_secret_master_key(&key_path, &key)?;
    Ok(t3claw_secrets::SecretMaterial::from(key))
}
...
fn local_dev_event_log(
    ...
    Ok(Arc::new(
        t3claw_reborn_event_store::FilesystemDurableEventLog::new(scoped),
    ))
}
...
fn local_dev_audit_log(
    ...
    Ok(Arc::new(
        t3claw_reborn_event_store::FilesystemDurableAuditLog::new(scoped),
    ))
}
...
async fn resolve_secret_master_key(
    explicit: Option<t3claw_secrets::SecretMaterial>,
) -> Result<t3claw_secrets::SecretMaterial, RebornBuildError> {
...
struct RebornProductionWiring {
    ...
    turn_run_wake_notifier: Arc<dyn t3claw_turns::TurnRunWakeNotifier>,
...
struct RebornProductionBuildContext {
    ...
    wiring_config: t3claw_host_runtime::ProductionWiringConfig,
...
fn production_wiring(
    ...
    turn_run_wake_notifier: Option<Arc<dyn t3claw_turns::TurnRunWakeNotifier>>,
    runtime_process_binding: RebornRuntimeProcessBinding,
...
fn planned_run_profile_resolver() -> Result<Arc<InMemoryRunProfileResolver>, RebornBuildError> {
    Ok(Arc::new(
        t3claw_reborn::planned_driver_factory::default_planned_run_profile_resolver().map_err(
...
type FilesystemProductionHostRuntimeServices<F> = HostRuntimeServices<
    ...
    t3claw_processes::FilesystemProcessStore<F>,
    t3claw_processes::FilesystemProcessResultStore<F>,
>;
...
where
    TPolicy: t3claw_trust::TrustPolicy + 'static,
    TWake: t3claw_turns::TurnRunWakeNotifier + 'static,
{
...
where
    ...
    TWake: t3claw_turns::TurnRunWakeNotifier + 'static,
{
    let filesystem = Arc::new(t3claw_filesystem::PostgresRootFilesystem::new(config.pool));
    filesystem.run_migrations().await?;
    build_filesystem_production_host_runtime_services(
        filesystem,
        config.event_store,
...
async fn build_filesystem_production_host_runtime_services<F, TPolicy, TWake>(
    filesystem: Arc<F>,
    event_store: t3claw_reborn_event_store::RebornEventStoreConfig,
    secret_master_key: Option<t3claw_secrets::SecretMaterial>,
    ...
    TWake: t3claw_turns::TurnRunWakeNotifier + 'static,
{
    ...
    .with_security_audit_sink(Arc::new(t3claw_events::TracingSecurityAuditSink))
    ...
    .with_run_profile_resolver(Arc::new(
        t3claw_reborn::planned_driver_factory::default_planned_run_profile_resolver()?,
    ))
    .with_reborn_event_store_config(
        t3claw_reborn_event_store::RebornProfile::Production,
        event_store,
    )
    .await?;
    ...
    F: RootFilesystem + 'static,
{
    fn new(
        scoped_filesystem: Arc<ScopedFilesystem<F>>,
        crypto: Arc<t3claw_secrets::SecretsCrypto>,
    ) -> Self {
        Self {
    ...
    fn from_master_key(
        scoped_filesystem: Arc<ScopedFilesystem<F>>,
        master_key: t3claw_secrets::SecretMaterial,
    ) -> Result<Self, crate::RebornCompositionError> {
        Ok(Self::new(
            scoped_filesystem,
            Arc::new(t3claw_secrets::SecretsCrypto::new(master_key)?),
        ))
    }
}
...
async fn build_filesystem_secret_credential_stores<F>(
    scoped_filesystem: Arc<ScopedFilesystem<F>>,
    master_key: Option<t3claw_secrets::SecretMaterial>,
...
async fn resolve_explicit_or_keychain_master_key(
    explicit: Option<t3claw_secrets::SecretMaterial>,
) -> Result<Option<t3claw_secrets::SecretMaterial>, t3claw_secrets::SecretError> {
    if let Some(master_key) = explicit {
        Ok(Some(master_key))
    } else if let Some(master_key) = t3claw_secrets::keychain::resolve_master_key_material().await?
    {
        Ok(Some(master_key))
    } else {
        Ok(None)
    ...
    F: RootFilesystem + 'static,
{
    fn new(
        filesystem: Arc<F>,
        secret_master_key: t3claw_secrets::SecretMaterial,
        event_store: t3claw_reborn_event_store::RebornEventStoreConfig,
    ...
    F: RootFilesystem + 'static,
{
    ...
    let event_stores = t3claw_reborn_event_store::build_reborn_event_stores(
    ...
    let services = HostRuntimeServices::new(
        ...
        Arc::new(t3claw_authorization::GrantAuthorizer::new()),
    ...
    let turn_coordinator: Arc<dyn t3claw_turns::TurnCoordinator> =
...
async fn build_libsql_production(
    ...
    path_or_url: String,
    auth_token: Option<t3claw_secrets::SecretMaterial>,
    secret_master_key: t3claw_secrets::SecretMaterial,
) -> Result<RebornServices, RebornBuildError> {
    use t3claw_filesystem::LibSqlRootFilesystem;
    ...
    filesystem.run_migrations().await?;
    let trigger_repository = Arc::new(t3claw_triggers::LibSqlTriggerRepository::new(db));
    trigger_repository
        .run_migrations()
        .await
        .map_err(|error| RebornBuildError::InvalidConfig {
    ...
    let stores = ProductionStoreBundle::new(
        filesystem,
        secret_master_key,
        t3claw_reborn_event_store::RebornEventStoreConfig::Libsql {
            path_or_url,
            auth_token,
        },
    )?;
...
async fn build_postgres_production(
    ...
    url: t3claw_secrets::SecretMaterial,
    secret_master_key: t3claw_secrets::SecretMaterial,
) -> Result<RebornServices, RebornBuildError> {
    use t3claw_filesystem::PostgresRootFilesystem;
    ...
    filesystem.run_migrations().await?;
    let trigger_repository = Arc::new(t3claw_triggers::PostgresTriggerRepository::new(pool));
    trigger_repository
        .run_migrations()
        .await
        .map_err(|error| RebornBuildError::InvalidConfig {
    ...
    let stores = ProductionStoreBundle::new(
        filesystem,
        secret_master_key,
        t3claw_reborn_event_store::RebornEventStoreConfig::Postgres { url },
    )?;
...
mod tests {
    ...
    use t3claw_authorization::{CapabilityLeaseStatus, CapabilityLeaseStore, GrantAuthorizer};
    use t3claw_filesystem::FilesystemError;
    #[cfg(any(feature = "libsql", feature = "postgres"))]
    use t3claw_filesystem::{
    ...
    use t3claw_product_workflow::{LifecyclePackageKind, LifecyclePackageRef};
    use t3claw_trust::{AuthorityCeiling, EffectiveTrustClass, TrustDecision, TrustProvenance};
    ...
    impl ConversationActorPairingService for FailingConversationActorPairingService {
        async fn pair_external_actor(
            ...
            _user_id: UserId,
        ) -> Result<(), t3claw_conversations::InboundTurnError> {
            Err(t3claw_conversations::InboundTurnError::DurableState {
    ...
    fn trigger_record_for_pairing_test() -> TriggerRecord {
        TriggerRecord {
            trigger_id: t3claw_triggers::TriggerId::new(),
            ...
            source: t3claw_triggers::TriggerSourceKind::Schedule,
            schedule: t3claw_triggers::TriggerSchedule::cron("* * * * *")
                .expect("valid cron expression"),
            completion_policy: t3claw_triggers::TriggerCompletionPolicy::Recurring,
            prompt: "pairing test prompt".to_string(),
            state: t3claw_triggers::TriggerState::Scheduled,
    ...
    async fn durable_trigger_conversation_services_propagates_init_error() {
        ...
        assert!(matches!(
            error,
            t3claw_conversations::InboundTurnError::DurableState { .. }
        ));
    }
    ...
    async fn local_dev_default_product_auth_preserves_manual_token_across_rebuilds() {
        ...
        let mut scope = scope;
        scope.resource.thread_id = Some(t3claw_host_api::ThreadId::new("auth-thread").unwrap());

        let challenge = product_auth
            .request_manual_token_setup(crate::RebornManualTokenSetupRequest::new(
                scope.clone(),
                t3claw_auth::AuthProviderId::new("github").unwrap(),
                CredentialAccountLabel::new("work github").unwrap(),
                t3claw_auth::AuthContinuationRef::SetupOnly,
        ...
        let account = product_auth
            ...
            .get_account(t3claw_auth::CredentialAccountLookupRequest::new(
                scope.clone(),
        ...
        let flows = product_auth
            ...
            .flows_for_owner(t3claw_auth::AuthFlowOwnerScope {
        ...
        assert_eq!(
            ...
            t3claw_auth::AuthFlowStatus::Completed
        );
    }
    ...
    async fn local_dev_gsuite_installs_activates_and_dispatches_through_host_runtime() {
        ...
        services
            ...
            .create_account(NewCredentialAccount {
                scope: auth_scope,
                provider: t3claw_first_party_extensions::google_provider_id()
    ...
    async fn local_dev_services_persist_thread_records_across_rebuilds() {
        ...
        let scope = t3claw_threads::ThreadScope {
            tenant_id: t3claw_host_api::TenantId::new("persist-tenant").unwrap(),
            agent_id: t3claw_host_api::AgentId::new("persist-agent").unwrap(),
            project_id: None,
            owner_user_id: Some(t3claw_host_api::UserId::new("persist-owner").unwrap()),
            mission_id: None,
        };
        let thread_id = t3claw_host_api::ThreadId::new("persisted-thread").unwrap();
        ...
        services
            ...
            .expect("local runtime")
            .thread_service
            .ensure_thread(t3claw_threads::EnsureThreadRequest {
                scope: scope.clone(),
        ...
        let history = rebuilt
            ...
            .thread_service
            .list_thread_history(t3claw_threads::ThreadHistoryRequest {
                scope,
    ...
    fn builtin_first_party_package_declares_skill_management_tools() {
        ...
        let registry = t3claw_host_runtime::builtin_first_party_handlers(Arc::new(
        ...
        for id in [
            ...
            assert!(registry.contains_handler(&t3claw_host_api::CapabilityId::new(id).unwrap()));
        }
        assert!(!registry.contains_handler(
            &t3claw_host_api::CapabilityId::new(SKILL_ACTIVATE_CAPABILITY_ID).unwrap()
        ));
    }
    ...
    fn web_access_network_policy() -> NetworkPolicy {
        NetworkPolicy {
            allowed_targets: vec![NetworkTargetPattern {
                scheme: Some(t3claw_host_api::NetworkScheme::Https),
    ...
    fn skill_mounts() -> MountView {
        let scope = t3claw_host_api::ResourceScope::local_default(
            UserId::new("local-dev-test-user").expect("valid user id"),
            t3claw_host_api::InvocationId::new(),
        )
        .expect("valid resource scope");
    ...
    fn local_dev_minimal_approval_policy() -> t3claw_host_api::runtime_policy::EffectiveRuntimePolicy
    {
        let mut policy = crate::local_dev_runtime_policy().expect("local-dev policy resolves");
        policy.requested_profile = t3claw_host_api::runtime_policy::RuntimeProfile::LocalYolo;
        policy.resolved_profile = t3claw_host_api::runtime_policy::RuntimeProfile::LocalYolo;
        policy.approval_policy = t3claw_host_api::runtime_policy::ApprovalPolicy::Minimal;
        policy
    }
