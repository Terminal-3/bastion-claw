...
//! `t3claw_reborn` or `t3claw_llm` directly.
...
use t3claw_loop_support::HostManagedModelGateway;
use t3claw_loop_support::HostSkillContextSource;
use t3claw_reborn_config::BudgetDefaults;
...
pub struct ResolvedRebornLlm {
    provider_id: String,
    model: String,
    pub(crate) config: t3claw_llm::LlmConfig,
}
...
impl ResolvedRebornLlm {
    ...
    pub fn from_llm_config(config: t3claw_llm::LlmConfig) -> Self {
        Self {
...
pub struct RebornRuntimeInput {
    ...
    /// override skips).
    #[cfg(any(test, feature = "test-support"))]
    pub(crate) model_cost_table_override: Option<Arc<dyn t3claw_loop_support::ModelCostTable>>,
}
...
impl RebornRuntimeInput {
    ...
    pub fn with_model_cost_table_override(
        mut self,
        cost_table: Arc<dyn t3claw_loop_support::ModelCostTable>,
    ) -> Self {
