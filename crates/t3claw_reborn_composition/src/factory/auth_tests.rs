...
impl RebornAuthContinuationDispatcher for NoopContinuationDispatcher {
    ...
    ) -> Result<(), t3claw_auth::AuthProductError> {
        Ok(())
    }
}
...
async fn local_dev_oauth_turn_gate_callback_resumes_default_turn_coordinator() {
    ...
    let gate_ref = t3claw_turns::GateRef::new("gate:auth-callback").unwrap();
    local_runtime
        .turn_state
...
async fn local_dev_notion_dcr_oauth_backend_builds_and_wires_registry() {
    ...
    let services = build_reborn_services(
        ...
        .with_notion_dcr_oauth_backend("http://127.0.0.1:3000", "T3Claw")
...
async fn submit_and_block_auth_run(
    turn_coordinator: &dyn t3claw_turns::TurnCoordinator,
    local_runtime: &RebornLocalRuntimeServices,
    scope: TurnScope,
    actor: TurnActor,
    gate_ref: &str,
) -> t3claw_turns::TurnRunId {
    let submit = turn_coordinator
        .submit_turn(SubmitTurnRequest {
            scope: scope.clone(),
            actor,
    ...
    local_runtime
        ...
        .block_run(BlockRunRequest {
            ...
            reason: BlockedReason::Auth {
                gate_ref: t3claw_turns::GateRef::new(gate_ref).unwrap(),
                credential_requirements: Vec::new(),
            },
        })
        .await
