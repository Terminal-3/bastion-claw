...
use chrono::Utc;
use t3claw_approvals::{ApprovalResolver, DenyApproval, LeaseApproval};
use t3claw_authorization::{CapabilityLeaseStatus, CapabilityLeaseStore};
use t3claw_host_api::{
...
use t3claw_run_state::{ApprovalRequestStore, ApprovalStatus};
...
struct RecordingSandboxTransport {
    requests: Mutex<Vec<t3claw_host_runtime::CommandExecutionRequest>>,
}
...
impl t3claw_host_runtime::SandboxCommandTransport for RecordingSandboxTransport {
    async fn run_command(
        &self,
        request: t3claw_host_runtime::CommandExecutionRequest,
...
fn tenant_sandbox_process_policy() -> t3claw_host_api::runtime_policy::EffectiveRuntimePolicy {
    let mut policy = local_dev_policy();
    policy.process_backend = t3claw_host_api::runtime_policy::ProcessBackendKind::TenantSandbox;
    policy
}

fn local_dev_minimal_policy() -> t3claw_host_api::runtime_policy::EffectiveRuntimePolicy {
    let mut policy = local_dev_policy();
...
fn local_dev_minimal_enterprise_policy() -> t3claw_host_api::runtime_policy::EffectiveRuntimePolicy
{
    let mut policy = local_dev_policy();
    policy.resolved_profile =
        t3claw_host_api::runtime_policy::RuntimeProfile::EnterpriseYoloDedicated;
    policy.approval_policy = t3claw_host_api::runtime_policy::ApprovalPolicy::Minimal;
    policy
}
