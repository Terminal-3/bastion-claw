...
mod tests {
    ...
    use t3claw_host_runtime::{SPAWN_SUBAGENT_CAPABILITY_ID, builtin_first_party_package};
    use t3claw_product_workflow::{
        LifecycleProductAction, LifecycleProductContext, LifecycleProductFacade,
        LifecycleProductSurfaceContext, LifecycleReadinessBlocker,
    };
    use t3claw_trust::{HostTrustPolicy, InvalidationBus, TrustPolicy};
    ...
    async fn extension_lifecycle_installs_activates_and_removes_catalog_package() {
        ...
        assert!(
            active
                .get_capability(&t3claw_host_api::CapabilityId::new("fixture.search").unwrap())
                .is_some()
        );
        assert!(
            active
                .get_capability(&t3claw_host_api::CapabilityId::new("fixture.write").unwrap())
                .is_some()
        );
    ...
    async fn extension_activation_updates_local_dev_host_trust_policy() {
        ...
        assert_eq!(
            ...
            t3claw_trust::TrustProvenance::AdminConfig
        );
        assert_eq!(
    ...
    async fn active_extension_trust_policy_is_digest_pinned() {
        ...
        assert_eq!(
            ...
            t3claw_trust::TrustProvenance::Default
        );
    }
    ...
    async fn extension_lifecycle_installs_activates_and_removes_github() {
        ...
        assert!(
            active
                .get_capability(
                    &t3claw_host_api::CapabilityId::new("github.search_issues").unwrap()
                )
                .is_some()
        );
        assert!(
            active
                .get_capability(
                    &t3claw_host_api::CapabilityId::new("github.comment_issue").unwrap()
                )
                .is_some()
        );
    ...
    async fn extension_lifecycle_installs_activates_and_removes_gsuite() {
        ...
        assert!(
            active
                .get_capability(
                    &t3claw_host_api::CapabilityId::new("google-calendar.list_events").unwrap()
                )
                .is_some()
        );
        assert!(
            active
                .get_capability(&t3claw_host_api::CapabilityId::new("gmail.send_message").unwrap())
                .is_some()
        );
    ...
    impl ExtensionInstallationStore for DeleteInstallationFailingStore {
        ...
        async fn update_health(
            &self,
            installation_id: &ExtensionInstallationId,
            health: t3claw_extensions::ExtensionHealthSnapshot,
        ) -> Result<(), ExtensionInstallationError> {
    ...
    impl RootFilesystem for DeleteFailingRootFilesystem {
        fn capabilities(&self) -> t3claw_filesystem::BackendCapabilities {
    ...
    fn test_extension_trust_policy() -> Arc<HostTrustPolicy> {
        Arc::new(
            HostTrustPolicy::new(vec![Box::new(t3claw_trust::AdminConfig::new())])
...
"#
    ...
    fn fixture_manifest_record_with_source(
        ...
        let host_ports =
            t3claw_host_runtime::default_host_port_catalog().expect("host port catalog");
        let contracts =
            t3claw_host_runtime::default_host_api_contract_registry().expect("host API contracts");
