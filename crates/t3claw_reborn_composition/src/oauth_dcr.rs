...
use t3claw_capabilities::CapabilityObligationHandler;
use t3claw_host_api::{
...
use t3claw_product_adapters::AuthPromptChallengeKind;
use t3claw_secrets::{SecretMaterial, SecretStore};
...
impl OAuthDcrProvider {
    ...
    pub(crate) async fn challenge_for_blocked_gate(
        ...
        scope: &TurnScope,
        owner_user_id: &t3claw_host_api::UserId,
        run_id: TurnRunId,
    ...
    pub(crate) async fn start_setup_flow(
        ...
        expires_at: t3claw_auth::Timestamp,
    ) -> Result<t3claw_auth::AuthFlowRecord, AuthProductError> {
    ...
    async fn prepare_flow_material(
        ...
        let pkce_verifier = SecretString::from(t3claw_common::pkce::generate_code_verifier());
...
impl OAuthDcrProviderRegistry {
    ...
    ) -> Result<Option<t3claw_auth::AuthFlowRecord>, AuthProductError> {
...
struct PreparedDcrFlow {
    authorization_url: t3claw_auth::OAuthAuthorizationUrl,
    opaque_state_hash: t3claw_auth::OpaqueStateHash,
    pkce_verifier_hash: t3claw_auth::PkceVerifierHash,
    pkce_verifier: SecretString,
...
pub(crate) struct DcrGateChallengeRequest<'a> {
    ...
    pub(crate) owner_user_id: &'a t3claw_host_api::UserId,
...
pub(crate) struct DcrSetupFlowRequest {
    ...
    pub(crate) expires_at: t3claw_auth::Timestamp,
}
...
struct DcrOAuthCallbackStateWire {
    ...
    session_id: Option<t3claw_auth::AuthSessionId>,
    provider: AuthProviderId,
    account_label: CredentialAccountLabel,
    requested_scopes: Vec<ProviderScope>,
    nonce: String,
...
impl DcrOAuthCallbackState {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            account_label,
            requested_scopes,
            nonce: t3claw_common::pkce::generate_code_verifier(),
        }
    }
...
impl DcrOAuthCallbackState {
    ...
    pub(crate) fn decode(raw: &str) -> Result<Self, AuthProductError> {
        ...
        let mut scope = AuthProductScope::new(wire.resource, t3claw_auth::AuthSurface::Callback);
...
fn auth_scope_for_blocked_turn(
    ...
    AuthProductScope::new(
        ...
        t3claw_auth::AuthSurface::Callback,
    )
}

fn challenge_view_from_flow(
    flow: &t3claw_auth::AuthFlowRecord,
) -> Result<AuthChallengeView, AuthProductError> {
...
mod tests {
    ...
    async fn dcr_provider_creates_blocked_gate_flow_and_stores_pkce_material() {
        let provider = OAuthDcrProvider::new(
            OAuthDcrProviderConfig {
                spec: HostOAuthProviderSpec {
                    provider_id: "notion",
                    capability_id: "t3claw_auth.notion_oauth",
            ...
            Arc::new(t3claw_secrets::InMemorySecretStore::new()),
            Arc::new(TestObligationHandler),
        )
        .unwrap();
        let auth = Arc::new(t3claw_auth::InMemoryAuthProductServices::new());
        let flow_manager: Arc<dyn AuthFlowManager> = auth.clone();
        let flow_source: Arc<dyn AuthFlowRecordSource> = auth.clone();
        let scope = TurnScope::new(
            t3claw_host_api::TenantId::new("tenant").unwrap(),
            Some(t3claw_host_api::AgentId::new("agent").unwrap()),
            Some(t3claw_host_api::ProjectId::new("project").unwrap()),
            t3claw_host_api::ThreadId::new("thread").unwrap(),
        );
        let owner = t3claw_host_api::UserId::new("user").unwrap();
    ...
    async fn dcr_provider_setup_flow_rejects_scope_mismatch() {
        ...
        assert_eq!(error.code(), t3claw_auth::AuthErrorCode::BackendUnavailable);
        assert!(
    ...
    fn dcr_oauth_callback_state_rejects_missing_prefix_or_corrupt_payload() {
        ...
        assert_eq!(
            missing_prefix.code(),
            t3claw_auth::AuthErrorCode::MalformedCallback
        );
        assert_eq!(
    ...
    async fn dcr_provider_setup_flow_cleans_up_material_and_cancels_flow_when_client_material_write_fails()
     {
        ...
        assert_eq!(
            flows[0].status,
            t3claw_auth::AuthFlowStatus::Canceled,
    ...
    fn callback_redirect_uri_carries_existing_callback_query_fields() {
        ...
        let scope = AuthProductScope::new(
            ResourceScope {
                tenant_id: t3claw_host_api::TenantId::new("tenant").unwrap(),
                user_id: t3claw_host_api::UserId::new("user").unwrap(),
                agent_id: None,
                project_id: None,
                mission_id: None,
                thread_id: Some(t3claw_host_api::ThreadId::new("thread").unwrap()),
                invocation_id: InvocationId::new(),
            },
            t3claw_auth::AuthSurface::Callback,
        );
    ...
    fn sample_auth_scope() -> AuthProductScope {
        AuthProductScope::new(
            ResourceScope {
                tenant_id: t3claw_host_api::TenantId::new("tenant").unwrap(),
                user_id: t3claw_host_api::UserId::new("user").unwrap(),
                agent_id: Some(t3claw_host_api::AgentId::new("agent").unwrap()),
                project_id: Some(t3claw_host_api::ProjectId::new("project").unwrap()),
                mission_id: None,
                thread_id: Some(t3claw_host_api::ThreadId::new("thread").unwrap()),
                invocation_id: InvocationId::new(),
            },
    ...
    impl RuntimeHttpEgress for DcrDiscoveryEgress {
        async fn execute(
            ...
            t3claw_host_api::RuntimeHttpEgressResponse,
            t3claw_host_api::RuntimeHttpEgressError,
        > {
            ...
            Ok(t3claw_host_api::RuntimeHttpEgressResponse {
                status,
                headers: Vec::new(),
    ...
    struct SecondPutFailingSecretStore {
        inner: t3claw_secrets::InMemorySecretStore,
    ...
    impl SecondPutFailingSecretStore {
        fn new() -> Self {
            Self {
                inner: t3claw_secrets::InMemorySecretStore::new(),
    ...
    impl SecretStore for SecondPutFailingSecretStore {
        async fn put(
            ...
            handle: SecretHandle,
            material: SecretMaterial,
        ) -> Result<t3claw_secrets::SecretMetadata, t3claw_secrets::SecretStoreError> {
            self.put_handles
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner())
                .push(handle.clone());
            if self.put_count.fetch_add(1, Ordering::SeqCst) == 1 {
                return Err(t3claw_secrets::SecretStoreError::StoreUnavailable {
        ...
        async fn metadata(
            ...
            scope: &ResourceScope,
            handle: &SecretHandle,
        ) -> Result<Option<t3claw_secrets::SecretMetadata>, t3claw_secrets::SecretStoreError>
        {
        ...
        async fn delete(
            ...
            scope: &ResourceScope,
            handle: &SecretHandle,
        ) -> Result<bool, t3claw_secrets::SecretStoreError> {
            self.deleted_handles
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner())
                .push(handle.clone());
        ...
        async fn lease_once(
            ...
            scope: &ResourceScope,
            handle: &SecretHandle,
        ) -> Result<t3claw_secrets::SecretLease, t3claw_secrets::SecretStoreError> {
        ...
        async fn consume(
            &self,
            scope: &ResourceScope,
            lease_id: t3claw_secrets::SecretLeaseId,
        ) -> Result<SecretMaterial, t3claw_secrets::SecretStoreError> {
        ...
        async fn leases_for_scope(
            &self,
            scope: &ResourceScope,
        ) -> Result<Vec<t3claw_secrets::SecretLease>, t3claw_secrets::SecretStoreError> {
    ...
    impl AuthFlowManager for BackendUnavailableFlowManager {
        ...
        async fn claim_oauth_callback(
            &self,
            _scope: &AuthProductScope,
            _request: t3claw_auth::OAuthCallbackClaimRequest,
        ) -> Result<t3claw_auth::AuthFlowRecord, AuthProductError> {
        ...
        async fn complete_oauth_callback(
            &self,
            _scope: &AuthProductScope,
            _input: t3claw_auth::OAuthCallbackInput,
        ) -> Result<t3claw_auth::AuthFlowRecord, AuthProductError> {
        ...
        async fn complete_credential_selection(
            &self,
            _scope: &AuthProductScope,
            _input: t3claw_auth::CredentialSelectionInput,
        ) -> Result<t3claw_auth::AuthFlowRecord, AuthProductError> {
        ...
        async fn complete_manual_token(
            &self,
            _scope: &AuthProductScope,
            _input: t3claw_auth::ManualTokenCompletionInput,
        ) -> Result<t3claw_auth::AuthFlowRecord, AuthProductError> {
        ...
        async fn cancel_manual_token(
            &self,
            _scope: &AuthProductScope,
            _interaction_id: t3claw_auth::AuthInteractionId,
        ) -> Result<Option<t3claw_auth::AuthFlowRecord>, AuthProductError> {
        ...
        async fn fail_oauth_callback(
            &self,
            _scope: &AuthProductScope,
            _input: t3claw_auth::OAuthCallbackFailureInput,
        ) -> Result<t3claw_auth::AuthFlowRecord, AuthProductError> {
        ...
        async fn mark_continuation_dispatched(
            ...
            _scope: &AuthProductScope,
            _flow_id: AuthFlowId,
            _emitted_at: t3claw_auth::Timestamp,
        ) -> Result<t3claw_auth::AuthFlowRecord, AuthProductError> {
    ...
    impl CapabilityObligationHandler for TestObligationHandler {
        async fn satisfy(
            &self,
            _request: t3claw_capabilities::CapabilityObligationRequest<'_>,
        ) -> Result<(), t3claw_capabilities::CapabilityObligationError> {
            Ok(())
        }
    }
}
