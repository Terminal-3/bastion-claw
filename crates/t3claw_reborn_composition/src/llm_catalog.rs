...
//!    `$T3CLAW_REBORN_HOME/providers.json` (same JSON shape as
//!    v1's `~/.t3claw/providers.json`). Loaded here via
//!    `t3claw_llm::ProviderRegistry::load_from_path`.
//! 2. **Selection** — boot TOML's `[llm.<slot>]` section, parsed by
//!    `t3claw_reborn_config::LlmSlotSelection`. "Use provider X
//!    for the `default` slot, with model Y."
//! 3. **Runtime config** — derived here. The resolved `ProviderDefinition`
//!    plus the selection's overrides becomes an `t3claw_llm::LlmConfig`
...
pub enum RebornLlmCatalogError {
    ...
    #[error(
        "llm provider id `{requested}` not found in the provider catalog \
         (compiled-in + $T3CLAW_REBORN_HOME/providers.json); known ids: [{known}]"
    )]
    ...
    CatalogLoad {
        #[source]
        source: t3claw_llm::registry::ProviderRegistryLoadError,
    },
...
/// `t3claw_llm` and never leak into CLI commands.
...
fn resolve_llm_from_env(
    ...
    t3claw_llm::resolve_llm_config_from_env(Some(boot.home().providers_file_path().as_path()))
...
/// 4. Let `t3claw_llm` resolve provider fields and build the full config.
pub fn resolve_llm_selection_against_catalog(
    selection: &LlmSlotSelection,
    user_providers_path: Option<&Path>,
) -> Result<t3claw_llm::LlmConfig, RebornLlmCatalogError> {
...
pub fn resolve_against_registry(
    ...
    let resolved = t3claw_llm::resolve_provider_config_from_selection(
        ProviderSelection {
    ...
    t3claw_llm::build_llm_config_from_resolved_provider(resolved)
        .map_err(|source| RebornLlmCatalogError::EnvResolution { source })
}
...
pub(crate) fn apply_stored_api_key(config: &mut t3claw_llm::LlmConfig, key: secrecy::SecretString) {
