...
//!    [`t3claw_hooks::manifest::HookManifestEntry`], and installs them
//!    through [`t3claw_hooks::registrar::HookRegistrar::install`] at the
...
//! [`t3claw_hooks::evaluator::PredicateEvaluator`] + its state backend, the
...
//! ([`t3claw_hooks::evaluator::PredicateEvaluator::with_state_backend`]) so
...
///
/// Re-exported from `t3claw_reborn` so the type is identical to the one
...
pub use t3claw_reborn::loop_driver_host::HookDispatcherBuilderFactory;
...
impl HooksActivationConfig {
    ...
    pub fn from_env() -> Self {
        Self::from_env_values(
            t3claw_common::env_helpers::env_or_override(HOOKS_ENABLED_ENV),
            t3claw_common::env_helpers::env_or_override(HOOKS_THIRD_PARTY_ENABLED_ENV),
        )
    }
