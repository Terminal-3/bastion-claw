...
use t3claw_hooks::manifest::HookManifestEntry;
use t3claw_hooks::predicate_state::{InMemoryPredicateStateBackend, PredicateStateBackend};
...
fn reborn_replay_error(
    reason: String,
) -> t3claw_reborn::loop_driver_host::RebornLoopDriverHostError {
    t3claw_reborn::loop_driver_host::RebornLoopDriverHostError::InvalidRequest { reason }
}
...
/// vocabulary (clean-boundary contract: `t3claw_extensions` stays free of
...
/// bypass the registrar's ceiling + attribution. An `t3claw_architecture`
...
pub(super) fn project_extension_hook_sets(
    ...
    tenant_root: Option<&t3claw_host_api::VirtualPath>,
...
pub(super) fn project_hook_entries(
    extension_id: &str,
    hooks: &[t3claw_extensions::HookSectionEntryV2],
...
pub(super) fn build_hook_dispatcher_builder_factory_with<F>(
    config: HooksActivationConfig,
    registry: &HookProjectionRegistry,
    tenant_context: Option<(&t3claw_host_api::TenantId, &t3claw_host_api::VirtualPath)>,
        ...
        + 'static,
{
    ...
    let fallback_tenant =
        t3claw_host_api::TenantId::new("reborn-hook-projection").map_err(|error| {
            RebornBuildError::InvalidConfig {
                reason: format!("could not build fallback audit tenant id: {error}"),
            }
        })?;
    let (audit_tenant, audit_root): (
        &t3claw_host_api::TenantId,
        Option<&t3claw_host_api::VirtualPath>,
