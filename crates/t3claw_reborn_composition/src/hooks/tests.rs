...
use t3claw_extensions::v2::ManifestSource;
use t3claw_extensions::{ExtensionManifest, ExtensionPackage};
use t3claw_hooks::HookPhase;
use t3claw_hooks::identity::{HookId, HookVersion};
use t3claw_hooks::points::ObserverHookContext;
use t3claw_hooks::registry::HookPointSpec;
use t3claw_hooks::sink::{ObserverHook, ObserverSink};
use t3claw_host_api::{HostPortCatalog, VirtualPath};
...
const TEST_NOOP_OBSERVER_CANONICAL_PATH: &str =
    "t3claw_reborn_composition::hooks::tests::NoOpObserverHook";
...
use t3claw_hooks::identity::{ExtensionId as HookExtensionId, HookLocalId};
...
body = { mode = "nonsense" }
"#;
    // HostBundled ids are reserved to the `t3claw.` prefix.
    let registry = projection(registry_with_manifest_source(
        "t3claw.broken",
        &manifest_toml("t3claw.broken", hooks_block),
    ...
    match build_hook_dispatcher_builder_factory(HooksActivationConfig::enabled(), &registry) {
        Err(RebornBuildError::InvalidConfig { reason }) => {
            assert!(
                reason.contains("t3claw.broken") && reason.contains("broken-hook"),
...
fn tenant_root_is_the_fixed_system_extensions_root() {
    let a = tenant_extension_root(&t3claw_host_api::TenantId::new("alpha").expect("a"))
        .expect("root a");
    let b =
        tenant_extension_root(&t3claw_host_api::TenantId::new("beta").expect("b")).expect("root b");
...
async fn third_party_subflag_off_yields_builtin_only_projection() {
    ...
    let tenant = t3claw_host_api::TenantId::new("alpha").expect("tenant");
...
/// contracts require the `t3claw.capability_provider/v1` host_api form for
...
async fn write_manifest<F: t3claw_filesystem::RootFilesystem>(fs: &F, id: &str, body: &str) {
    fs.write_file(
        &t3claw_host_api::VirtualPath::new(format!("/system/extensions/{id}/manifest.toml"))
...
async fn failed_merge_does_not_consume_hook_budget() {
    ...
    let contracts = t3claw_host_runtime::default_host_api_contract_registry()
...
"#;
    ...
    let real_tenant = t3claw_host_api::TenantId::new("acme-real-tenant").expect("tenant");
