...
pub(super) struct HookProjection {
    pub(super) extension_id: t3claw_host_api::ExtensionId,
    ...
    pub(super) source: t3claw_extensions::ManifestSource,
    /// Package root, for the projection-layer containment check only.
    pub(super) root: t3claw_host_api::VirtualPath,
    /// The declared `[[hooks]]` payloads — the ONLY package content carried.
    pub(super) hooks: Vec<t3claw_extensions::HookSectionEntryV2>,
}
...
impl HookProjection {
    ...
    pub(super) fn from_package(package: &t3claw_extensions::ExtensionPackage) -> Option<Self> {
...
/// [`t3claw_host_runtime::HostRuntimeServices::new`] (it becomes the
...
///
/// The discovery layer ([`t3claw_extensions::ExtensionDiscovery`] /
/// [`t3claw_extensions::ExtensionPackage::from_manifest`]) hardcodes package
...
/// per-tenant [`t3claw_filesystem::ScopedFilesystem`] / per-identity backend,
...
pub fn tenant_extension_root(
    _tenant_id: &t3claw_host_api::TenantId,
) -> Result<t3claw_host_api::VirtualPath, RebornBuildError> {
    t3claw_host_api::VirtualPath::new("/system/extensions").map_err(|error| {
...
pub(super) fn enforce_root_containment(
    tenant_root: &t3claw_host_api::VirtualPath,
    package_root: &t3claw_host_api::VirtualPath,
) -> Result<(), String> {
...
pub struct ThirdPartyDiscoveryInput<'a, F: t3claw_filesystem::RootFilesystem> {
    pub filesystem: &'a F,
    pub tenant_id: &'a t3claw_host_api::TenantId,
}
...
where
    F: t3claw_filesystem::RootFilesystem,
{
        ...
        && let Some(input) = third_party_input
    {
        ...
        let discovered = match t3claw_host_runtime::discover_extensions_tolerant_bounded(
            input.filesystem,
            &root,
