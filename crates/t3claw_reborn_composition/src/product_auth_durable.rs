...
use t3claw_host_api::{ResourceScope, ScopedPath};
...
// `t3claw_secrets::CredentialAccountStore` so the runtime credential
// broker shares one source of truth with the product-auth UX layer.
//
// Today two `CredentialAccount` records coexist:
//   * `t3claw_auth::CredentialAccount` — product-auth UX record stored
...
//   * `t3claw_secrets::CredentialAccount` — runtime broker record
    ...
    F: RootFilesystem,
{
    ...
    async fn read_flow(
        &self,
        scope: &t3claw_auth::AuthProductScope,
        flow_id: AuthFlowId,
    ...
    async fn flow_records_for_owner(
        ...
        for surface in AuthSurface::ALL {
            let scope = t3claw_auth::AuthProductScope::new(resource.clone(), surface);
            flows.extend(
            ...
            for entry in entries {
                ...
                let mut session_scope =
                    t3claw_auth::AuthProductScope::new(resource.clone(), surface);
    ...
    async fn account_scopes_for_owner(
    ...
    ) -> Result<Vec<t3claw_auth::AuthProductScope>, AuthProductError> {
        let resource = ResourceScope {
            tenant_id: owner.tenant_id.clone(),
            user_id: owner.user_id.clone(),
            agent_id: owner.agent_id.clone(),
        ...
        for surface in AuthSurface::ALL {
            scopes.push(t3claw_auth::AuthProductScope::new(
                resource.clone(),
                surface,
            ));
...
use t3claw_auth::{credential_status_for_completed_flow, is_terminal_status, scope_matches};
