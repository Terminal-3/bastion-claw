...
use t3claw_turns::{TurnPersistenceSnapshot, TurnStatus};
...
) -> HashMap<(t3claw_host_api::TenantId, t3claw_turns::TurnRunId), TriggerActiveRunState> {
    snapshot
        .runs
        .iter()
        .map(|run| {
...
fn active_run_state_from_index(
    run_index: &HashMap<
        (t3claw_host_api::TenantId, t3claw_turns::TurnRunId),
...
impl<F> TriggerTurnSnapshotSource
    for LocalTriggerTurnSnapshotSource<t3claw_turns::FilesystemTurnStateStore<F>>
where
    F: t3claw_filesystem::RootFilesystem + Send + Sync + 'static,
{
    async fn snapshot(&self) -> Result<TurnPersistenceSnapshot, TriggerError> {
        self.store
...
impl TriggerTurnSnapshotSource
    for LocalTriggerTurnSnapshotSource<t3claw_turns::InMemoryTurnStateStore>
{
    async fn snapshot(&self) -> Result<TurnPersistenceSnapshot, TriggerError> {
...
mod tests {
    ...
    use t3claw_triggers::{TriggerId, TriggerPollerWorkerConfig};
    use t3claw_turns::{
    ...
    fn turn_run_record(
        ...
        let scope = TurnScope::new(
            ...
            None,
            None,
            t3claw_host_api::ThreadId::new(format!("thread-{run_id}")).expect("thread id"),
        );
        TurnRunRecord {
            run_id,
            turn_id: TurnId::new(),
            ...
            reply_target_binding_ref: t3claw_turns::ReplyTargetBindingRef::new(format!(
                "reply:{run_id}"
            ))
