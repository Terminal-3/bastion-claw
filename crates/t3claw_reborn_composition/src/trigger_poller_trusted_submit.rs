...
use t3claw_host_api::{AgentId, ProjectId, TenantId, Timestamp, UserId};
use t3claw_product_workflow::automation_trigger_thread_metadata_json;
use t3claw_safety::{
...
use t3claw_turns::{AdmissionRejectionReason, TurnError};
    ...
    B: ConversationBindingService,
{
    async fn materialize_prompt(
        ...
        .await
        .map_err(classify_materializer_inbound_error)?;
        let content_ref = t3claw_triggers::TriggerInboundContentRef::new(format!(
            "thread-message:{}",
            accepted.message_id
        ))?;
...
) -> Result<t3claw_threads::AcceptedInboundMessage, InboundTurnError> {
...
mod tests {
    ...
    use t3claw_host_api::{ProjectId, TenantId, ThreadId, UserId};
    use t3claw_product_workflow::AUTOMATION_TRIGGER_THREAD_SOURCE_TAG;
    use t3claw_safety::{InjectionWarning, Severity};
    use t3claw_threads::{
    ...
    impl ConversationBindingService for PanicBindingService {
        ...
        async fn link_conversation_to_thread(
            &self,
            _request: t3claw_conversations::LinkConversationRequest,
        ) -> Result<t3claw_conversations::LinkedConversationBinding, InboundTurnError> {
            panic!("foreign-tenant materialization must reject before conversation linking")
        }

        async fn validate_reply_target(
            &self,
            _request: t3claw_conversations::ValidateReplyTargetRequest,
        ) -> Result<t3claw_conversations::ReplyTargetBinding, InboundTurnError> {
    ...
    impl CanonicalSessionThreadService for InterceptingPromptThreadService {
        ...
        ) -> Result<t3claw_threads::ThreadHistory, SessionThreadError> {
    ...
    fn thread_busy_inbound_errors_are_retryable_backend_failures() {
        let error = classify_materializer_inbound_error(InboundTurnError::TurnSubmissionFailed {
            error: TurnError::ThreadBusy(t3claw_turns::ThreadBusy {
    ...
    fn retryable_turn_errors_are_backend_failures() {
        for error in [
            ...
            TurnError::CapacityExceeded {
                resource: t3claw_turns::TurnCapacityResource::SubmitTurn,
                cap: 1,
            },
            TurnError::Conflict {
    ...
    async fn unsafe_trigger_prompt_is_rejected_before_turn_submission() {
        let conversations = t3claw_conversations::InMemoryConversationServices::default();
