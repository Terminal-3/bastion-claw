...
use t3claw_wasm_product_adapters::{EgressPolicy, EgressPolicyError, EgressPolicyTarget};
use thiserror::Error;
...
fn slack_extension_id() -> Result<ExtensionId, ProtocolHttpEgressError> {
    ExtensionId::new("t3claw_slack").map_err(|error| ProtocolHttpEgressError::PolicyDenied {
...
fn map_runtime_http_error(error: RuntimeHttpEgressError) -> ProtocolHttpEgressError {
    match error.reason_code() {
        t3claw_host_api::RuntimeHttpEgressReasonCode::PolicyDenied
        | t3claw_host_api::RuntimeHttpEgressReasonCode::RequestDenied => {
            ProtocolHttpEgressError::PolicyDenied {
                reason: RedactedString::new(error.stable_runtime_reason()),
            }
        }
        t3claw_host_api::RuntimeHttpEgressReasonCode::ResponseBodyLimitExceeded => {
            ProtocolHttpEgressError::LeakDetected
        }
        t3claw_host_api::RuntimeHttpEgressReasonCode::CredentialUnavailable
        | t3claw_host_api::RuntimeHttpEgressReasonCode::NetworkError
        | t3claw_host_api::RuntimeHttpEgressReasonCode::ResponseError => {
