...
use t3claw_host_runtime::RuntimeCredentialAccountRequest;
use t3claw_host_runtime::RuntimeCredentialAccountResolver;
use t3claw_secrets::{InMemorySecretStore, SecretStore};
...
fn test_filesystem() -> Arc<ScopedFilesystem<InMemoryBackend>> {
    let mounts = t3claw_host_api::MountView::new(vec![MountGrant::new(
...
async fn filesystem_account_record_source_rejects_malformed_scan_records() {
    ...
    let malformed_account_id = t3claw_auth::CredentialAccountId::new();
    let malformed_path = super::paths::account_path(&scope, malformed_account_id)
        .expect("account path derivation must succeed");
    let malformed = t3claw_filesystem::Entry::bytes(b"{ malformed account json".to_vec())
        .with_content_type(t3claw_filesystem::ContentType::json());
    filesystem
        .put(
            &scope.resource,
            &malformed_path,
            malformed,
            t3claw_filesystem::CasExpectation::Absent,
        )
        .await
    ...
    let completed = service
        .complete_oauth_callback(
            ...
            OAuthCallbackInput {
                flow_id: flow.id,
                opaque_state_hash: state_hash("state"),
                outcome: t3claw_auth::ProviderCallbackOutcome::Authorized {
...
fn fs_error_maps_version_mismatch_to_backend_conflict() {
    ...
    use t3claw_filesystem::{FilesystemError, FilesystemOperation};
    use t3claw_host_api::VirtualPath;
...
async fn filesystem_cleanup_for_lifecycle_deactivates_owner_and_revokes_on_uninstall() {
    use t3claw_auth::{SecretCleanupAction, SecretCleanupRequest, SecretCleanupService};
    ...
    let account = service
        .create_account(t3claw_auth::NewCredentialAccount {
            scope: scope.clone(),
            provider: google_provider(),
            label: account_label(),
            status: CredentialAccountStatus::Configured,
...
async fn unavailable_auth_provider_client_validates_before_returning_backend_unavailable() {
    ...
    let valid = OAuthProviderCallbackRequest {
        ...
        authorization_code_hash: code_hash("c"),
        pkce_verifier: t3claw_auth::PkceVerifierSecret::new(SecretString::from("real-verifier"))
            .unwrap(),
        pkce_verifier_hash: pkce_hash("p"),
        scopes: vec![],
    };
...
async fn filesystem_oauth_reauth_purges_previous_provider_secrets() {
    ...
    use t3claw_auth::{CredentialAccountUpdateBinding, ProviderCallbackOutcome};
...
async fn filesystem_manual_token_submit_cleans_up_secret_when_account_write_fails() {
    ...
    use t3claw_auth::CredentialAccountId;
    use t3claw_filesystem::CasExpectation;
    ...
    let dummy_account = t3claw_auth::CredentialAccount {
        id: account_id,
        scope: scope.clone(),
        provider: google_provider(),
        label: account_label(),
        status: t3claw_auth::CredentialAccountStatus::Configured,
        ownership: CredentialOwnership::UserReusable,
        owner_extension: None,
        granted_extensions: vec![],
        access_secret: None,
    ...
    use t3claw_filesystem::{ContentType, Entry};
...
async fn filesystem_fail_oauth_callback_marks_flow_failed() {
    use t3claw_auth::{AuthErrorCode, OAuthCallbackFailureInput};
    let filesystem = test_filesystem();
    let secret_store: Arc<dyn SecretStore> = Arc::new(InMemorySecretStore::new());
    let scope = test_scope();
    let service = test_service(filesystem, secret_store);
...
async fn filesystem_complete_credential_selection_completes_flow() {
    use t3claw_auth::{AuthFlowKind, CredentialSelectionInput};
    let filesystem = test_filesystem();
    let secret_store: Arc<dyn SecretStore> = Arc::new(InMemorySecretStore::new());
    let scope = test_scope();
    let service = test_service(filesystem, secret_store);
...
async fn filesystem_create_flow_rejects_invalid_update_binding() {
    use t3claw_auth::CredentialAccountUpdateBinding;
    let filesystem = test_filesystem();
    let secret_store: Arc<dyn SecretStore> = Arc::new(InMemorySecretStore::new());
    let scope = test_scope();
    let service = test_service(filesystem, secret_store);
...
async fn filesystem_project_credential_recovery_returns_setup_required_when_empty() {
    use t3claw_auth::CredentialRecoveryRequest;
    let filesystem = test_filesystem();
    let secret_store: Arc<dyn SecretStore> = Arc::new(InMemorySecretStore::new());
    let scope = test_scope();
    let service = test_service(filesystem, secret_store);
    ...
    let recovery = service
        ...
        .await
        .unwrap();
    use t3claw_auth::CredentialRecoveryState;
    assert!(
        matches!(
            recovery.state,
...
async fn filesystem_credential_setup_service_update_path() {
    use t3claw_auth::{CredentialAccountMutation, CredentialAccountUpdate, CredentialSetupService};
    let filesystem = test_filesystem();
    let secret_store: Arc<dyn SecretStore> = Arc::new(InMemorySecretStore::new());
    let scope = test_scope();
    let service = test_service(filesystem, secret_store);
...
async fn filesystem_oauth_cas_conflict_branch_purges_previous_secrets() {
    ...
    use t3claw_auth::ProviderCallbackOutcome;
    use t3claw_secrets::SecretMaterial;
