...
//! native surface.
//!
//! `t3claw_webui_v2::webui_v2_routes()` returns an
...
use lru::LruCache;
use t3claw_host_api::ingress::{IngressRouteDescriptor, RateLimitPolicy, RateLimitScope};
    ...
    let bucket_key = match scope {
        RateLimitScope::PerCaller => {
            let Some(caller) = request.extensions().get::<WebUiAuthenticatedCaller>() else {
                tracing::debug!(
                    target = "t3claw::reborn::webui_rate_limit",
...
mod tests {
    use super::*;
    use t3claw_host_api::{TenantId, UserId};
    ...
    fn build_rate_limit_state_accepts_webui_v2_descriptors() {
        let descriptors = t3claw_webui_v2::webui_v2_routes();
