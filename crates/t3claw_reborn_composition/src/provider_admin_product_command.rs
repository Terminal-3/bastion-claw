...
fn config_load_error_reason(error: &t3claw_reborn_config::RebornConfigFileError) -> String {
    match error {
        t3claw_reborn_config::RebornConfigFileError::Io { source, .. } => {
            format!("read failed: {source}")
        }
        t3claw_reborn_config::RebornConfigFileError::Toml { source, .. } => {
            format!("TOML parse failed: {source}")
        }
        t3claw_reborn_config::RebornConfigFileError::IncompatibleApiVersion {
            found,
            expected,
            ..
        } => {
        ...
        t3claw_reborn_config::RebornConfigFileError::InlineSecret { source, .. } => {
            format!("field validation failed: {source}")
        }
        t3claw_reborn_config::RebornConfigFileError::InvalidField { field, reason, .. } => {
            format!("field `{field}` validation failed: {reason}")
        }
        t3claw_reborn_config::RebornConfigFileError::InvalidApiVersion {
            found, reason, ..
        } => {
...
fn config_update_error_reason(error: &t3claw_reborn_config::RebornConfigFileUpdateError) -> String {
    match error {
        t3claw_reborn_config::RebornConfigFileUpdateError::Lock { source, .. } => {
            format!("lock failed: {source}")
        }
        t3claw_reborn_config::RebornConfigFileUpdateError::Read { source, .. } => {
            format!("read failed: {source}")
        }
        t3claw_reborn_config::RebornConfigFileUpdateError::Parse { source, .. } => {
            format!("TOML parse failed: {source}")
        }
        t3claw_reborn_config::RebornConfigFileUpdateError::Validate { source, .. } => {
            format!("validation failed: {}", config_load_error_reason(source))
        }
        t3claw_reborn_config::RebornConfigFileUpdateError::Write { source, .. } => {
