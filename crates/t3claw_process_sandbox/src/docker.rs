...
fn next_container_name(phase: SandboxProcessPhase) -> String {
    ...
    format!("t3claw-sandbox-{phase}-{}-{sequence}", std::process::id())
}
...
impl<'a> DockerPhaseSpec<'a> {
    ...
    fn network_args(&self, config: &DockerProcessSandboxConfig) -> Vec<String> {
        ...
        if self.brokered_run() {
            args.extend([
                "--env".to_string(),
                "T3CLAW_EGRESS_LOCKDOWN=broker-only".to_string(),
            ]);
        }
        args
    }
    ...
    fn env_args(
            ...
            && let Some(broker) = &config.broker
        {
            ...
            env.insert("T3CLAW_BROKER_PROXY".to_string(), broker.proxy_url.clone());
        }
