...
- Read `Cargo.toml` for dependencies (internally only `t3claw_common` and `t3claw_skills`) and note the ReDoS guardrail at the top of `src/lib.rs`: do not add `fancy-regex` without redesigning `executor/orchestrator.rs::__regex_match__`.
...
- A dependency on the main `t3claw` crate, product transport/channels, or UI behavior — the engine is testable in isolation.
...
- Fast local check: `cargo test -p t3claw_engine`
- Lint: `cargo clippy -p t3claw_engine --all-targets -- -D warnings`
