...
/// `T3CLAW_ORCHESTRATOR_MAX_DURATION_SECS`. Cached for the process lifetime.
fn orchestrator_max_duration() -> std::time::Duration {
    static CACHED: OnceLock<std::time::Duration> = OnceLock::new();
    *CACHED.get_or_init(|| {
        let secs = std::env::var("T3CLAW_ORCHESTRATOR_MAX_DURATION_SECS")
            .ok()
...
fn classify_orchestrator_failure(prefix: &str, err_msg: &str) -> OrchestratorFailure {
    ...
    // `T3CLAW_ORCHESTRATOR_MAX_DURATION_SECS`, which is wrong advice for
...
/// argument threaded through `t3claw_skills::prefilter_skills` on
...
mod tests {
    ...
    fn failure_reason_maps_timeout_to_user_safe_message() {
        ...
        assert!(
            rendered.contains("T3CLAW_ORCHESTRATOR_MAX_DURATION_SECS"),
    ...
    /// `T3CLAW_ORCHESTRATOR_MAX_DURATION_SECS` — completely wrong for a
    ...
    // Rust unit test suite in crates/t3claw_llm/src/reasoning.rs.
