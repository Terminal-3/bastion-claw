...
//! `t3claw_product_adapters` contract that says NoOps must be a
...
fn build_payload(
    ...
    if let Some((command, arguments)) = extract_first_bot_command(&message, policy) {
        ...
        // `t3claw_product_adapters` validation fires on the
...
mod tests {
    use super::*;
    use t3claw_product_adapters::ProductAdapterId;
    use t3claw_product_adapters::auth::mark_shared_secret_header_verified;
    ...
    fn command_arguments_with_control_char_rejected_via_shared_validation() {
        ...
        // and the shared `t3claw_product_adapters` validation
    ...
    fn command_arguments_exceeding_byte_limit_rejected_via_shared_validation() {
        ...
        // per `t3claw_product_adapters::inbound`) and assert the
    ...
    fn group_media_caption_mention_is_recognized_as_bot_mention() {
        ...
        // `@t3claw_bot help` carries its mention in
        ...
        let payload = br#"{
            ...
            "message": {
                ...
                "reply_to_message": {
                    "message_id": 7,
                    "date": 1699999999,
                    "from": {"id": 9000, "is_bot": true, "first_name": "T3Claw"},
                    "chat": {"id": -42, "type": "supergroup"},
                    "text": "hi there"
                }
            }
