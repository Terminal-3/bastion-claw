...
use t3claw_product_adapters::redaction::RedactedString;
        ...
        .map_err(|err| match err {
            crate::payload::PayloadParseError::UnauthenticatedPayload => {
                ProductAdapterError::Authentication(
                    t3claw_product_adapters::ProtocolAuthFailure::Missing,
                )
            }
            crate::payload::PayloadParseError::InvalidJson { reason } => {
                ProductAdapterError::MalformedInboundPayload {
                    reason: t3claw_product_adapters::redaction::RedactedString::new(reason),
                }
            }
            crate::payload::PayloadParseError::MissingUpdateId => {
                ProductAdapterError::MalformedInboundPayload {
                    reason: t3claw_product_adapters::redaction::RedactedString::new(
...
fn egress_err_to_delivery_status(
    ...
    attempt_id: t3claw_product_adapters::DeliveryAttemptId,
...
fn map_egress_error(err: ProtocolHttpEgressError) -> ProductAdapterError {
    let reason = t3claw_product_adapters::redaction::RedactedString::new(err.to_string());
    match err {
        ProtocolHttpEgressError::Timeout
        | ProtocolHttpEgressError::Network(_)
...
mod tests {
    use super::*;
    use t3claw_product_adapters::FakeProtocolHttpEgress;
    ...
    fn test_outbound_target() -> t3claw_product_adapters::ProductOutboundTarget {
        let reply = crate::render::build_reply_target_binding(-100, Some(7), Some(42));
        let conv = t3claw_product_adapters::ExternalConversationRef::new(
            None,
            "-100",
            None::<&str>,
            None::<&str>,
        ...
        .expect("valid");
        t3claw_product_adapters::ProductOutboundTarget::new(reply, conv, None)
    }

    fn test_outbound_target_no_topic_no_reply() -> t3claw_product_adapters::ProductOutboundTarget {
    ...
    fn test_projection_cursor() -> t3claw_product_adapters::ProjectionCursor {
        t3claw_product_adapters::ProjectionCursor::new("test-cursor").expect("valid")
    }
    ...
    fn parse_inbound_refuses_unverified_evidence() {
        ...
        let unverified = ProtocolAuthEvidence::failed(
            t3claw_product_adapters::ProtocolAuthFailure::SharedSecretMismatch,
        );
        let err = adapter
    ...
    async fn render_outbound_final_reply_uses_constrained_egress() {
        ...
        let sink = t3claw_product_adapters::FakeOutboundDeliverySink::new();
        let envelope = ProductOutboundEnvelope {
            ...
            payload: ProductOutboundPayload::FinalReply(t3claw_product_adapters::FinalReplyView {
                turn_run_id: t3claw_turns::TurnRunId::new(),
                text: "hi".into(),
    ...
    async fn render_outbound_progress_skipped_when_capability_off() {
        ...
        let envelope = ProductOutboundEnvelope {
            ...
            payload: ProductOutboundPayload::Progress(
                t3claw_product_adapters::ProgressUpdateView {
                    turn_run_id: t3claw_turns::TurnRunId::new(),
                    kind: t3claw_product_adapters::ProgressKind::Typing,
                    generated_at: chrono::Utc::now(),
                },
            ),
            delivery_attempt_id: uuid::Uuid::new_v4(),
    ...
    async fn render_outbound_records_retryable_on_telegram_5xx() {
        ...
        egress.program_response(
            "api.telegram.org",
            Ok(t3claw_product_adapters::EgressResponse::new(
                502,
                Vec::new(),
            )),
        );
