...
//! [`RootFilesystem`](t3claw_filesystem::RootFilesystem) surface so secrets,
//! credential accounts, and credential sessions all share the same dispatch
//! fabric as the rest of Reborn (`t3claw_processes`, `t3claw_authorization`,
//! `t3claw_outbound`, `t3claw_run_state`).
//!
...
//! caller-supplied [`MountView`](t3claw_host_api::MountView): composition
...
//! [`t3claw_filesystem::CLAUDE.md`](../t3claw_filesystem/CLAUDE.md) invariant
...
struct SerializableCredentialSession {
    scope: ResourceScope,
    invocation_id: t3claw_host_api::InvocationId,
    capability_id: t3claw_host_api::CapabilityId,
    extension_id: t3claw_host_api::ExtensionId,
...
/// [`MountView`](t3claw_host_api::MountView): the `/secrets` alias resolves
/// to a per-tenant/per-user [`VirtualPath`](t3claw_host_api::VirtualPath)
...
// tenant/user-scoped [`VirtualPath`](t3claw_host_api::VirtualPath) before
...
/// [`VirtualPath`](t3claw_host_api::VirtualPath) results (post-resolution),
...
// -- Per-record locks -------------------------------------------------------
//
// Mirror the `FILESYSTEM_RECORD_LOCKS` shape used in `t3claw_run_state` and
// `t3claw_authorization`: process-local locks keyed by virtual path. The
...
mod tests {
    ...
    async fn filesystem_secret_store_writes_tenant_id_indexed_projection() {
        use t3claw_filesystem::{Filter, IndexKey, IndexValue, Page};
