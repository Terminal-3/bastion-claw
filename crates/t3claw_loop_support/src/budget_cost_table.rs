...
//! `t3claw_llm` into the loop layer without re-exporting LLM crate
...
/// the `LlmProvider::cost_per_token()` family from `t3claw_llm` into
...
/// known per-token price via `t3claw_llm::costs::model_cost`) so the
