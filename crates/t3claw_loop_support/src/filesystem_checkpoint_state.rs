...
//! mount alias. The [`ScopedFilesystem`](t3claw_filesystem::ScopedFilesystem)
...
async fn put_bytes_compat<F>(
    filesystem: &ScopedFilesystem<F>,
    resource_scope: &t3claw_host_api::ResourceScope,
    path: &ScopedPath,
    body: Vec<u8>,
    cas: CasExpectation,
) -> Result<(), TurnError>
...
mod tests {
    ...
    use t3claw_filesystem::{DirEntry, FileStat, FileType, RecordVersion, VersionedEntry};
    use t3claw_host_api::{
