...
fn host_capability_port_composition_rejects_external_raw_construction() {
    ...
    assert!(
        offenders.is_empty(),
        "HostRuntimeLoopCapabilityPort must be constructed only inside t3claw_loop_support; offenders: {offenders:#?}"
    );
}
...
async fn factory_stages_provider_tool_call_arguments_without_custom_resolver_override() {
    ...
    assert!(
        matches!(
            outcome,
            t3claw_turns::run_profile::CapabilityOutcome::Completed(_)
        ),
...
impl LoopCapabilityInputResolver for UnusedInputResolver {
    async fn resolve_capability_input(
        ...
        _input_ref: &t3claw_turns::run_profile::CapabilityInputRef,
...
fn dispatch_trust_decision() -> TrustDecision {
    TrustDecision {
        ...
        authority_ceiling: AuthorityCeiling {
            allowed_effects: vec![t3claw_host_api::EffectKind::DispatchCapability],
...
fn should_skip(path: &Path) -> bool {
    path.components().any(|component| {
        ...
        name == "t3claw_loop_support" || name == "tests" || name == "target"
    })
}
