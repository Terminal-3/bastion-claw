...
use t3claw_host_api::{AgentId, CapabilityId, MissionId, ProjectId, TenantId, ThreadId, UserId};
...
async fn thread_context_port_preserves_summary_replacements_as_system_messages() {
    ...
    fixture
        ...
        .create_summary_artifact(CreateSummaryArtifactRequest {
            ...
            start_sequence: 1,
            end_sequence: 1,
            summary_kind: t3claw_threads::SummaryKind::Compaction,
...
async fn model_port_preserves_capability_info_for_filtered_capability_view() {
    ...
    .with_capability_port(Arc::new(StaticToolDefinitionPort::new(vec![
        provider_tool_definition(
            CapabilityId::new("t3claw.loop.capability_info").unwrap(),
            "capability_info",
        ),
        provider_tool_definition(allowed_id.clone(), "demo__allowed"),
        provider_tool_definition(hidden_id, "demo__hidden"),
...
async fn transcript_port_appends_tool_result_reference_envelope_idempotently() {
    ...
    let context = fixture
        .thread_service
        .load_context_window(t3claw_threads::LoadContextWindowRequest {
            scope: fixture.thread_scope.clone(),
            thread_id: fixture.thread_id.clone(),
            max_messages: 16,
        })
...
impl LoopContextPort for StaticLoopContextPort {
    ...
    ) -> Result<LoopContextBundle, t3claw_turns::run_profile::AgentLoopHostError> {
...
struct ThreadFixture {
    ...
    thread_id: ThreadId,
    user_message_id: t3claw_threads::ThreadMessageId,
...
impl SessionThreadService for GatedFinalizeThreadService {
    ...
    async fn load_context_window(
        &self,
        request: t3claw_threads::LoadContextWindowRequest,
...
struct FailOnceMilestoneSink {
    attempts: Mutex<Vec<t3claw_turns::run_profile::LoopHostMilestone>>,
}
...
async fn wait_for_in_memory_milestones(
    ...
    expected: usize,
) -> Vec<t3claw_turns::run_profile::LoopHostMilestone> {
    for _ in 0..20 {
...
#[async_trait]
impl t3claw_turns::run_profile::LoopHostMilestoneSink for FailOnceMilestoneSink {
    async fn publish_loop_milestone(
        &self,
        milestone: t3claw_turns::run_profile::LoopHostMilestone,
    ) -> Result<(), t3claw_turns::run_profile::AgentLoopHostError> {
        let mut attempts = self.attempts.lock().unwrap();
        if attempts.is_empty() {
            attempts.push(milestone);
            return Err(t3claw_turns::run_profile::AgentLoopHostError::new(
...
struct FailOnModelStartedMilestoneSink {
    published: Mutex<Vec<t3claw_turns::run_profile::LoopHostMilestone>>,
}
...
#[async_trait]
impl t3claw_turns::run_profile::LoopHostMilestoneSink for FailOnModelStartedMilestoneSink {
    async fn publish_loop_milestone(
        &self,
        milestone: t3claw_turns::run_profile::LoopHostMilestone,
    ) -> Result<(), t3claw_turns::run_profile::AgentLoopHostError> {
...
#[async_trait]
impl t3claw_turns::run_profile::LoopHostMilestoneSink for FailOnModelCompletedMilestoneSink {
    async fn publish_loop_milestone(
        &self,
        milestone: t3claw_turns::run_profile::LoopHostMilestone,
    ) -> Result<(), t3claw_turns::run_profile::AgentLoopHostError> {
