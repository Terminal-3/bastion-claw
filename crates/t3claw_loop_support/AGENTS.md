...
- Use these neighboring contracts before changing behavior:
  ...
  - `crates/t3claw_capabilities/AGENTS.md`
  - `crates/t3claw_skills/AGENTS.md`
  - `crates/t3claw_reborn/CLAUDE.md`

## What This Crate Owns

- Loop host support services for `AgentLoopHost` / `t3claw_turns` loop ports.
...
- Fast local check: `cargo test -p t3claw_loop_support`
- Boundary check after dependency/API changes: `cargo test -p t3claw_architecture`
- Run `cargo test -p t3claw_turns` and `cargo test -p t3claw_reborn` when host-port contracts change.
