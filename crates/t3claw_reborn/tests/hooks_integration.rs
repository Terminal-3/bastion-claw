...
use t3claw_hooks::HookRegistrar;
use t3claw_hooks::dispatch::{HookDispatcher, HookDispatcherBuilder};
use t3claw_hooks::error::HookError;
use t3claw_hooks::evaluator::PredicateEvaluator;
use t3claw_hooks::failure_policy::{FailureCategory, FailureDisposition};
use t3claw_hooks::identity::{ExtensionId, HookId, HookLocalId, HookVersion};
use t3claw_hooks::installed_hook::PredicateBackedBeforeCapabilityHook;
use t3claw_hooks::kinds::observer::NoteCategory;
use t3claw_hooks::manifest::{
    HookManifestBody, HookManifestEntry, HookManifestKind, HookManifestScope, WasmBudget,
};
use t3claw_hooks::ordering::HookPhase;
use t3claw_hooks::points::{
    BeforeCapabilityHookContext, BeforePromptHookContext, EventTriggeredHookContext,
    ObserverHookContext,
};
use t3claw_hooks::predicate::{
    CapabilityPredicate, HookPredicateSpec, OnExceededAction, ValueOrRateBound,
};
use t3claw_hooks::registry::{HookBindingScope, HookPointSpec, HookRegistry};
use t3claw_hooks::sink::{
...
use t3claw_reborn::hook_gate_refs::{
...
fn event_triggered_dispatcher_with_scope(
    ...
    HookDispatcherBuilder::new(HookRegistry::new())
        ...
        .install_installed_event_triggered(
            ...
            HookPhase::Telemetry,
            event_kind,
            t3claw_host_api::ExtensionId::new(owning_extension).expect("valid ext id"),
            scope,
...
fn event_triggered_dispatcher_with_subjects(
    ...
    scope: HookBindingScope,
    owning_extension: &str,
    subjects: &[(HookId, t3claw_host_api::ExtensionId)],
) -> Arc<HookDispatcher> {
...
fn pause_approval_dispatcher() -> Arc<HookDispatcher> {
    ...
    HookDispatcherBuilder::new(HookRegistry::new())
        .install_installed_before_capability(
            hook_id,
            HookPhase::Policy,
            t3claw_host_api::ExtensionId::new("integration-tests").expect("valid ext id"),
            HookBindingScope::Global,
...
async fn event_triggered_hook_respects_own_capabilities_scope_filter() {
    ...
    let stream = EventStreamKey::from_scope(&scope);
    let log = Arc::new(InMemoryDurableEventLog::new());
    let ext_a = t3claw_host_api::ExtensionId::new("ext-a").expect("valid ext");
    let ext_b = t3claw_host_api::ExtensionId::new("ext-b").expect("valid ext");
...
async fn event_triggered_subscription_with_foreign_tenant_stream_fails_host_build() {
    let fixture = Fixture::new().await;
    let log = Arc::new(InMemoryDurableEventLog::new());
    let foreign_tenant = t3claw_host_api::TenantId::new("tenant-foreign").expect("valid tenant");
...
async fn event_triggered_subscription_with_foreign_user_stream_fails_host_build() {
    let fixture = Fixture::new().await;
    let log = Arc::new(InMemoryDurableEventLog::new());
    let foreign_user = t3claw_host_api::UserId::new("user-foreign").expect("valid user");
...
async fn event_triggered_self_lifecycle_event_does_not_redispatch() {
    ...
    let stream = EventStreamKey::from_scope(&scope);
    let log = Arc::new(InMemoryDurableEventLog::new());
    let ext = t3claw_host_api::ExtensionId::new("ext-self").expect("valid ext");
...
async fn event_triggered_replay_gap_emits_subscription_terminated_milestone() {
    use t3claw_turns::run_profile::{LoopDriverNoteKind, LoopHostMilestoneKind};
    let fixture = Fixture::new().await;
    let scope = runtime_scope(&fixture);
    let stream = EventStreamKey::from_scope(&scope);
    let log = Arc::new(InMemoryDurableEventLog::new());
...
/// dispatcher-level unit test in `t3claw_hooks` proves a manually wired
...
async fn factory_hook_security_audit_sink_records_deny_through_build() {
    ...
    assert_eq!(
        event
            .capability_id
            .as_ref()
            .map(t3claw_host_api::CapabilityId::as_str),
        Some("cap.blocked"),
...
async fn pause_approval_hook_surfaces_as_approval_required_with_real_gate_ref() {
    ...
    let host = fixture
        ...
        .with_hook_gate_ref_factory_builder({
            let f: Arc<dyn t3claw_hooks::middleware::HookGateRefFactory> = Arc::new(factory);
            move |_| Arc::clone(&f)
        })
        .build_text_only_host_with_capabilities(fixture.request(), inner.clone())
        .await
...
async fn router_backed_gate_ref_factory_sources_context_per_mint_when_reused() {
    ...
    let host_factory = fixture
        ...
        .with_hook_dispatcher(pause_approval_dispatcher())
        .with_hook_gate_ref_factory_builder({
            let f: Arc<dyn t3claw_hooks::middleware::HookGateRefFactory> =
    ...
    let turn_id_b = t3claw_turns::TurnId::new();
...
// `t3claw_hooks::middleware::{model_port, transcript_port, checkpoint_port}`
...
async fn after_model_fires_exactly_once_at_durable_boundary() {
    ...
    let bundle = host
        .build_prompt_bundle(t3claw_turns::run_profile::LoopPromptBundleRequest {
    ...
    host.finalize_assistant_message(t3claw_turns::run_profile::FinalizeAssistantMessage {
        reply: t3claw_turns::run_profile::AssistantReply {
...
async fn observer_panic_does_not_fail_model_call() {
    ...
    let response = host
        .finalize_assistant_message(t3claw_turns::run_profile::FinalizeAssistantMessage {
            reply: t3claw_turns::run_profile::AssistantReply {
...
async fn installed_hook_with_own_scope_does_not_fire_on_other_provider_capabilities() {
    ...
    let dispatcher = HookDispatcherBuilder::new(HookRegistry::new())
        .install_installed_before_capability(
            hook_id,
            HookPhase::Policy,
            t3claw_host_api::ExtensionId::new("ext-a").expect("valid ext id"),
            HookBindingScope::OwnCapabilities,
            Box::new(AlwaysDeny),
        )
...
fn own_capabilities_dispatcher(owning_ext: &str, local_id: &str) -> Arc<HookDispatcher> {
    ...
    HookDispatcherBuilder::new(HookRegistry::new())
        .install_installed_before_capability(
            hook_id,
            HookPhase::Policy,
            t3claw_host_api::ExtensionId::new(owning_ext).expect("valid ext id"),
            HookBindingScope::OwnCapabilities,
            Box::new(AlwaysDeny),
        )
        .expect("install installed hook with own-scope")
...
async fn own_capabilities_hook_fires_when_provider_matches() {
    let fixture = Fixture::new().await;
    let ext_a = t3claw_host_api::ExtensionId::new("ext-a").expect("valid ext id");
...
async fn own_capabilities_hook_does_not_fire_when_provider_differs() {
    let fixture = Fixture::new().await;
    let ext_b = t3claw_host_api::ExtensionId::new("ext-b").expect("valid ext id");
    let inner = Arc::new(ProviderAwareCapabilityPort::new(vec![
...
async fn hook_telemetry_attribution_is_per_run_not_captured() {
    ...
    let factory_with_hook = fixture.factory().with_hook_dispatcher_builder_factory(|| {
        use t3claw_hooks::dispatch::HookDispatcherBuilder as HDBuilder;
        use t3claw_hooks::registry::HookRegistry as HReg;
        let hook_id = HookId::derive(
        ...
        Ok(HDBuilder::new(HReg::new())
            .install_installed_before_capability(
                hook_id,
                HookPhase::Policy,
                t3claw_host_api::ExtensionId::new("ext-tele").expect("valid ext id"),
                HookBindingScope::Global,
                Box::new(AlwaysDeny),
            )
    ...
    let new_turn_id = t3claw_turns::TurnId::new();
...
async fn before_prompt_hook_message_is_resolvable_via_factory_wiring() {
    use t3claw_hooks::dispatch::HookDispatcherBuilder as HDBuilder;
    use t3claw_hooks::registry::HookRegistry as HReg;
    use t3claw_hooks::sink::{RestrictedBeforePromptHook, RestrictedMutatorSink};
    ...
    impl RestrictedBeforePromptHook for InjectingHook {
        async fn evaluate(
            &self,
            _ctx: &t3claw_hooks::points::BeforePromptHookContext,
            sink: &mut dyn RestrictedMutatorSink,
        ) {
            let _ = sink.add_envelope_snippet(
                "injected hook context".to_string(),
                t3claw_hooks::kinds::mutator::PatchOrdinalHint::Last,
            );
        }
    }
    ...
    let dispatcher = HDBuilder::new(HReg::new())
        .install_installed_before_prompt(
            hook_id,
            HookPhase::Policy,
            t3claw_host_api::ExtensionId::new("ext-prompt").expect("valid ext id"),
            HookBindingScope::Global,
