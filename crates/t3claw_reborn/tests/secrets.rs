...
use t3claw_host_api::{InvocationId, ResourceScope, SecretHandle, TenantId, UserId};
use t3claw_reborn::secrets::{
...
use t3claw_secrets::{SecretMaterial, SecretStoreError};
...
async fn reborn_secret_store_fails_closed_on_consume_when_existing_rows_use_another_master_key() {
    ...
    let handle = t3claw_host_api::SecretHandle::new("openai_key").unwrap();
    store
        .put(
            scope.clone(),
            handle.clone(),
