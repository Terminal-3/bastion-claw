...
use t3claw_reborn::app_loop_family::build_loop_family_registry;
use t3claw_reborn::planned_driver::PlannedDriver;
...
async fn planned_driver_live_default_smoke() {
    ...
    let resolved = resolver
        .resolve_run_profile(t3claw_turns::RunProfileResolutionRequest::interactive_default())
        .await
...
async fn planned_driver_rejects_mismatched_profile_assignment() {
    ...
    request.resolved_run_profile.loop_driver.version = t3claw_turns::RunProfileVersion::new(99);
...
async fn planned_driver_resume_rejects_mismatched_ids_before_checkpoint_load() {
    ...
    let other_context = t3claw_agent_loop::test_support::test_run_context("foreign-run");
...
async fn planned_driver_resume_rejects_mismatched_profile_before_checkpoint_load() {
    ...
    let host = ForbiddenResumeHost::new(context.clone());
    let mut request = resume_request(&context, TurnCheckpointId::new());
    let other_context = t3claw_agent_loop::test_support::test_run_context("foreign-profile");
