...
use t3claw_filesystem::{LocalFilesystem, RootFilesystem};
use t3claw_hooks::{
...
use t3claw_reborn::driver_registry::{
    DriverKind, DriverRegistry, DriverRequirements, LoopDriverRegistryKey,
};
use t3claw_reborn::loop_driver_host::{
...
use t3claw_reborn::text_loop_driver::TextOnlyModelReplyDriver;
...
const SYNTHETIC_CAPABILITY_INFO_ID: &str = "t3claw.loop.capability_info";
...
async fn text_only_host_factory_builds_complete_agent_loop_driver_host() {
    ...
    let context = host_dyn
        .load_loop_context(LoopContextRequest {
            after: None,
            limit: 8,
            mode: t3claw_turns::run_profile::PromptMode::TextOnly,
        })
        .await
        .unwrap();
...
async fn progress_event_serde_roundtrip_all_variants() {
    ...
    let bundle_ref = t3claw_turns::run_profile::LoopPromptBundleRef::for_run(&context, "bundle")
        .expect("bundle ref");
...
async fn turn_runner_worker_completes_after_libsql_turn_and_thread_services_reopen() {
    ...
    let run_id = {
        ...
        let submit = turn_store
            .submit_turn(
                ...
                &t3claw_turns::AllowAllTurnAdmissionPolicy,
                &resolver,
            )
            .await
            .unwrap();
    ...
    let transition_port: Arc<dyn t3claw_turns::runner::TurnRunTransitionPort> = turn_store.clone();
...
/// [`MountView`](t3claw_host_api::MountView) so tenant isolation is
/// structural rather than something this test has to thread through paths.
#[cfg(feature = "libsql-restart-tests")]
async fn build_libsql_thread_service(
    db: Arc<libsql::Database>,
) -> t3claw_threads::FilesystemSessionThreadService<t3claw_filesystem::LibSqlRootFilesystem> {
    use t3claw_filesystem::{LibSqlRootFilesystem, ScopedFilesystem};
    use t3claw_host_api::{MountAlias, MountGrant, MountPermissions};
    ...
    t3claw_threads::FilesystemSessionThreadService::new(scoped)
}
...
async fn libsql_filesystem_turn_store(
    db: Arc<libsql::Database>,
) -> t3claw_turns::FilesystemTurnStateStore<t3claw_filesystem::LibSqlRootFilesystem> {
    use t3claw_filesystem::{LibSqlRootFilesystem, ScopedFilesystem};
    use t3claw_host_api::{MountAlias, MountGrant, MountPermissions, MountView, VirtualPath};
    ...
    t3claw_turns::FilesystemTurnStateStore::new(scoped)
}
...
async fn turn_runner_worker_drives_script_capability_through_real_host_runtime() {
    ...
    let runtime: Arc<dyn HostRuntime + Send + Sync> = Arc::new(
        HostRuntimeServices::new(
            ...
            t3claw_host_runtime::CapabilitySurfaceVersion::new("surface-v1").unwrap(),
        )
...
async fn turn_runner_blocks_on_approval_then_coordinator_resume_completes_same_run() {
    ...
    assert_eq!(
        turn_store
            ...
            .kind,
        t3claw_turns::TurnEventKind::Resumed,
...
// factory shaped exactly like the one `t3claw_reborn_composition::hooks`
...
/// `t3claw_reborn_composition::hooks::NoOpObserverHook`. The only surviving
/// `NoOpObserverHook` is the composition crate's test-only one at
/// `t3claw_reborn_composition::hooks::tests::NoOpObserverHook`. This constant
...
const E2E_NOOP_OBSERVER_PATH: &str = "t3claw_reborn::tests::loop_driver_host::E2eNoOpObserver";
...
async fn build_runtime_host_with_optional_hooks(
    ...
    hook_factory: Option<t3claw_reborn::loop_driver_host::HookDispatcherBuilderFactory>,
) -> (
...
// doubles that exercise the `t3claw_reborn` host-factory PLUMBING — i.e. that
...
// `t3claw_reborn_composition::hooks` and
// `t3claw_reborn_composition::runtime` tests. Keep this split: host plumbing
...
/// note above.
fn first_party_only_hook_factory() -> t3claw_reborn::loop_driver_host::HookDispatcherBuilderFactory
{
    Arc::new(|| {
...
fn extension_deny_hook_factory(
    extension: &str,
    deny_target: &str,
) -> t3claw_reborn::loop_driver_host::HookDispatcherBuilderFactory {
    ...
    Arc::new(move || {
        ...
        let ext_id =
            t3claw_host_api::ExtensionId::new(extension.clone()).expect("valid extension id");
...
impl HostInputQueue for SingleMessageQueue {
    async fn next_after(
        &self,
        _run_id: TurnRunId,
        after: t3claw_turns::run_profile::LoopInputCursorToken,
        _limit: usize,
        ...
        match pending {
            Some(input) => {
                let cursor =
                    t3claw_turns::run_profile::LoopInputCursorToken::new("input-cursor:1").unwrap();
                let ack_token =
                    t3claw_turns::run_profile::LoopInputAckToken::new("input-ack:1").unwrap();
                Ok(HostInputBatch {
    ...
    async fn ack_consumed(
        &self,
        _run_id: TurnRunId,
        _tokens: Vec<t3claw_turns::run_profile::LoopInputAckToken>,
...
async fn text_only_host_skill_context_does_not_expand_capability_surface() {
    ...
    assert!(surface.descriptors.is_empty());
    let outcome = host
        .invoke_capability_batch(t3claw_turns::run_profile::CapabilityBatchInvocation {
...
async fn text_only_host_maps_explicit_unknown_runtime_outcome_to_failure() {
    ...
    assert_eq!(
        ...
        CapabilityFailureKind::Unknown(
            t3claw_turns::run_profile::CapabilityFailureKindValue::new("streaming")
...
async fn text_only_host_rejects_mismatched_capability_authority_context() {
    ...
    assert!(matches!(
        error,
        t3claw_reborn::loop_driver_host::RebornLoopDriverHostError::InvalidRequest { .. }
    ));
    assert!(runtime.invocations().is_empty());
}
...
impl HostIdentityContextSource for StaticIdentityContextSource {
    ...
    async fn resolve_identity_message_content(
        &self,
        _run_context: &LoopRunContext,
        message_ref: &t3claw_turns::LoopMessageRef,
...
struct TestHostRuntimeCapabilityFactory {
    ...
    visible_request: t3claw_host_runtime::VisibleCapabilityRequest,
...
struct RecordingHostRuntime {
    surface: Mutex<t3claw_host_runtime::VisibleCapabilitySurface>,
...
impl RecordingHostRuntime {
    fn with_surface(surface: t3claw_host_runtime::VisibleCapabilitySurface) -> Self {
        Self {
    ...
    fn set_surface(&self, surface: t3claw_host_runtime::VisibleCapabilitySurface) {
...
impl HostRuntime for RecordingHostRuntime {
    ...
    async fn visible_capabilities(
        &self,
        _request: t3claw_host_runtime::VisibleCapabilityRequest,
    ) -> Result<t3claw_host_runtime::VisibleCapabilitySurface, HostRuntimeError> {
...
) -> t3claw_host_runtime::VisibleCapabilitySurface {
    host_runtime_surface_with_version(
...
fn host_runtime_surface_with_version(
    ...
    t3claw_host_runtime::VisibleCapabilitySurface {
        version: t3claw_host_runtime::CapabilitySurfaceVersion::new(version).unwrap(),
...
fn host_runtime_visible_request(
...
) -> t3claw_host_runtime::VisibleCapabilityRequest {
    let user_id = fixture
        .thread_scope
        .owner_user_id
        .clone()
    ...
    t3claw_host_runtime::VisibleCapabilityRequest::new(
        context,
...
impl AgentLoopDriver for ApprovalBlockThenFinalReplyDriver {
    ...
    async fn run(
        ...
        Ok(LoopExit::Blocked(LoopBlocked {
            ...
            credential_requirements: Vec::new(),
            checkpoint_id: t3claw_turns::TurnCheckpointId::new(),
...
fn driver_host_error(error: t3claw_turns::run_profile::AgentLoopHostError) -> AgentLoopDriverError {
...
impl TurnStateStore for StaticTurnStateStore {
    ...
    ) -> Result<t3claw_turns::ResumeTurnResponse, TurnError> {
...
impl HostFixture {
    ...
    async fn new_with_submission_state(
        ...
        let resolved = InMemoryRunProfileResolver::default()
            ...
            .await
            .unwrap();
        let turn_id = t3claw_turns::TurnId::new();
        let run_id = TurnRunId::new();
        let state = t3claw_turns::TurnRunState {
            scope: turn_scope.clone(),
    ...
    async fn stage_checkpoint_state(
        ...
        payload: &[u8],
    ) -> t3claw_turns::CheckpointStateRecord {
    ...
    fn respond_with_capability_calls(&self) {
        *self.response.lock().unwrap() = Ok(HostManagedModelResponse {
            ...
            output: ParentLoopOutput::CapabilityCalls(vec![
                t3claw_turns::run_profile::CapabilityCallCandidate {
