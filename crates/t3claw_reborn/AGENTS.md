...
- Use these neighboring contracts before changing behavior:
  - `crates/t3claw_agent_loop/CLAUDE.md`
  - `crates/t3claw_turns/AGENTS.md`
  - `crates/t3claw_loop_support/CLAUDE.md`
  - `crates/t3claw_reborn_composition/CLAUDE.md`
...
- `turn_runner.rs` the concrete turn-runner composition over the neutral `t3claw_turns` runner contract.
- `runtime.rs`, `model_gateway.rs`, `model_routes.rs`, `production_readiness.rs`, and secrets/model runtime seams.

## Do Not Move In Here

- Loop family/executor behavior owned by `t3claw_agent_loop`.
- Neutral runner/host contracts owned by `t3claw_turns`.
...
- Fast local check: `cargo test -p t3claw_reborn`
...
- Boundary check after dependency/API changes: `cargo test -p t3claw_architecture`
...
- Do not duplicate neutral DTOs from `t3claw_turns`.
