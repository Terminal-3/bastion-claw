...
//! that contract to the shared `t3claw_llm` provider abstraction.
...
use t3claw_host_api::sha256_digest_token;
use t3claw_llm::{
...
use t3claw_threads::{ProviderToolCallReferenceEnvelope, SessionThreadService, ThreadScope};
use t3claw_turns::run_profile::LoopModelUsage;
use t3claw_turns::{
    TurnId, TurnRunId,
    run_profile::{
...
impl LlmModelProfilePolicy {
    ...
    /// to its per-token price via [`t3claw_llm::costs::model_cost`].
    /// Profiles whose `model_override` is unknown to the LLM cost table
    /// fall back to [`t3claw_llm::costs::default_cost`] (roughly GPT-4o
...
/// [`HostManagedLoopModelPort`](t3claw_turns::run_profile::HostManagedLoopModelPort):
...
/// Host-managed model gateway backed by the shared `t3claw_llm::LlmProvider` abstraction.
#[derive(Clone)]
    ...
    P: LlmProvider + ?Sized + Send + Sync,
{
    ...
    async fn stream_model_with_capabilities(
        &self,
        request: HostManagedModelRequest,
        capabilities: Arc<dyn t3claw_turns::run_profile::LoopCapabilityPort>,
    ) -> Result<HostManagedModelResponse, HostManagedModelError> {
        let route = self
            .policy
            .route_for(&request.model_profile_id)
...
async fn complete_model_request<P>(
    ...
    capabilities: Option<Arc<dyn t3claw_turns::run_profile::LoopCapabilityPort>>,
...
fn validate_provider_replay_identity(
    ...
    provider_call.validate().map_err(|error| {
        t3claw_loop_support::raw_host_managed_model_error(
...
mod tests {
    ...
    fn tool_result_replay_prefers_model_observation_over_safe_summary() {
        ...
        let envelope = t3claw_threads::ToolResultReferenceEnvelope::with_model_observation(
            "result:tool-error",
            t3claw_threads::ToolResultSafeSummary::new("tool failed").expect("safe summary"),
            observation.clone(),
        )
