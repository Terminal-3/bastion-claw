...
//! downstream consumer is `t3claw_reborn_composition`, which composes the
...
//! `t3claw_architecture` enforce that nothing else takes a normal cargo
...
//! (`t3claw_reborn::driver_registry::DriverRegistry`,
//! `t3claw_reborn::model_gateway::LlmProviderModelGateway`, …) so that a
