...
use t3claw_events::SecurityAuditSink;
...
pub trait RuntimeTurnStateStore:
    ...
    + t3claw_turns::TurnEventProjectionSource
    + Send
    + Sync
{
}
    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
{
    ...
    pub coordinator: Arc<dyn t3claw_turns::TurnCoordinator>,
    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
{
    ...
    let coordinator: Arc<dyn t3claw_turns::TurnCoordinator> = base_coordinator_arc;
    completion_observer
    ...
    let spawn_decorator = Arc::new(SubagentSpawnCapabilityDecorator::new(
        SubagentSpawnDeps {
            coordinator: Arc::clone(&coordinator) as Arc<dyn t3claw_turns::TurnCoordinator>,
            child_runs,
            ...
            gate_store: Arc::clone(&parts.subagent_gate_store)
                as Arc<dyn t3claw_loop_support::SubagentGateResolutionStore>,
...
impl SubagentSpawnCapabilityDecorator {
    fn new(
        ...
        let spawn_id = CapabilityId::new(t3claw_loop_support::DEFAULT_SPAWN_SUBAGENT_CAPABILITY_ID)
            .map_err(|error| DefaultPlannedRuntimeBuildError::RunProfile(error.to_string()))?;
        let parameters_schema = Arc::new(
            t3claw_loop_support::build_spawn_subagent_parameters_schema(&flavor_catalog),
        );
        Ok(Self {
...
mod tests {
    ...
    fn builtin_flavor_catalog_threads_enum_into_schema() {
        ...
        use t3claw_loop_support::build_spawn_subagent_parameters_schema;
