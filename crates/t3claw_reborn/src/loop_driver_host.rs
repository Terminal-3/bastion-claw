...
use t3claw_events::{DurableEventLog, EventCursor, EventStreamKey, ReadScope, SecurityAuditSink};
use t3claw_hooks::dispatch::{HookDispatcher, HookDispatcherBuilder};
use t3claw_hooks::middleware::{
...
use t3claw_host_api::ExtensionId;
...
use t3claw_threads::{SessionThreadService, ThreadScope};
...
impl HookPromptMaterializationSink for InstructionStoreBackedHookSink {
    fn put(
        &self,
        role: &str,
        content_ref: &t3claw_turns::LoopMessageRef,
        safe_content: String,
...
impl LoopCapabilityPort for SurfaceTrackingLoopCapabilityPort {
    ...
    async fn register_provider_tool_call(
        &self,
        tool_call: ProviderToolCall,
    ) -> Result<t3claw_turns::run_profile::CapabilityCallCandidate, AgentLoopHostError> {
...
// `LoopCapabilityInputResolver` is now defined in `t3claw_loop_support`
...
/// [`crate::t3claw_hooks::points::SanitizedArguments`] already caps per-string
...
/// `t3claw_hooks` middleware as a
/// [`t3claw_hooks::middleware::CapabilityInputResolver`].
///
...
///   [`t3claw_hooks::points::SanitizedArguments`]) apply to predicate
...
impl HookCapabilityInputResolver for HookCapabilityInputResolverAdapter {
    async fn resolve(
        &self,
        invocation: &t3claw_turns::run_profile::CapabilityInvocation,
...
// `LoopCapabilityResultWriter` is now defined in `t3claw_loop_support`
// (workspace-10 refactor); imported above.
...
impl EventTriggeredHookSubscription {
    ...
    fn effective_read_scope(
        &self,
        run_scope: &t3claw_turns::TurnScope,
        thread_scope: &t3claw_threads::ThreadScope,
    ...
    /// caller, and widening its visibility would let other `t3claw_reborn`
    ...
    async fn spawn(
        ...
        tenant_id: t3claw_host_api::TenantId,
        run_context: LoopRunContext,
    ...
    async fn run(
        ...
        loop {
                ...
                .await
            {
                ...
                Err(t3claw_events::EventError::ReplayGap {
                    requested,
                    earliest,
                }) => {
                    tracing::error!(
...
async fn emit_subscription_terminated_note(
    ...
    let summary = match t3claw_turns::run_profile::LoopSafeSummary::new(safe_summary) {
        Ok(s) => s,
        Err(_) => {
    ...
    let milestone = t3claw_turns::run_profile::LoopHostMilestone {
        ...
        kind: t3claw_turns::run_profile::LoopHostMilestoneKind::DriverNote {
            kind: t3claw_turns::run_profile::LoopDriverNoteKind::EventSubscriptionTerminated,
    ...
    G: HostManagedModelGateway + ?Sized,
{
    ...
    /// `t3claw_reborn_composition::hooks` ("Predicate counter scoping").
    ...
    hook_gate_ref_factory: Option<Arc<dyn t3claw_hooks::middleware::HookGateRefFactory>>,
...
pub type HookGateRefFactoryBuilder = Arc<
    dyn Fn(&LoopRunContext) -> Arc<dyn t3claw_hooks::middleware::HookGateRefFactory> + Send + Sync,
>;
    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
{
    ...
    /// [`t3claw_turns::run_profile::HookMilestoneSink`] (typically a
    /// [`t3claw_turns::run_profile::RunScopedHookMilestoneSink`] wrapping
    ...
    pub fn with_hook_gate_ref_factory(
        mut self,
        factory: Arc<dyn t3claw_hooks::middleware::HookGateRefFactory>,
    ) -> Self {
    ...
    where
        F: Fn(&LoopRunContext) -> Arc<dyn t3claw_hooks::middleware::HookGateRefFactory>
            + Send
            + Sync
            + 'static,
    {
...
impl LoopTranscriptPort for RebornLoopDriverHost {
    ...
    ) -> Result<t3claw_turns::LoopMessageRef, AgentLoopHostError> {
    ...
    G: HostManagedModelGateway + ?Sized + Send + Sync + 'static,
{
    async fn create_host(
        ...
        Box<dyn t3claw_turns::run_profile::AgentLoopDriverHost + Send + Sync>,
    ...
    > {
        ...
        host_result
            .map(|host| {
                Box::new(host)
                    as Box<dyn t3claw_turns::run_profile::AgentLoopDriverHost + Send + Sync>
            })
...
fn validate_thread_scope(
    ...
    // Reborn text-only hosts currently wrap `t3claw_threads::ThreadScope`,
...
fn turn_error_to_host_error(error: TurnError) -> AgentLoopHostError {
    match &error {
        TurnError::Unauthorized => t3claw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "access",
        ...
        TurnError::InvalidRequest { .. } => t3claw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "request",
        ...
        TurnError::Unavailable { .. } => t3claw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "store",
            AgentLoopHostErrorKind::Unavailable,
        ...
        TurnError::ScopeNotFound => t3claw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "scope_lookup",
        ...
        TurnError::Conflict { .. } => t3claw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "write",
            AgentLoopHostErrorKind::CheckpointRejected,
        ...
        TurnError::CapacityExceeded { .. } => t3claw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "write",
            AgentLoopHostErrorKind::Unavailable,
        ...
        TurnError::InvalidTransition { .. } => t3claw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "write",
            AgentLoopHostErrorKind::CheckpointRejected,
        ...
        TurnError::LeaseMismatch => t3claw_loop_support::raw_agent_loop_host_error(
            "checkpoint_state",
            "write",
            AgentLoopHostErrorKind::CheckpointRejected,
        ...
        TurnError::ThreadBusy(_) | TurnError::AdmissionRejected(_) => {
            t3claw_loop_support::raw_agent_loop_host_error(
                "checkpoint_state",
                "admission",
                AgentLoopHostErrorKind::Unavailable,
...
mod hook_resolver_adapter_tests {
    ...
    use super::*;
    use std::sync::Mutex;
    use t3claw_host_api::{AgentId, CapabilityId, ProjectId, TenantId, ThreadId};
...
mod tests {
    ...
    use t3claw_host_api::{AgentId, ProjectId, TenantId, ThreadId, UserId};
    use t3claw_turns::{
    ...
    async fn checkpoint_port_load_payload_rejects_schema_version_mismatch() {
        ...
        let bumped_version =
            t3claw_turns::RunProfileVersion::new(stored_schema_version.as_u64() + 1);
...
mod event_subscription_scope_tests {
    use super::*;
    use t3claw_events::{InMemoryDurableEventLog, RuntimeEvent};
    use t3claw_host_api::{
        AgentId, CapabilityId, ProjectId, ResourceScope, TenantId, ThreadId, UserId,
    };
    use t3claw_threads::ThreadScope;
    ...
    fn effective_read_scope_rejects_mission_widening() {
        let (tenant, agent, user, project, thread) = ids();
        let foreign_mission = t3claw_host_api::MissionId::new("mission-OTHER").expect("mission");
        let supplied = ReadScope {
    ...
    impl DurableEventLog for HeadCursorFailingLog {
        async fn append(
            &self,
            _event: RuntimeEvent,
        ) -> Result<t3claw_events::EventLogEntry<RuntimeEvent>, t3claw_events::EventError> {
        ...
        async fn read_after_cursor(
            ...
            _limit: usize,
        ) -> Result<t3claw_events::EventReplay<RuntimeEvent>, t3claw_events::EventError> {
        ...
        async fn head_cursor(
            ...
            _stream: &EventStreamKey,
            _after: EventCursor,
        ) -> Result<EventCursor, t3claw_events::EventError> {
            Err(t3claw_events::EventError::DurableLog {
    ...
    async fn event_triggered_head_cursor_failure_emits_terminated_milestone() {
        ...
        use t3claw_turns::{TurnId, TurnRunId};
        ...
        let dispatcher =
            t3claw_hooks::dispatch::HookDispatcherBuilder::new(t3claw_hooks::HookRegistry::new())
                .build_arc();
...
mod turn_error_to_host_error_tests {
    use super::*;
    use t3claw_turns::{TurnCapacityResource, TurnError};
