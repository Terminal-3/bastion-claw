...
#[cfg(feature = "filesystem-goal-store")]
use t3claw_host_api::ScopedPath;
...
#[cfg(feature = "filesystem-goal-store")]
#[async_trait]
impl<F> t3claw_loop_support::SubagentSpawnGoalStore for FilesystemSubagentGoalStore<F>
where
    F: RootFilesystem + 'static,
{
    async fn put_goal(
        ...
        scope: &TurnScope,
        run_id: TurnRunId,
        goal: t3claw_loop_support::SubagentGoalRecord,
...
#[async_trait]
impl t3claw_loop_support::SubagentSpawnGoalStore for InMemoryBoundedSubagentGoalStore {
    async fn put_goal(
        &self,
        scope: &TurnScope,
        run_id: TurnRunId,
...
fn map_goal_error(error: SubagentGoalStoreError) -> t3claw_turns::run_profile::AgentLoopHostError {
    let kind = match error {
        SubagentGoalStoreError::NotFound { .. } => {
            t3claw_turns::run_profile::AgentLoopHostErrorKind::InvalidInvocation
        }
        SubagentGoalStoreError::PayloadTooLarge { .. } => {
            t3claw_turns::run_profile::AgentLoopHostErrorKind::BudgetExceeded
        }
    ...
    t3claw_turns::run_profile::AgentLoopHostError::new(kind, error.to_string())
}
...
mod tests {
    ...
    use t3claw_host_api::{AgentId, ProjectId, TenantId, ThreadId};
    #[cfg(feature = "filesystem-goal-store")]
