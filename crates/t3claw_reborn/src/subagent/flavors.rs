...
use t3claw_loop_support::{SubagentDefinition, SubagentDefinitionResolver, SubagentKindId};
use t3claw_turns::{RunProfileRequest, TurnRunId, run_profile::AgentLoopHostError};
...
/// Returns one [`t3claw_loop_support::SpawnSubagentFlavorDescriptor`] per
...
pub fn builtin_flavor_catalog() -> Vec<t3claw_loop_support::SpawnSubagentFlavorDescriptor> {
    BUILTIN_SUBAGENT_FLAVORS
        .iter()
        .map(|f| t3claw_loop_support::SpawnSubagentFlavorDescriptor {
            id: t3claw_loop_support::SubagentKindId::new(f.id.as_str())
            ...
            requested_run_profile: RunProfileRequest::new(SUBAGENT_PLANNED_PROFILE_ID).map_err(
                |reason| {
                    AgentLoopHostError::new(
                        t3claw_turns::run_profile::AgentLoopHostErrorKind::Internal,
                        reason,
                    )
                },
            )?,
...
mod tests {
    ...
    use t3claw_loop_support::DEFAULT_SPAWN_SUBAGENT_CAPABILITY_ID;
    ...
    mod caller_level {
        ...
        use async_trait::async_trait;
        use t3claw_agent_loop::test_support::test_run_context;
        use t3claw_host_api::{CapabilityId, RuntimeKind};
        ...
        use t3claw_turns::{LoopResultRef, RunProfileId, RunProfileVersion};
        ...
        async fn filter_for_flavor(
            ...
            t3claw_loop_support::CapabilitySurfaceProfileFilter,
            Arc<HostSurfaceSpy>,
        ) {
        ...
        async fn visible_ids(
            filter: &t3claw_loop_support::CapabilitySurfaceProfileFilter,
        ) -> Vec<String> {
            let mut ids = filter
