...
//!
//! `t3claw_hooks` owns the hook middleware contract, but it deliberately
...
use t3claw_hooks::middleware::HookGateRefFactory;
use t3claw_host_api::{ApprovalRequestId, CapabilityId, UserId, sha256_digest_token};
use t3claw_turns::{
    LoopGateRef,
    run_profile::{
...
/// Per-invocation metadata captured outside `t3claw_hooks` and consumed by
...
///
/// Existing approval resolution in `t3claw_approvals::ApprovalResolver`
...
/// for router-backed hook gate factories without changing `t3claw_hooks`.
