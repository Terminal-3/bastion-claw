...
//! the sealed `t3claw_agent_loop` framework. It intentionally holds an opaque
...
mod tests {
    ...
    use std::sync::Mutex;
    use t3claw_agent_loop::test_support::{
    ...
    fn default_planned_driver_descriptor_uses_default_family_identity() {
        ...
        assert_eq!(
            ...
            // `use super::*` -> `use t3claw_agent_loop::state::*`) — the
    ...
    fn run_context_for_driver(driver: &PlannedDriver) -> t3claw_turns::run_profile::LoopRunContext {
        let descriptor = driver.descriptor();
