...
//!
//! `t3claw_turns` owns the trusted applier and the private validation policy.
//! This module provides Reborn-specific evidence adapters.

use std::sync::Arc;

use async_trait::async_trait;
use t3claw_loop_support::RunCancellationFactory;
use t3claw_threads::{
...
pub use t3claw_turns::loop_exit::{
...
pub trait ApprovalGateEvidenceStore: Send + Sync {
    async fn pending_approval_gate(
        &self,
        scope: &TurnScope,
        gate_ref: &t3claw_turns::LoopGateRef,
    ) -> Result<bool, TurnError>;
}
    ...
    S: SessionThreadService + ?Sized + Send + Sync,
{
    ...
    async fn verify_final_checkpoint(
        ...
        let checkpoint = self
            .loop_checkpoint_store
            .get_loop_checkpoint(t3claw_turns::GetLoopCheckpointRequest {
                scope: request.scope.clone(),
    ...
    async fn verify_failure_evidence(
        ...
        let state = match t3claw_agent_loop::state::LoopExecutionState::from_checkpoint_payload(
            checkpoint_state.payload.as_bytes(),
            t3claw_agent_loop::state::CheckpointKind::Final,
        ) {
            Ok(state) => state,
...
fn thread_scope_from_turn_scope(scope: &TurnScope) -> Result<ThreadScope, TurnError> {
    // `t3claw_threads::ThreadScope` is currently agent-scoped. Reject
