...
pub enum AppEvent {
    ...
    Warning {
        /// Originating module/target (e.g. `t3claw::bridge::router`).
        source: String,
        message: String,
        #[serde(skip_serializing_if = "Option::is_none")]
        thread_id: Option<String>,
    ...
    /// (dependency direction: `t3claw_engine` depends on
    /// `t3claw_common`, not vice versa), so the wire type is a
...
/// Wire-side mirror of `t3claw_engine::CodeExecutionFailure`.
///
