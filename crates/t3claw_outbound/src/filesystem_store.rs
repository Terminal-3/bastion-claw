...
//! [`RootFilesystem`](t3claw_filesystem::RootFilesystem) surface accessed
//! through a [`ScopedFilesystem`]. The [`MountView`](t3claw_host_api::MountView)
//! wired by composition resolves `/outbound` to a tenant/user-scoped
//! [`VirtualPath`](t3claw_host_api::VirtualPath) (e.g.
...
use t3claw_event_projections::{ProjectionCursor, ProjectionScope};
...
use t3claw_host_api::{ResourceScope, ScopedPath, TenantId, ThreadId};
use t3claw_turns::{TurnActor, TurnScope};
...
/// enforced by the [`MountView`](t3claw_host_api::MountView) the
    ...
    F: RootFilesystem,
{
    ...
    /// [`t3claw_processes::filesystem_store::put_with_byte_fallback`] so
    ...
    async fn write_communication_preference(
        ...
        let cas = match request.expected_version {
            Some(version) => CasExpectation::Version(
                t3claw_filesystem::RecordVersion::from_backend(version.raw()),
            ),
...
/// is a control character; `validate_scope_id` in `t3claw_host_api` rejects
...
mod tests {
    ...
    fn sentinel_is_control_character_rejected_by_scope_id_validators() {
        ...
        // ever contain. `validate_scope_id` (`t3claw_host_api::ids`)
