...
- Fast local check: `cargo test -p t3claw_outbound`
- Backend parity check without live Postgres: `T3CLAW_SKIP_POSTGRES_TESTS=1 cargo test -p t3claw_outbound --all-features`
- Lint check: `cargo clippy -p t3claw_outbound --all-targets --all-features -- -D warnings`
