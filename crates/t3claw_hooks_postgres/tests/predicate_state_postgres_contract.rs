...
//! contract harness from `t3claw_hooks::predicate_state::contract`.
//!
...
//! `T3CLAW_HOOKS_POSTGRES_URL` (or `DATABASE_URL`) to a libpq URL. With
//! no URL the tests print a skip notice and pass, matching the
//! env-gated-skip pattern used by `t3claw_reborn_event_store` and
//! `t3claw_filesystem` (no testcontainers dependency).
//!
//! # Isolation
//!
...
use t3claw_hooks_postgres::PostgresPredicateStateBackend;
...
fn db_url() -> Option<String> {
    std::env::var("T3CLAW_HOOKS_POSTGRES_URL")
...
macro_rules! pg_contract {
    ($name:ident) => {
        ...
        async fn $name() {
            let Some(url) = db_url() else {
                eprintln!(
                    "skipping postgres predicate contract `{}`: \
                     T3CLAW_HOOKS_POSTGRES_URL / DATABASE_URL not set",
                    stringify!($name)
                );
                return;
            };
            ...
            t3claw_hooks::predicate_state::contract::$name(move || {
...
impl t3claw_hooks::predicate_state::PredicateStateBackend for PgBackendHandle {
    async fn record_invocation(
        &self,
        key: &t3claw_hooks::predicate_state::InvocationKey,
        event_id: &t3claw_hooks::predicate_state::PredicateEventId,
        now: chrono::DateTime<chrono::Utc>,
        window: std::time::Duration,
    ) -> Result<u32, t3claw_hooks::predicate_state::PredicateBackendError> {
        self.0.record_invocation(key, event_id, now, window).await
    }

    async fn record_value(
        &self,
        key: &t3claw_hooks::predicate_state::ValueKey,
        event_id: &t3claw_hooks::predicate_state::PredicateEventId,
        now: chrono::DateTime<chrono::Utc>,
        value: rust_decimal::Decimal,
        window: std::time::Duration,
    ) -> Result<rust_decimal::Decimal, t3claw_hooks::predicate_state::PredicateBackendError> {
    ...
    ) -> Result<u64, t3claw_hooks::predicate_state::PredicateBackendError> {
