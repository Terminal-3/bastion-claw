# Agent Map — t3claw_hooks_postgres
...
- Read `src/lib.rs` first — it documents why this is a separate crate, the
  dual-backend split (Postgres here, libSQL in `t3claw_hooks_libsql`, parity
  in `t3claw_hooks_parity`), and the shared two-table typed schema.
- Read `Cargo.toml` for actual dependencies and the `postgres` feature gate.
- The trait contract this crate implements lives in
  `t3claw_hooks::predicate_state` (`PredicateStateBackend`); the shared
...
- The `PredicateStateBackend` trait itself, the in-memory backend, or the
  libSQL backend (those live in `t3claw_hooks` / `t3claw_hooks_libsql`).
...
- Fast local check: `cargo test -p t3claw_hooks_postgres`
- Postgres-backed integration/adversarial tests are env-gated on
  `T3CLAW_HOOKS_POSTGRES_URL` / `DATABASE_URL` and skip (passing) when no
...
- If production persistence behavior changes, keep PostgreSQL/libSQL parity:
  update the libSQL counterpart (`t3claw_hooks_libsql`) and the
  cross-backend parity suite (`t3claw_hooks_parity`) in lockstep.

## Agent Notes

- Keep edits inside this crate unless the trait contract in `t3claw_hooks`
