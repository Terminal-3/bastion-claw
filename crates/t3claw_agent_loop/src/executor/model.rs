...
pub(super) struct ModelInput {
    ...
    pub(super) messages: Vec<t3claw_turns::run_profile::LoopModelMessage>,
    pub(super) surface_version: t3claw_turns::run_profile::CapabilitySurfaceVersion,
...
pub(super) enum ModelStep {
    Response(
        ...
        t3claw_turns::run_profile::LoopModelResponse,
    ),
            ...
            match ctx.host.stream_model(request.clone()).await {
                Ok(response) => {
                    match &response.output {
                        t3claw_turns::run_profile::ParentLoopOutput::AssistantReply(reply) => {
                            debug!(
                                iteration = state.iteration,
                        ...
                        t3claw_turns::run_profile::ParentLoopOutput::CapabilityCalls(calls) => {
                            debug!(
                                iteration = state.iteration,
