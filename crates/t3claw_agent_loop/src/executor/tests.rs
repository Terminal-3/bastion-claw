use t3claw_host_api::{ApprovalRequestId, CorrelationId, ResourceEstimate};
use t3claw_turns::{
...
async fn reply_admission_rejects_candidate_before_finalizing_and_continues() {
    ...
    let host = MockHost::new(vec![reply_response(), calls_response(), reply_response()])
        .with_batch_outcomes(vec![t3claw_turns::run_profile::CapabilityBatchOutcome {
...
async fn capability_stage_returns_after_batch_summary() {
    ...
    let step = CapabilityStage
        .process(
            ...
            CapabilityInput {
                state,
                surface: t3claw_turns::run_profile::LoopCapabilityPort::visible_capabilities(
                    &host,
...
async fn terminate_hint_after_batch_completes_without_extra_model_call() {
    ...
    let completed = host
        ...
        .find_map(|event| match event {
            t3claw_turns::run_profile::LoopProgressEvent::CapabilityBatchCompleted {
                result_count,
                denied_count,
                gated_count,
                failed_count,
        ...
        let host = MockHost::new(vec![calls_response()])
            .with_batch_outcomes(vec![t3claw_turns::run_profile::CapabilityBatchOutcome {
                outcomes: vec![CapabilityOutcome::Failed(
                    t3claw_turns::run_profile::CapabilityFailure {
                        error_kind,
    ...
    let host = MockHost::new(vec![calls_response()])
        .with_batch_outcomes(vec![t3claw_turns::run_profile::CapabilityBatchOutcome {
            outcomes: vec![CapabilityOutcome::Denied(
                t3claw_turns::run_profile::CapabilityDenied {
                    reason_kind:
                        t3claw_turns::run_profile::CapabilityDeniedReasonKind::EmptySurface,
    ...
    let host = MockHost::new(vec![provider_two_calls_response(), reply_response()])
        .with_batch_outcomes(vec![t3claw_turns::run_profile::CapabilityBatchOutcome {
            outcomes: vec![
                ...
                CapabilityOutcome::Denied(t3claw_turns::run_profile::CapabilityDenied {
                    reason_kind:
                        t3claw_turns::run_profile::CapabilityDeniedReasonKind::EmptySurface,
                    safe_summary: "provider call denied".to_string(),
                }),
...
async fn repeated_model_visible_capability_failures_stop_with_no_progress_fallback() {
    ...
    .with_batch_outcomes(
        (0..3)
            .map(|_| t3claw_turns::run_profile::CapabilityBatchOutcome {
                outcomes: vec![CapabilityOutcome::Failed(
                    t3claw_turns::run_profile::CapabilityFailure {
...
async fn repeated_model_visible_multi_call_failures_stop_with_no_progress_fallback() {
    ...
    .with_batch_outcomes(
        (0..3)
            .map(|_| t3claw_turns::run_profile::CapabilityBatchOutcome {
                outcomes: vec![
                    CapabilityOutcome::Failed(t3claw_turns::run_profile::CapabilityFailure {
                        error_kind: CapabilityFailureKind::OperationFailed,
...
async fn executor_post_capability_trips_policy_and_sets_flags_in_final_state() {
    ...
    assert_eq!(
        ...
        Some(t3claw_turns::run_profile::CompactionInitiator::CapabilityResultOverflow),
...
async fn executor_emits_compaction_started_with_capability_result_overflow_initiator() {
    ...
    let compaction_started_events: Vec<_> = progress_events
        ...
        .filter(|event| {
            matches!(
                event,
                t3claw_turns::run_profile::LoopProgressEvent::CompactionStarted { .. }
            )
        })
        .collect();
    assert_eq!(
    ...
    match compaction_started_events[0] {
        t3claw_turns::run_profile::LoopProgressEvent::CompactionStarted { initiator, .. } => {
            assert_eq!(
                initiator,
                &t3claw_turns::run_profile::CompactionInitiator::CapabilityResultOverflow,
