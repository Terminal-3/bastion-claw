                ...
                invocations: visible_calls
                    ...
                    .map(|call| {
                        let resume = pending_approval_resume
                            ...
                            .map(
                                |resume| t3claw_turns::run_profile::CapabilityApprovalResume {
...
impl CapabilityStage {
    ...
    async fn handle_capability_error(
        ...
        mut model_observation: Option<t3claw_turns::run_profile::ModelVisibleToolObservation>,
        capability_batch: &mut CapabilityBatchTurnSummary,
    ) -> Result<BatchStep, AgentLoopExecutorError> {
        clear_matching_pending_approval_resume(&mut state, &call);
    ...
    async fn fail_unsupported_process_wait(
        ...
        mut state: LoopExecutionState,
        call: &CapabilityCallCandidate,
        _process_ref: &t3claw_turns::run_profile::LoopProcessRef,
    ) -> Result<BatchStep, AgentLoopExecutorError> {
...
async fn append_spawned_child_result(
    host: &(dyn t3claw_turns::run_profile::AgentLoopDriverHost + Send + Sync),
    state: &mut LoopExecutionState,
    call: &CapabilityCallCandidate,
...
async fn append_blocked_capability_error_result(
    ...
    call: &CapabilityCallCandidate,
    summary: &CapabilityErrorSummary,
    model_observation: Option<t3claw_turns::run_profile::ModelVisibleToolObservation>,
    capability_batch: &mut CapabilityBatchTurnSummary,
) -> Result<(), AgentLoopExecutorError> {
...
) -> Option<(t3claw_turns::LoopGateRef, CapabilityCallCandidate)> {
    let mut shared_gate: Option<t3claw_turns::LoopGateRef> = None;
...
mod tests {
    ...
    fn call(input: &str) -> CapabilityCallCandidate {
        let capability_id = t3claw_host_api::CapabilityId::new("test.cap").unwrap();
