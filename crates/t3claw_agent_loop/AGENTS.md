...
- `AgentLoopDriver` / `PlannedDriver` host wiring; that bridge belongs in `t3claw_reborn`.
...
- Fast local check: `cargo test -p t3claw_agent_loop`
- Boundary check after dependency/API changes: `cargo test -p t3claw_architecture`
- Run `cargo test -p t3claw_reborn` when changes affect drivers or loop-host integration.
