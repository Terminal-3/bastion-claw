...
- This crate depends upward on neutral contracts in `t3claw_turns`.
- This crate must not depend on `t3claw_reborn`, host runtime crates, product
...
- The framework never sees `AgentLoopDriver`; `PlannedDriver` in
  `t3claw_reborn` adapts runner-facing driver calls to this crate's executor.
