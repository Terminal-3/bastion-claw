...
/// Write the current log ring buffer to a timestamped file under `~/.t3claw/logs/`
/// (falls back to `$T3CLAW_HOME/logs/` or the current directory). Returns the
...
/// without racing `T3CLAW_HOME` env mutation across threads.
...
fn log_output_dir() -> Option<std::path::PathBuf> {
    if let Ok(custom) = std::env::var("T3CLAW_HOME") {
        return Some(std::path::PathBuf::from(custom).join("logs"));
    }
    if let Ok(home) = std::env::var("HOME") {
        return Some(std::path::PathBuf::from(home).join(".t3claw").join("logs"));
    }
