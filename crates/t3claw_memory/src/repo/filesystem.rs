...
//! [`RootFilesystem`] trait from `t3claw_filesystem`. This is the
//! single Reborn-native memory repository — the per-backend
//! `Reborn{LibSql,Postgres}MemoryDocumentRepository` SQL repos were
//! collapsed onto this implementation once `t3claw_filesystem` grew
...
//! expose only CAS (see `t3claw_filesystem/CLAUDE.md` invariant 2).
    ...
    F: RootFilesystem + ?Sized + 'static,
{
    ...
    async fn query_all(
        ...
        prefix: &VirtualPath,
        filter: &Filter,
    ) -> Result<Vec<t3claw_filesystem::VersionedEntry>, FilesystemError> {
        let mut out = Vec::new();
    ...
    async fn read_document_with_version(
        ...
        path: &MemoryDocumentPath,
        operation: FilesystemOperation,
    ) -> Result<Option<(Vec<u8>, t3claw_filesystem::RecordVersion)>, FilesystemError> {
    ...
    fn document_relative_path_from_chunk_entry(
        entry: &t3claw_filesystem::VersionedEntry,
    ) -> Option<String> {
        match entry
            .entry
            .indexed
    ...
    fn map_chunk_hits_to_ranked_results(
        scope: &MemoryDocumentScope,
        entries: Vec<t3claw_filesystem::VersionedEntry>,
