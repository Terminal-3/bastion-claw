...
- Durable storage adapters live in `t3claw_reborn_openai_compat_storage`;
...
`t3claw_reborn_composition::build_openai_compat_route_mount` performs that
host wiring for `t3claw-reborn serve` by mounting the router inside the
...
- This crate still must not call v1 gateway handlers, raw `SseManager`/
  `AppEvent` streams, `t3claw_llm`, `TurnCoordinator`, projection internals,
