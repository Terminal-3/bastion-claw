...
pub const REBORN_HOME_ENV: &str = "T3CLAW_REBORN_HOME";

const V1_BASE_DIR_ENV: &str = "T3CLAW_BASE_DIR";
...
impl RebornHome {
    ...
    ///
    /// `$T3CLAW_REBORN_HOME/config.toml`. The file is **optional**:
    ...
    ///
    /// `$T3CLAW_REBORN_HOME/providers.json`. Same JSON shape as v1's
    /// `~/.t3claw/providers.json` so the same editor tooling and
    ...
    /// via `t3claw_llm::ProviderRegistry`; this accessor just hands
    /// out the path.
...
fn default_home_from_candidate(
    ...
    Ok(path.join(".t3claw").join("reborn"))
}
...
fn default_v1_state_root_from_home(raw_home: &OsString) -> Option<PathBuf> {
    validated_absolute_candidate(raw_home).map(|path| path.join(".t3claw"))
}
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::V1StateRoot { name, .. } => {
                write!(
                    formatter,
                    "{name} must not point at the v1 T3Claw state root"
                )
            }
