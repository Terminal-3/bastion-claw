...
///
/// The first-run seed is intentionally smaller than `t3claw-reborn config
...
fn first_run_config_toml() -> String {
    ...
    format!(
        r#"# T3Claw Reborn first-run configuration.
#
# This sparse file is created automatically the first time an
# `t3claw-reborn` command starts the runtime. It records the stable,
...
#   t3claw-reborn config init --force
#
...
mod tests {
    ...
    fn seed_default_config_file_writes_sparse_first_run_config() {
        ...
        assert!(
            text.contains("api_version = \"t3claw.runtime/v1\""),
    ...
    fn seed_default_config_file_does_not_clobber_existing_config() {
        ...
        std::fs::write(&path, "api_version = \"t3claw.runtime/v1\"\n").expect("write config");
        ...
        assert_eq!(text, "api_version = \"t3claw.runtime/v1\"\n");
    }
