//! Boot configuration contracts for the standalone T3Claw Reborn binary.
//!
//! This crate is intentionally small and has no T3Claw workspace dependencies.
//! It owns process/environment boot configuration that must be shared by the
//! `t3claw-reborn` binary and later Reborn runtime composition without pulling
...
//!   `$T3CLAW_REBORN_HOME/config.toml`. Read once at process start;
//!   provides the *selection* layer of the three-layer config model
//!   (catalog → selection → runtime config). See `config_file.rs`.
//! - Provider catalog — lives in `$T3CLAW_REBORN_HOME/providers.json`
...
//!   `t3claw_llm::ProviderRegistry` in the composition root (this
