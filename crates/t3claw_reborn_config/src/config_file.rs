...
//!
//! Operator-facing file at `$T3CLAW_REBORN_HOME/config.toml`. Read once
//! at process start by `t3claw-reborn run`. Provides the *selection*
//! layer of the three-layer config model:
//!
//! - **Catalog**: `providers.json` (this crate exposes the path; the
//!   composition root loads the file via `t3claw_llm::ProviderRegistry`).
...
//!
//! Layering note: this crate must stay free of T3Claw workspace
...
//! typed counterparts live in `t3claw_host_api` (TenantId, AgentId,
//! UserId, ProjectId, DeploymentMode, RuntimeProfile, ApprovalPolicy) or
//! `t3claw_reborn_composition` (RebornDriverChoice, RebornHarnessId).
...
/// `t3claw_reborn_composition::RebornRuntimeApiVersion::V1`. A future
/// major bump fails parse closed; minor bumps are accepted.
pub const REBORN_CONFIG_API_VERSION: &str = "t3claw.runtime/v1";
...
pub struct RebornConfigFile {
    /// API version. When set, must be parseable as `t3claw.runtime/vN.M`
    ...
    /// `t3claw_reborn_webui_ingress` when the standalone CLI's
    ...
    /// `t3claw-reborn serve` only when the binary is built with the
...
pub struct PolicySection {
    ...
    /// `t3claw_host_api::runtime_policy::DeploymentMode`.
...
/// Stringly typed by design — the `t3claw_reborn_config` crate stays
...
pub struct WebuiSection {
    ...
    /// at composition time (`t3claw-reborn serve` accepts `0` only
    ...
    /// `T3CLAW_REBORN_WEBUI_TOKEN`. The token VALUE never appears in
    ...
    /// `T3CLAW_REBORN_WEBUI_USER_ID`. Stringly typed; composition
...
pub struct SlackSection {
    ...
    /// is not mounted by `t3claw-reborn serve`.
...
/// here. Env vars (`T3CLAW_TRIGGER_POLLER_*`) override this section.
#[derive(Debug, Clone, Default, Deserialize)]
#[serde(deny_unknown_fields)]
...
fn validate_api_version(found: &str, path: &Path) -> Result<(), RebornConfigFileError> {
    // Expected shape: `t3claw.runtime/vMAJOR.MINOR` (minor optional).
    let Some(rest) = found.strip_prefix("t3claw.runtime/v") else {
        return Err(RebornConfigFileError::InvalidApiVersion {
            path: path.display().to_string(),
            found: found.to_string(),
            reason: "expected prefix `t3claw.runtime/v`".to_string(),
        });
    };
...
"#;
        ...
        assert_eq!(cfg.api_version.as_deref(), Some("t3claw.runtime/v1"));
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some("T3CLAW_REBORN_SLACK_SIGNING_SECRET")
        );
    }
...
"#;
    ...
    fn rejects_url_shaped_storage_backend_without_echoing_credentials() {
        let toml = r#"
[storage]
backend = "postgres://user:password@db.example.com/t3claw"
"#;
        let err = RebornConfigFile::parse_text(toml, &attributed())
...
[storage]
backend = "postgres"
url_env = "postgres://user:password@db.example.com/t3claw?sslmode=require"
"#;
        let err = RebornConfigFile::parse_text(toml, &attributed())
...
backend = "postgres"
url_env = "T3CLAW_REBORN_POSTGRES_URL"
secret_master_key_env = "postgres://user:password.example.com/t3claw"
"#;
        let err = RebornConfigFile::parse_text(toml, &attributed())
...
"#;
    ...
    fn accepts_v1_minor_bumps_forward_compat() {
        for version in ["t3claw.runtime/v1.0", "t3claw.runtime/v1.7"] {
    ...
    fn rejects_malformed_api_version() {
        let toml = r#"
api_version = "t3claw.runtime/notaversion"
"#;
        let err = RebornConfigFile::parse_text(toml, &attributed())
    ...
    fn rejects_malformed_api_version_minor() {
        let toml = r#"
api_version = "t3claw.runtime/v1.not-a-number"
"#;
        let err = RebornConfigFile::parse_text(toml, &attributed())
