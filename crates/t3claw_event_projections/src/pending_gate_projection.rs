...
use t3claw_host_api::{AgentId, ProjectId, TenantId, ThreadId, Timestamp, UserId};
use t3claw_turns::{
...
impl TurnEventSink for PendingGateProjection {
    async fn publish(&self, event: TurnLifecycleEvent) -> Result<(), t3claw_turns::TurnError> {
        self.project_event(event)
            .await
            .map_err(|error| match error {
                ProjectionError::InvalidRequest { reason } => {
                    t3claw_turns::TurnError::InvalidRequest {
                ...
                ProjectionError::TurnEventRebaseRequired { .. } => {
                    t3claw_turns::TurnError::Unavailable {
                        reason: "pending gate projection failed: turn event rebase required"
                            .to_string(),
                    }
                }
                ProjectionError::RebaseRequired { .. } => t3claw_turns::TurnError::Unavailable {
                    reason: "pending gate projection failed: runtime rebase required".to_string(),
                },
                ProjectionError::Source { operation } => t3claw_turns::TurnError::Unavailable {
