...
use t3claw_turns::{GateRef, TurnBlockedGateKind, TurnEventKind, TurnRunId, TurnStatus};
...
async fn publish_maps_projection_failure_to_turn_unavailable() {
    ...
    match err {
        t3claw_turns::TurnError::Unavailable { reason } => {
...
async fn publish_maps_invalid_projection_failure_to_turn_invalid_request() {
    ...
    match err {
        t3claw_turns::TurnError::InvalidRequest { reason } => {
