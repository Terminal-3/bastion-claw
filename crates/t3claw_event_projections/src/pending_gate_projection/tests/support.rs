...
use t3claw_turns::{GateRef, TurnBlockedGateMetadata, TurnStatus};
...
impl TurnEventProjectionSource for MemoryTurnEventSource {
    async fn read_turn_events_after(
        ...
        limit: usize,
    ) -> Result<t3claw_turns::TurnEventPage, t3claw_turns::TurnError> {
        self.requested_limits
            .lock()
            .expect("requested limits lock")
            .push(limit);
...
impl TurnEventProjectionSource for FailingTurnEventSource {
    async fn read_turn_events_after(
        ...
        Err(t3claw_turns::TurnError::Unavailable {
