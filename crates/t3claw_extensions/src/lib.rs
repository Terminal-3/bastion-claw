//! Extension manifest and registry contracts for T3Claw Reborn.
//!
//! `t3claw_extensions` discovers and validates extension packages, extracts
...
use t3claw_trust::TrustPolicyInput;
use thiserror::Error;
...
impl ExtensionPackage {
    ...
    /// pass it to `t3claw_trust::TrustPolicy::evaluate` to get an effective
    /// [`t3claw_trust::TrustDecision`].
...
impl ExtensionDiscovery {
    ...
    fn extension_dir_id(entry: &t3claw_filesystem::DirEntry) -> Option<ExtensionId> {
    ...
    async fn load_package_entry<F>(
        fs: &F,
        root: &VirtualPath,
        entry: &t3claw_filesystem::DirEntry,
        expected: ExtensionId,
        source: ManifestSource,
        host_port_catalog: &HostPortCatalog,
        host_api_contracts: &HostApiContractRegistry,
