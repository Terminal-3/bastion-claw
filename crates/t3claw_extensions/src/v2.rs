...
//! - extension IDs starting with `t3claw.` are reserved for HostBundled;
...
pub const RESERVED_HOST_BUNDLED_ID_PREFIX: &str = "t3claw.";
...
///
/// This is a *structural* bound enforced by `t3claw_extensions` at parse
...
/// registrar (`t3claw_hooks::registrar::MAX_HOOKS_PER_EXTENSION`); the
...
/// TOML payload (see [`HookSectionEntryV2`]) so `t3claw_extensions` never
...
/// `t3claw_hooks::HookManifestEntry` (which applies its own field-level
...
///
/// **Clean-boundary contract:** `t3claw_extensions` is substrate and must
/// not depend on `t3claw_hooks`. So this DTO does *not* embed the typed
/// hook entry (`t3claw_hooks::manifest::HookManifestEntry`). Instead it
...
/// loader (`t3claw_reborn_composition`, which depends on *both* crates)
...
pub struct HookSectionEntryV2 {
    ...
    /// `t3claw_hooks::HookManifestEntry` by the composition loader (via
...
/// `t3claw.product_adapter/v1` or `t3claw.capability_provider/v1`. It is
...
///
/// `t3claw_extensions` owns the generic envelope and section dispatch. Domain
...
pub struct ExtensionManifestV2 {
    ...
    /// only ever comes from `t3claw_trust::TrustPolicy::evaluate` on a
    ...
    /// loader projects them into typed `t3claw_hooks::HookManifestEntry`
...
pub enum ManifestV2Error {
    ...
    #[error(
        "manifest source {manifest_source:?} is not allowed to use legacy top-level capabilities; declare t3claw.capability_provider/v1 host_api instead"
    )]
...
mod tests {
    ...
    fn product_auth_source() -> RuntimeCredentialRequirementSource {
        RuntimeCredentialRequirementSource::ProductAuthAccount {
            provider: t3claw_host_api::RuntimeCredentialAccountProviderId::new("google").unwrap(),
            setup: t3claw_host_api::RuntimeCredentialAccountSetup::ManualToken,
        }
    }
