...
[[capabilities]]
id = "t3claw.memory.native.context.retrieve"
...
fn rejects_reserved_id_prefix_for_installed_source() {
    let toml = third_party_wasm_manifest("t3claw.fake", "t3claw.fake.echo");
...
fn host_bundled_accepts_non_reserved_id() {
    // Spec: the `t3claw.` prefix is reserved *for* HostBundled. It is not
...
fn rejects_manifest_exceeding_max_size() {
    use t3claw_extensions::{MAX_MANIFEST_BYTES, ManifestV2Error};
...
fn rejects_unknown_host_api_fail_closed() {
    ...
    let toml = telegram_multi_host_api_manifest()
        .replace("t3claw.product_adapter/v1", "t3claw.unknown/v1");
