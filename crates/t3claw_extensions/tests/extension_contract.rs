...
async fn discovery_validates_host_api_manifest_with_supplied_contracts() {
    ...
    contracts
        .register(std::sync::Arc::new(TestProductAdapterContract {
            id: HostApiId::new("t3claw.product_adapter/v1").unwrap(),
        }))
        .unwrap();
...
//
// `t3claw_extensions` carries hook declarations as structurally-typed
...
// semantic projection into typed `t3claw_hooks::HookManifestEntry` values
