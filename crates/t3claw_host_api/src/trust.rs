//! Requested-trust vocabulary for T3Claw Reborn.
//!
...
//! and [`RequestedTrustClass`]; the host policy engine in `t3claw_trust`
...
//!   eventual built-in tools (see `crates/t3claw_trust/CONTRACT.md` §9 for
//!   the migration path).
...
//!   from `t3claw_trust::TrustPolicy::evaluate` and is attached to
//!   [`crate::ExecutionContext::trust`] at dispatch time.
//!
//! See `t3claw_trust::TrustPolicy` for the engine that bridges request to
//! effective trust, `crates/t3claw_trust/CONTRACT.md` for the full
...
/// their own and must be matched against host policy in `t3claw_trust`
...
/// privileged request is decided by `t3claw_trust::TrustPolicy::evaluate`,
/// not by parsing.
...
/// | Built-in tools (eventual) | `Bundled` | See `crates/t3claw_trust/CONTRACT.md` §9 for migration |
///
...
/// treated as the same package — the `t3claw_trust` policy binds trust to
