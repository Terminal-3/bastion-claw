//! Runtime policy vocabulary for T3Claw Reborn.
//!
...
//! resolver lives in `t3claw_runtime_policy` (or
//! `t3claw_host_runtime::policy`) and is the only sanctioned producer of
//! [`EffectiveRuntimePolicy`].
//!
//! ## What lives here
//!
//! - [`DeploymentMode`] — where T3Claw is running and who owns the machine
//!   boundary.
...
//!   `t3claw_authorization` / future `t3claw_capabilities`.
...
/// Where T3Claw is running and who owns the machine boundary.
///
...
///
/// Profiles are vocabulary; the resolver in `t3claw_runtime_policy` turns
...
/// `t3claw_authorization`) still run on top.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
#[non_exhaustive]
...
///
/// Produced by the resolver in `t3claw_runtime_policy` from
...
/// `t3claw_runtime_policy`, which is the only sanctioned source. Treat
