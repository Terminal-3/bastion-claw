...
//! decisions* — the moments when a defense-in-depth boundary in T3Claw
...
use t3claw_host_api::{CapabilityId, ResourceScope};
...
mod tests {
    ...
    fn event_can_be_enriched_with_scope_and_capability() {
        use t3claw_host_api::{InvocationId, UserId};
        ...
        .expect("valid scope");
        let cap = CapabilityId::new("t3claw.echo".to_string()).expect("valid capability id");
