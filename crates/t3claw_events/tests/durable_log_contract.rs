...
async fn durable_audit_log_appends_and_replays() {
    ...
    let ctx = ExecutionContext::local_default(
        ...
        extension_id(),
        RuntimeKind::Wasm,
        t3claw_host_api::TrustClass::FirstParty,
        Default::default(),
        MountView::default(),
    )
    ...
    let denied = AuditEnvelope::denied(
        &ctx,
        t3claw_host_api::AuditStage::Denied,
...
async fn replay_jsonl_advances_cursor_with_limit() {
    ...
    let first: t3claw_events::EventReplay<RuntimeEvent> =
        replay_jsonl(&bytes, None, 2).expect("first replay");
    assert_eq!(first.entries.len(), 2);
    assert_eq!(first.next_cursor, EventCursor::new(2));

    let second: t3claw_events::EventReplay<RuntimeEvent> =
...
async fn replay_jsonl_with_zero_limit_is_rejected() {
    let bytes = b"";
    let result: Result<t3claw_events::EventReplay<RuntimeEvent>, _> = replay_jsonl(bytes, None, 0);
    assert!(matches!(
        result,
        Err(EventError::InvalidReplayRequest { .. })
    ));
...
async fn replay_jsonl_rejects_malformed_line_rather_than_silently_skipping() {
    ...
    let result: Result<t3claw_events::EventReplay<RuntimeEvent>, _> = replay_jsonl(&bytes, None, 1);
    assert!(matches!(result, Err(EventError::Serialize { .. })));
}
...
async fn replay_jsonl_with_future_cursor_returns_replay_gap() {
    ...
    let result: Result<t3claw_events::EventReplay<RuntimeEvent>, _> =
...
async fn direct_construction_serialize_path_resanitizes_error_kind() {
    ...
    let event = RuntimeEvent {
        event_id: t3claw_events::RuntimeEventId::new(),
