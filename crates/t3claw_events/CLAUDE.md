...
append-log substrate for T3Claw Reborn.
...
`crates/t3claw_reborn_event_store/`. Keep storage drivers out of this
substrate crate: downstream store crates should depend on `t3claw_events`,
...
Forbidden dependencies (enforced by `t3claw_architecture`): authorization,
approvals, capabilities, dispatcher, extensions, host_runtime, secrets,
network, mcp, processes, resources, run_state, scripts, wasm.

`t3claw_filesystem` and `t3claw_memory` are **deliberately not forbidden**
...
`crates/t3claw_architecture/tests/reborn_dependency_boundaries.rs`; if the
