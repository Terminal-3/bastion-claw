# t3claw_hooks — Reborn loop hook framework
...
- The runner-facing `AgentLoopDriver` trait — that stays in `t3claw_turns`.
- The concrete `LoopCapabilityPort` / `LoopPromptPort` / `LoopModelPort` impls —
  those stay in `t3claw_loop_support` and `t3claw_reborn`.
- The Reborn-side middleware composition that wraps host ports — that lives in
  `t3claw_reborn::loop_driver_host` and consumes types from this crate.
...
```
t3claw_turns       -> no dependency on t3claw_hooks
t3claw_hooks       -> depends on t3claw_turns + t3claw_host_api
t3claw_reborn      -> depends on t3claw_hooks for host composition (follow-up)
t3claw_engine      -> no hook ownership; optional future driver consumer
```

Architecture test in `t3claw_architecture::tests::reborn_dependency_boundaries`
proves the `t3claw_turns -> t3claw_hooks` edge stays absent.
...
- **Builtin** — compiled into T3Claw, identity = crate path + symbol. May
  produce any decision kind via `BuiltinHookSink`.
- **Trusted** — user-placed in `~/.t3claw/hooks/` or workspace `hooks/`. Cannot
...
`t3claw_reborn` (or any other internal crate) reads a hook from the
...
- All model-visible hook output is bounded, typed, redacted/trust-labeled, and
  ...
  `t3claw_host_runtime::memory_context` once that helper is extracted).
