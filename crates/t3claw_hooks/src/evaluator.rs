    ...
    pub fn warn_in_memory_backend_active_in_production(&self) {
        tracing::warn!(
            "predicate evaluator is using the in-memory backend: replay dedup is \
             ...
             durable backend (see crates/t3claw_hooks/docs/successors/03-persistent-counter.md)."
        );
    }
...
mod tests {
    ...
    fn tenant() -> t3claw_host_api::TenantId {
        t3claw_host_api::TenantId::new("alpha").expect("ok")
    }
    ...
    async fn invocation_counter_partitions_by_tenant() {
        ...
        let now = base();
        let alpha = t3claw_host_api::TenantId::new("alpha").expect("ok");
        let beta = t3claw_host_api::TenantId::new("beta").expect("ok");
