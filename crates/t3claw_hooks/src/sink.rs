...
pub trait PrivilegedMutatorSink: Send {
    ...
    /// raw body; `t3claw_prompt_envelope::wrap_untrusted` performs the
...
mod tests {
    ...
    async fn restricted_gate_sink_cannot_allow_at_type_level() {
        ...
        let ctx = BeforeCapabilityHookContext::new_unresolved(
            t3claw_host_api::TenantId::new("t".to_string()).expect("valid tenant"),
            "cap.x".to_string(),
            [0u8; 32],
        );
        DenyOnly
    ...
    async fn event_triggered_sink_is_observer_only_at_type_level() {
        ...
        let tenant = t3claw_host_api::TenantId::new("t".to_string()).expect("valid tenant");
        let user = t3claw_host_api::UserId::new("u".to_string()).expect("valid user");
        let invocation_id = t3claw_host_api::InvocationId::new();
        let scope = t3claw_host_api::ResourceScope::local_default(user, invocation_id)
            .expect("valid scope");
        let event = t3claw_events::RuntimeEvent::hook_failed(
            scope,
            t3claw_host_api::CapabilityId::new("hooks.failed").expect("valid capability"),
        ...
        let ctx = EventTriggeredHookContext {
            tenant_id: tenant,
            event: &event,
            event_cursor: t3claw_events::EventCursor::new(1),
            is_replay: false,
        };
