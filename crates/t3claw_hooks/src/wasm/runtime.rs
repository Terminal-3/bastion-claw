...
// Shared resource limiter extracted into `t3claw_wasm_limiter` so both
// `t3claw_hooks` and `t3claw_wasm` depend on it through Cargo rather
...
// `t3claw_hooks -> t3claw_wasm` is preserved.
use t3claw_wasm_limiter::WasmResourceLimiter;
...
pub struct WasmHookModuleRequest<'a> {
    pub extension_id: &'a t3claw_host_api::ExtensionId,
