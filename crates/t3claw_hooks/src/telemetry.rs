...
//! summaries defined in `t3claw_turns`.
//!
//! The `t3claw_turns` crate cannot depend on `t3claw_hooks` (that boundary
//! is enforced by `t3claw_architecture`). To let the dispatcher emit
...
use t3claw_turns::run_profile::HookDecisionSummary;
...
/// in this crate and cannot cross into `t3claw_turns`.
...
mod tests {
    ...
    /// same output as `HookId::to_hex()`. Downstream `t3claw_turns`
