...
//! `t3claw_turns`, and what downstream SSE / audit / replay consumers parse
...
/// `validate_name_segment` limit in `t3claw_host_api::ids` so the two
...
///
/// - [`t3claw_host_api::ExtensionId`] is the *authority-bearing* identifier:
///   validated at construction, compared and trusted across the host.
/// - `t3claw_hooks::identity::ExtensionId` (this type) is a transparent
...
///
/// ```ignore
/// let host_ext: t3claw_host_api::ExtensionId = /* ... */;
/// let identity_ext: t3claw_hooks::identity::ExtensionId = (&host_ext).into();
...
impl From<&t3claw_host_api::ExtensionId> for ExtensionId {
    fn from(host: &t3claw_host_api::ExtensionId) -> Self {
        ...
        // (both mirror `t3claw_host_api::ids::validate_name_segment`).
        ...
        let msg = "t3claw_host_api::ExtensionId is pre-validated and shares the identity grammar";
...
mod tests {
    ...
    /// `LoopHostMilestoneKind::Hook*` variants in `t3claw_turns`, and what
    ...
    fn extension_id_from_host_api_round_trips() {
        let host = t3claw_host_api::ExtensionId::new("github-mcp.v1").expect("valid host id");
    ...
    /// `From<&t3claw_host_api::ExtensionId> for ExtensionId`: if the two
    ...
    fn extension_id_from_host_api_round_trips_grammar_corners() {
        ...
        for raw in corners {
            let host = t3claw_host_api::ExtensionId::new(raw)
