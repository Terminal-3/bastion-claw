...
//! `t3claw_reborn::loop_driver_host` and lands in a follow-up slice.
...
use t3claw_turns::run_profile::{HookDecisionSummary, HookMilestoneSink, LoopHostMilestoneKind};
...
impl HookDispatcher {
    ...
    /// [`t3claw_turns::run_profile::RunScopedHookMilestoneSink`] (or
    ...
    pub(crate) fn count_bindings_for_extension(
        &self,
        extension: &t3claw_host_api::ExtensionId,
    ) -> usize {
    ...
    ///
    /// `owning_extension` is the [`t3claw_host_api::ExtensionId`] of the
    ...
    pub(crate) fn install_installed_before_capability(
        ...
        hook_id: HookId,
        phase: HookPhase,
        owning_extension: t3claw_host_api::ExtensionId,
    ...
    pub(crate) fn install_observer(
        ...
        owning_extension: Option<t3claw_host_api::ExtensionId>,
        scope: HookBindingScope,
    ...
    pub async fn dispatch_observer_at(
        &self,
        point: HookPointSpec,
        tenant: t3claw_host_api::TenantId,
    ...
    /// hooks are fed by durable event-log replay in `t3claw_reborn`, not by
    ...
    fn scope_provider_for_runtime_event(
        &self,
        event: &RuntimeEvent,
    ) -> Option<t3claw_host_api::ExtensionId> {
    ...
    /// [`t3claw_host_api::CapabilityId`] on a best-effort basis. If the
    ...
    fn record_capability_block_audit(&self, capability_name: &str) {
        ...
        if let Ok(cap_id) = t3claw_host_api::CapabilityId::new(capability_name.to_string()) {
    ...
    /// SelfAuthored hooks).
    fn lookup_owning_extension(&self, hook_id_hex: &str) -> Option<t3claw_host_api::ExtensionId> {
...
/// use std::sync::Arc;
/// use std::time::Duration;
/// use t3claw_hooks::dispatch::HookDispatcherBuilder;
/// use t3claw_hooks::registry::HookRegistry;
///
...
mod tests {
    ...
    fn tenant() -> t3claw_host_api::TenantId {
        t3claw_host_api::TenantId::new("alpha").expect("tenant ok")
    }

    fn host_ext() -> t3claw_host_api::ExtensionId {
        t3claw_host_api::ExtensionId::new("ext").expect("ext id ok")
    }
    ...
    async fn hook_deny_with_unparseable_capability_name_records_event_without_capability_id() {
        ...
        // else-arm.
        assert!(
            t3claw_host_api::CapabilityId::new(String::new()).is_err(),
    ...
    async fn multiple_phase_denies_record_exactly_one_hook_deny_event() {
        use t3claw_events::{InMemorySecurityAuditSink, SecurityAuditSink, SecurityBoundary};
    ...
    async fn pause_approval_does_not_record_hook_deny_event() {
        use t3claw_events::{InMemorySecurityAuditSink, SecurityAuditSink};
    ...
    async fn own_capabilities_observer_filters_foreign_providers() {
        let owner = t3claw_host_api::ExtensionId::new("ext.owner").expect("ok");
        let other = t3claw_host_api::ExtensionId::new("ext.other").expect("ok");
    ...
    fn compile_time_seal_test() {
        ...
        //
        // Reachable only from inside `t3claw_hooks`. External callers must
    ...
    /// `t3claw_reborn` reads a registry-sourced extension hook and
    ...
    /// "Loader responsibility" section in `crates/t3claw_hooks/CLAUDE.md`.
    ///
    ...
    async fn tier_specific_installers_are_documented_as_loader_contract() {
        ...
        dispatcher
            .install_installed_before_capability(
                installed_cap,
                HookPhase::Policy,
                t3claw_host_api::ExtensionId::new("loader-installed").expect("valid ext"),
    ...
    use t3claw_turns::run_profile::{InMemoryHookMilestoneSink, LoopHostMilestoneKind};
    ...
    fn host_ext_named(name: &str) -> t3claw_host_api::ExtensionId {
        t3claw_host_api::ExtensionId::new(name).expect("valid ext id")
    }
    ...
    fn event_resource_scope() -> t3claw_host_api::ResourceScope {
        let user = t3claw_host_api::UserId::new("user-ev").expect("valid user");
        let invocation = t3claw_host_api::InvocationId::new();
        t3claw_host_api::ResourceScope::local_default(user, invocation).expect("valid scope")
    }

    fn event_capability() -> t3claw_host_api::CapabilityId {
        t3claw_host_api::CapabilityId::new("event.fixture").expect("valid capability")
    }
    ...
    async fn providerless_hook_meta_event_resolves_provider_via_registry() {
        let owner = t3claw_host_api::ExtensionId::new("ext").expect("valid extension id");
    ...
    async fn hook_failed_with_spoofed_provider_does_not_fire_target_extension_hooks() {
        let ext_a = t3claw_host_api::ExtensionId::new("ext-a").expect("valid extension id");
        let ext_b = t3claw_host_api::ExtensionId::new("ext-b").expect("valid extension id");
    ...
    async fn unknown_lifecycle_hook_id_with_carried_provider_stays_inert() {
        let target = t3claw_host_api::ExtensionId::new("target").expect("valid extension id");
