...
use t3claw_turns::LoopResultRef;
...
impl crate::middleware::gate_ref::HookGateRefFactory for FailingGateRefFactory {
    async fn mint_approval_ref(
        &self,
        _reason: &str,
    ) -> Result<t3claw_turns::LoopGateRef, AgentLoopHostError> {
...
fn snapshot_fixture_invocation() -> CapabilityInvocation {
    CapabilityInvocation {
        surface_version: t3claw_turns::run_profile::CapabilitySurfaceVersion::new("snapshot:v1")
        ...
        input_ref: t3claw_turns::run_profile::CapabilityInputRef::new(
...
fn invocation_arguments_digest_differs_for_different_input_refs() {
    ...
    let surface = t3claw_turns::run_profile::CapabilitySurfaceVersion::new("v").expect("ok");
    let a = CapabilityInvocation {
        surface_version: surface.clone(),
        capability_id: cap_id.clone(),
        input_ref: t3claw_turns::run_profile::CapabilityInputRef::new("input:a").expect("ok"),
        approval_resume: None,
    };
    let b = CapabilityInvocation {
        surface_version: surface,
        capability_id: cap_id,
        input_ref: t3claw_turns::run_profile::CapabilityInputRef::new("input:b").expect("ok"),
        approval_resume: None,
    };
    assert_ne!(
...
fn invocation_arguments_digest_differs_for_different_capability_ids() {
    let surface = t3claw_turns::run_profile::CapabilitySurfaceVersion::new("v").expect("ok");
    let input_ref =
        t3claw_turns::run_profile::CapabilityInputRef::new("input:shared").expect("ok");
    let a = CapabilityInvocation {
        surface_version: surface.clone(),
...
use t3claw_host_api::ExtensionId as HostExtensionId;
...
fn install_predicate_hook(
    ...
    dispatcher
        .install_installed_before_capability(
            hook_id,
            HookPhase::Policy,
            t3claw_host_api::ExtensionId::new("ext-test").expect("valid"),
