...
use t3claw_turns::TurnCheckpointId;
use t3claw_turns::run_profile::{AgentLoopHostError, LoopCheckpointPort, LoopCheckpointRequest};
...
mod tests {
    ...
    use async_trait::async_trait;
    use std::sync::Mutex;
    use t3claw_turns::run_profile::{LoopCheckpointKind, LoopCheckpointStateRef};
            ...
            *self.calls.lock().expect("not poisoned") += 1;
            if self.fail {
                return Err(AgentLoopHostError::new(
                    t3claw_turns::run_profile::AgentLoopHostErrorKind::CheckpointRejected,
