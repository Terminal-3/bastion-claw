...
//! [`t3claw_prompt_envelope`] helper — the same primitive memory-context
...
use t3claw_prompt_envelope::{EnvelopeSource, EnvelopeTrust, wrap_untrusted};
use t3claw_turns::LoopMessageRef;
...
/// adapter-wrap [`t3claw_turns::run_profile::InstructionMaterializationStore`]
...
/// so this seam stays narrow and keeps `t3claw_hooks` decoupled from
...
pub struct HookedLoopPromptPort {
    ...
    /// [`t3claw_turns::run_profile::InstructionMaterializationStore`]);
...
mod tests {
    ...
    use std::sync::Mutex;
    use t3claw_turns::run_profile::{LoopPromptBundle, LoopPromptBundleRef, PromptMode};
    ...
    impl PrivilegedBeforePromptHook for TrustedHijackHook {
        async fn evaluate(
            ...
            // `t3claw_prompt_envelope`. The trusted-snippet constructor
