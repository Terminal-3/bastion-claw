...
> Purpose: validate the T3Claw hooks design against well-established
> hook/policy/extension systems. For each axis where T3Claw diverges,
...
| **ICLAW** | T3Claw `t3claw_hooks` | LLM agent loop |
...
## Where T3Claw stands out (vs prior art)
...
## Where T3Claw is conventional (and should be)
...
## Where T3Claw is conventional but **shouldn't** be (open questions)
...
## Where T3Claw diverges but the *why* is weak (review needed)
...
- Axes were chosen *after* drafting T3Claw's design, so the table is unavoidably colored by ICLAW's vocabulary. A second pass with axes chosen from one of the survey systems (e.g., K8S's admission-controller checklist) would be a useful adversarial check.
