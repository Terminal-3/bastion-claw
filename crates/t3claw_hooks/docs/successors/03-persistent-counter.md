...
the rest of the project (`src/db/mod.rs`, `t3claw_events`). The
...
driven by `event_id` (which is `RuntimeEventId` from `t3claw_events`,
...
- **`InMemoryPredicateStateBackend`**: keeps current behavior; used
  for tests + the standalone `t3claw_hooks` integration tests that
...
`t3claw_hooks::predicate_state::PredicateStateBackend` is `pub`,
...
|---------|-------|----------|-------------------|
| `InMemoryPredicateStateBackend` | `t3claw_hooks` | no (process-local) | **no** — dedup is process-local |
| `PostgresPredicateStateBackend` | `t3claw_hooks_postgres` | yes | yes (SQL `UNIQUE`/`PRIMARY KEY`) |
| `LibSqlPredicateStateBackend` | `t3claw_hooks_libsql` | yes | yes (SQL `UNIQUE`/`PRIMARY KEY`) |
...
The cross-backend adversarial parity suite (`t3claw_hooks_parity`)
...
- **Trait + in-memory + contract harness** (`t3claw_hooks`, PR 1/4):
  the `predicate_state::contract` module + `predicate_backend_contract_test!`
  macro behind the `contract-tests` feature.
- **Postgres backend + contract + adversarial** (`t3claw_hooks_postgres`,
  PR 2/4): env-gated on `T3CLAW_HOOKS_POSTGRES_URL` / `DATABASE_URL`.
- **libSQL backend + contract + adversarial** (`t3claw_hooks_libsql`,
  PR 3/4): embedded temp-file db, runs anywhere.
- **Cross-backend parity matrix + multi-host adversarial**
  (`t3claw_hooks_parity`, PR 4/4): one scripted sequence fed to all
