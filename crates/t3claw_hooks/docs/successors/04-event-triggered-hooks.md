...
```rust
// in t3claw_hooks::points::event_triggered
//
...
- **Subscription side**: a new dispatcher path
  ...
  reborn factory wires it to `t3claw_events`'s event-stream
...
- Hook dispatch receives the durable `RuntimeEvent` directly. `t3claw_hooks`
  depends on `t3claw_events` as of PR #3640 — the milestone projection in
  PR #3573 covered `t3claw_reborn`'s milestone wiring, but the hook crate
  ...
  network, WASM, or Reborn internals; the `t3claw_architecture` boundary
  test enforces this.

## Cross-cutting constraints

- **Cross-crate boundary**: `t3claw_hooks` already depends on
  `t3claw_events` for the milestone projection (PR #3573), and it
...
6. **Cross-crate boundary**: `t3claw_architecture` test confirms
   `t3claw_hooks` doesn't gain a forbidden dep on
   `t3claw_events` or `t3claw_host_runtime`.
...
- Cross-crate dep direction: `t3claw_events` is already a
  dependency of `t3claw_hooks` (via the milestone projection in
  PR #3573), so the dep direction is established. The new traffic is
  the *consumer-side* of the event stream, which means
  `t3claw_hooks` learns about `EventStreamSubscriber` or whatever
  trait `t3claw_events` exposes.
