...
> Companion to [`crates/t3claw_hooks/tests/real_hooks.rs`](../tests/real_hooks.rs).
>
...
Both seams live in `t3claw_reborn`. From a standalone
`t3claw_hooks` test, the best a hook author can do is:
...
an `t3claw_reborn` integration test
...
either (a) depending on `t3claw_reborn` as a dev-dep (heavy + the
...
2. Ship a `MockCapabilityInputResolver` as part of `t3claw_hooks`'s
...
3. Accept the friction and document that NumericSum hooks must be
   integration-tested via `t3claw_reborn`. Cheapest, but it's a
...
### F5 — `HookId::derive` requires the crate's `identity::ExtensionId`, not `t3claw_host_api::ExtensionId`

**What happened:** A hook author already holds an
`t3claw_host_api::ExtensionId` (the authoritative identifier). To
mint a `HookId` via `HookId::derive`, they need
`t3claw_hooks::identity::ExtensionId` — a different newtype.
...
1. Add a `From<&t3claw_host_api::ExtensionId>` impl for
   `t3claw_hooks::identity::ExtensionId`. One-liner, makes the
...
| F5 — Two `ExtensionId` types are confusing | Low | **Fixed** — `From<&t3claw_host_api::ExtensionId>` impl + cross-link rustdoc |
