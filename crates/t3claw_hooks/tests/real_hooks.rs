//! Three "real" hooks built against the public `t3claw_hooks` API.
//!
...
//!    arguments (the resolver wiring lives in `t3claw_reborn`, so the
...
//!    `crates/t3claw_reborn/tests/hooks_integration.rs`).
//!
...
use t3claw_hooks::HookTrustClass;
use t3claw_hooks::dispatch::HookDispatcherBuilder;
use t3claw_hooks::evaluator::PredicateEvaluator;
use t3claw_hooks::identity::HookLocalId;
use t3claw_hooks::kinds::gate::GateDecisionView;
use t3claw_hooks::kinds::mutator::{HookPatchView, PatchOrdinalHint, SnippetBodyView};
use t3claw_hooks::manifest::{HookManifestBody, HookManifestEntry, HookManifestKind};
use t3claw_hooks::ordering::HookPhase;
use t3claw_hooks::points::{BeforeCapabilityHookContext, BeforePromptHookContext};
use t3claw_hooks::predicate::{
    CapabilityPredicate, HookPredicateSpec, OnExceededAction, ValueOrRateBound,
};
use t3claw_hooks::registrar::HookRegistrar;
use t3claw_hooks::registry::HookRegistry;
use t3claw_hooks::sink::{PrivilegedBeforePromptHook, PrivilegedMutatorSink};
use t3claw_host_api::{ExtensionId, TenantId};
...
async fn polymarket_daily_cap_denies_after_ten_invocations() {
    ...
    let ctx = |digest: [u8; 32]| {
        BeforeCapabilityHookContext::new(
            ...
            digest,
            t3claw_hooks::points::SanitizedArguments::unresolved(),
...
/// the resolver path lives in `t3claw_reborn`, so this test only
/// exercises that the manifest is well-formed and that the registrar
/// accepts it. The dispatch-time fire-or-not test is in
/// `crates/t3claw_reborn/tests/hooks_integration.rs::numeric_sum_predicate_caps_total_value_against_real_inputs`.
...
async fn pii_redaction_warning_injects_trusted_snippet() {
    use t3claw_hooks::identity::{ExtensionId as IdentExtensionId, HookId, HookVersion};

    let hook_id = HookId::derive(
        &IdentExtensionId::new("t3claw-builtin").expect("valid IdentExtensionId in test"),
        "1.0.0",
