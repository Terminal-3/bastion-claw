...
use t3claw_host_api::{AgentId, CapabilityId, InvocationId, ProjectId, TenantId, ThreadId, UserId};
use t3claw_threads::{
...
async fn read_thread_rejects_cross_scope_lookup_with_unknown_thread() {
    ...
    assert!(
        matches!(&err, t3claw_threads::SessionThreadError::UnknownThread { thread_id } if thread_id == &thread.thread_id),
