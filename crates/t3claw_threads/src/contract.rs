...
use t3claw_host_api::{AgentId, MissionId, ProjectId, TenantId, ThreadId, UserId};
...
impl ThreadScope {
    /// Convert into a [`t3claw_host_api::ResourceScope`] suitable for the
    ...
    pub fn to_resource_scope(&self) -> t3claw_host_api::ResourceScope {
        t3claw_host_api::ResourceScope {
            tenant_id: self.tenant_id.clone(),
            user_id: self.owner_user_id.clone().unwrap_or_else(|| {
                t3claw_host_api::UserId::from_trusted(
                    t3claw_host_api::SYSTEM_RESERVED_ID.to_string(),
                )
            }),
