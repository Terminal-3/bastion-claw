...
//! [`ScopedFilesystem`](t3claw_filesystem::ScopedFilesystem). The paths in
//! this module are alias-relative [`ScopedPath`](t3claw_host_api::ScopedPath)
//! strings — at every op the [`ScopedFilesystem`] resolves the alias against
//! its caller-supplied [`MountView`](t3claw_host_api::MountView) and enforces
//! per-grant ACL before backend dispatch. The composition layer wires the
//! alias to a tenant/user-scoped
//! [`VirtualPath`](t3claw_host_api::VirtualPath), so tenant isolation is
...
use t3claw_host_api::{HostApiError, InvocationId, ResourceScope, ScopedPath, ThreadId};
use uuid::Uuid;
