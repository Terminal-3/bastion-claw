...
//! `t3claw_hooks`. The framework crate (`t3claw_hooks`) owns the public
...
//! which lives in its own `t3claw_hooks_postgres` crate for the same reason.
//!
...
pub mod test_support {
    use t3claw_hooks::predicate_hash::{self, Digest};
    use t3claw_hooks::predicate_state::InvocationKey;

    /// `scope_hash` (tenant digest) bytes — see
    /// [`t3claw_hooks::predicate_hash::scope_hash`]. Returns the canonical
    ...
    /// [`t3claw_hooks::predicate_hash::invocation_key_hash`]. Mirrors the
