# t3claw_filesystem guardrails

`t3claw_filesystem` is the **universal storage dispatch fabric** for T3Claw.
...
- Depend on `t3claw_*` system-service or runtime crates other than
  `t3claw_host_api` and `t3claw_safety`.
...
- Run the full crate tests, both feature combinations:
  `cargo test -p t3claw_filesystem --all-features`,
  `cargo check -p t3claw_filesystem --no-default-features --features libsql`,
  `cargo check -p t3claw_filesystem --no-default-features --features postgres`.
