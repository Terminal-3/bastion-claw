//! Capability authorization contracts for T3Claw Reborn.
//!
//! `t3claw_authorization` evaluates authority-bearing host API contracts. It
...
use t3claw_trust::{AuthorityCeiling, TrustDecision};
use thiserror::Error;
...
/// this separate because `t3claw_trust::EffectiveTrustClass` deliberately
...
/// [`CompositeRootFilesystem`](t3claw_filesystem::CompositeRootFilesystem)
/// or the in-memory backend for tests). The [`ScopedFilesystem`] resolves
/// the `/authorization` alias to a tenant/user-scoped
/// [`VirtualPath`](t3claw_host_api::VirtualPath) per its
/// [`MountView`](t3claw_host_api::MountView) and enforces per-op ACL
    ...
    F: RootFilesystem,
{
    ...
    /// fallback shape (`t3claw_processes::put_with_byte_fallback`).
    ...
    /// [`VirtualPath`](t3claw_host_api::VirtualPath) results because
...
/// `t3claw_trust::InvalidationBus` runs listeners synchronously, while this
...
// mount alias. The [`MountView`](t3claw_host_api::MountView) granted by
// composition resolves the alias to a tenant/user-scoped
// [`VirtualPath`](t3claw_host_api::VirtualPath) before any backend op —
...
/// [`VirtualPath`](t3claw_host_api::VirtualPath)s) so the per-op ACL
