//! Approval resolution service for T3Claw Reborn.
//!
//! `t3claw_approvals` resolves durable approval requests and issues scoped
//! authorization leases. It does not prompt users, execute capabilities, or
//! dispatch runtime work.

use t3claw_authorization::{CapabilityLease, CapabilityLeaseError, CapabilityLeaseStore};
use t3claw_events::AuditSink;
use t3claw_host_api::{
...
use t3claw_run_state::{ApprovalRecord, ApprovalRequestStore, ApprovalStatus, RunStateError};
use thiserror::Error;
    ...
    L: CapabilityLeaseStore + ?Sized,
{
    ...
    pub async fn deny(
        &self,
        scope: &ResourceScope,
        request_id: t3claw_host_api::ApprovalRequestId,
        denial: DenyApproval,
    ...
    async fn emit_approval_resolved(
        &self,
        scope: &ResourceScope,
        request: &t3claw_host_api::ApprovalRequest,
        resolved_by: Principal,
        decision: ApprovalDecisionKind,
    ) {
        self.emit_audit_best_effort(t3claw_host_api::AuditEnvelope::approval_resolved(
            scope,
            request,
            resolved_by,
            decision,
    ...
    async fn emit_audit_best_effort(&self, record: t3claw_host_api::AuditEnvelope) {
