...
- `src/app.rs` (and `src/cli/mod.rs`) call `t3claw_embeddings::create_provider(&cfg, ProviderDeps { session, bedrock_setup })` and attach the result to `Workspace` via `with_embeddings_cached`.
