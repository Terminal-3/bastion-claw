...
impl RecordingLlm {
    ...
    /// workspace type out of `t3claw_llm` avoids a heavy reverse dep.
...
mod tests {
    ...
    fn from_env_returns_none_when_unset() {
        let _env_lock = t3claw_common::env_helpers::lock_env();
        let prior = std::env::var_os("T3CLAW_RECORD_TRACE");
        ...
        match prior {
            Some(value) => {
                // SAFETY: Tests serialize env access with lock_env().
                unsafe { std::env::set_var("T3CLAW_RECORD_TRACE", value) };
            }
            None => {
                // SAFETY: Tests serialize env access with lock_env().
                unsafe { std::env::remove_var("T3CLAW_RECORD_TRACE") };
