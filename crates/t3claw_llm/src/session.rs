...
impl SessionManager {
    ...
    pub async fn near_wallet_login(
        ...
        let response = self
            .client
            .post(&url)
            .header(reqwest::header::USER_AGENT, "t3claw")
        ...
        if !response.status().is_success() {
            ...
            let preview = t3claw_common::truncate_for_preview(&body, 200);
...
pub async fn create_session_manager(config: SessionConfig) -> Arc<SessionManager> {
    ...
    if let Some(token) = t3claw_common::env_helpers::env_or_override("NEARAI_SESSION_TOKEN") {
