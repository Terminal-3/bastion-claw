...
impl ProviderRegistry {
    ...
    /// from `~/.t3claw/providers.json` (v1's canonical location).
    ///
    ...
    /// `$T3CLAW_REBORN_HOME/providers.json` here so the two binaries
    /// (v1 and Reborn-standalone) can have independent user catalogs
    /// without colliding on `~/.t3claw/providers.json`.
    ...
    /// explicit `$T3CLAW_REBORN_HOME/providers.json` is present, a syntax
...
fn user_providers_path() -> Option<std::path::PathBuf> {
    Some(t3claw_common::paths::t3claw_base_dir().join("providers.json"))
}
