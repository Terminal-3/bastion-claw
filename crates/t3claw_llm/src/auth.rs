//! Public auth facade for `t3claw_llm`.
//!
//! External callers (the setup wizard, the `t3claw login` subcommand, and
...
///
/// Keeps display/browser-launch concerns out of `t3claw_llm`: the wizard,
...
pub enum LoginRequest {
    ...
    /// persisted to disk by `t3claw_llm`; the caller does not see them.
...
impl OpenAiCodexLoginOptions {
    ...
    ///
    /// Use this from `t3claw login --openai-codex` so config-file
...
/// It is `None` when `t3claw_llm` already persisted credentials on disk
